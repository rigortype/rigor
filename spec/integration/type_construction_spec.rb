...
RSpec.describe "Rigor type construction (integration)" do
  ...
  describe "fixtures/rooted_declaration_header.rb — a rooted header resets to the top level" do
    ...
    it "still asserts every rooted and non-rooted spelling" do
      expect(marked_lines(harness, "assert_type(").size).to eq(21)
    end
    ...
    it "reports the undefined call on both the rooted and the non-rooted receiver" do
      expect(harness.errors.map { |d| [d.line, d.rule] })
        .to eq([[100, "call.undefined-method"], [101, "call.undefined-method"],
                [195, "call.undefined-method"], [196, "call.undefined-method"]])
    end
  end
  ...
  # Issue #1429 (ADR-117 Decision point 3) — class guards, truthiness, `nil?`, `!` and `&.` narrow a global or constant
  # read as they narrow a local; a class guard disjoint from an inferred `Nominal` reads the receiver as `bot` without
  # reporting its clause unreachable; `respond_to?(:m)` admits `m`; and a guard's narrowing of a global or constant is
  # restored where code may run that rebinds it. Each entry runs as `rigor check` runs a project (`analyze(files:)`), so
  # the file's own methods are discovered, and each carries its own global writes.
  ...
  describe "self-asserting `assert_type` calls in converted fixtures" do
    ...
    describe "fixtures/block_next_arm_contribution.rb — issue #841 block `next`-arm inference" do
      let(:harness) { harness_for("block_next_arm_contribution") }

      # Every `next value` that leaves the block joins the fall-through tail in block-return inference, typed
      # in the scope that reaches it; a nested block / lambda / loop retargets its own `next`, and an arm the
      # analysis proves dead contributes nothing. A predicate block with a falsey arm infers `bool`, not
      # `Constant[true]`.
      it "joins `next` arms into the inferred block value type" do
        mismatches = harness.errors.select { |d| d.message.start_with?("assert_type ") }
        expect(mismatches).to be_empty
      end

      # The reported symptom: `BlockFolding` folded `all?` to `Constant[true]` off the tail alone, and the
      # surrounding condition then read as a flow constant on a program that can answer false. The fixture's
      # `all_true?` control keeps a genuine fold alive, so an empty set here is not vacuous.
      it "fires no flow.always-truthy-condition on the predicate with a falsey arm" do
        flow_constants = harness.diagnostics.select { |d| d.rule == "flow.always-truthy-condition" }
        expect(flow_constants).to be_empty
      end
    end
