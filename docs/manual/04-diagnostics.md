...
| <a id="rule-global-write-type-mismatch"></a>`global.write-type-mismatch` | A special global is assigned a value its setter rejects, so the write raises `TypeError` every time it runs: `$/ = 1` or `$/ = /x/` (`$/`, `$-0`, `$,` and `$\` take only a String or nil), `$~ = "x"` (a MatchData or nil), `$0 = nil` (a String), `$. = "3"` (an Integer or a Float), `$stdout = 1` or `$stderr = nil` (`$stdout`, `$>` and `$stderr` take anything that responds to `write`, so `$stdout = StringIO.new` is fine). Ruby's setter decides, not the global's RBS type. Fires only when every possible value is rejected, so a `Dynamic` value or a union with an accepted member stays silent, and only on `$g = value`. A write to `$stdin` is never checked. `warning` under `lenient`. | high |
| <a id="rule-global-readonly-write"></a>`global.readonly-write` | A read-only special global is written — `$!`, `$$`, `$?`, `$<`, `$FILENAME`, `$*`, `$:` / `$LOAD_PATH` / `$-I`, `$"` / `$LOADED_FEATURES`, `$-W`, `$-p`, `$-l` or `$-a` — which raises `NameError` whatever the value. Covers `$g = value`, `$g += value` and a multiple-assignment target. `$LOAD_PATH ||= []` never writes and `$LOAD_PATH << dir` is a method call, so neither fires. An error in every profile. | high |
