...
- **Remaining (all non-gating, demand-driven; each its own behaviour-preserving slice — verify before committing):** boilerplate Phase 0c (`Base.suggest`/SpellChecker — **LANDED**, retired the levenshtein copies in statesman/rails-routes/activerecord; a class method to dodge a self-check same-name collision), the example plugins `rigor-lisp-eval` / `rigor-pattern` (**LANDED** onto `node_rule`, deleting their hand-rolled `Walker`s — the SKILL-consistency gap is closed), 0d (`config_schema` `{kind:,default:}` defaults — needs a small ADR, ~17 plugins), 0e (`Plugin::Inflector` — **LANDED via [ADR-39](adr/39-plugin-target-library-invocation.md), Accepted:** real `ActiveSupport::Inflector` behind an allow-list+rescue harness, all 3 consumers migrated, byte-identical on Redmine+Mastodon; **remaining:** ADR-39 slice 3 (static `config/initializers/inflections.rb` ingestion for project-custom inflections) + ADR-39 slice 5 (the **selectable isolation strategy** — `Plugin::Isolation`: `none`/`ruby_box`/`process` via `RIGOR_PLUGIN_ISOLATION` / `.rigor.yml` `plugins_isolation:`, **all 3 landed, `process` the default** (forked once + reused, crash-contained; falls back to `none` without `fork`). Default validated byte-identical on Redmine + the whole spec suite, no segfault; `ruby_box` = gated experimental, blocked on an upstream `Ruby::Box` VM segfault. Remaining: exact-gem-version loading + route the Rack catalogue through `Isolation`)); `Source::Literals` symbol-only variants; the `dynamic_return` generalisation (escape-valve-consumer migration path); ADR-38 block-form; `NodeRuleTest` / `DynamicReturnTest` harnesses.
