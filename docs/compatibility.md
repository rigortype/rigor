...
| **`.rigor.yml` keys + value grammar** | **Two co-equal sources:** `Configuration::DEFAULTS` + the coercers in [`lib/rigor/configuration.rb`](../lib/rigor/configuration.rb) (which keys exist and what they mean) **and** [`schemas/rigor-config.schema.json`](../schemas/rigor-config.schema.json) (their declared shape, incl. namespaces reserved for a sibling implementation and never read here). Semantics: [`docs/internal-spec/config.md`](internal-spec/config.md) ([ADR-99](adr/99-config-schema-authority.md)) | [Manual ch. 3 — Configuration](manual/03-configuration.md) | **yes** — a documented key keeps its name, shape, and default semantics |
...
| **Cache schema version** | `Store::PAYLOAD_ABI_VERSION` (= `Rigor::VERSION`) + `Descriptor::SCHEMA_VERSION` + `Store::FORMAT_VERSION` in [`lib/rigor/cache/`](../lib/rigor/cache/) (marker `<version>.8.2`) | [Manual ch. 12 — Caching](manual/12-caching.md) | **yes** — a schema/format bump invalidates the cache, never silently mis-reads it |
...
| Persistent cache | `Cache::Store::PAYLOAD_ABI_VERSION`.`Cache::Descriptor::SCHEMA_VERSION`.`Cache::Store::FORMAT_VERSION` (the `schema_version.txt` marker) | `<Rigor::VERSION>.8.2` |
