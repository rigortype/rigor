...
PUNDIT_PLUGIN_LIB = File.expand_path("../../../plugins/rigor-pundit/lib", __dir__)
...
RSpec.describe "plugins/rigor-pundit" do
  ...
  # Issue #1056 — the "policy index did not load" notice is a RUN-scoped disclosure, not a per-file
  ...
  describe "the policy index load error is disclosed once per run (#1056)" do
    before { allow(Rigor::Plugin::Pundit::PolicyDiscoverer).to receive(:new).and_raise(RuntimeError, "boom") }
    ...
    it "emits one row at .rigor.yml:1:1, not one per analysed file" do
      ...
      expect(rows.first.message).to include("failed to discover policies")
    end
  end
end
