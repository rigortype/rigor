...
- **DX tooling track.** ~~`rigor explain <rule-id>`~~ (closed in v0.1.2 Track 2), ~~`rigor diff <baseline>`~~ (closed in v0.1.2 Track 2), ~~`# rigor:disable-file <rule>`~~ (closed in v0.1.2 Track 2), `.rigor.yml` JSON schema for editor autocomplete. Last item queues for v0.1.3+.
