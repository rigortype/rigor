...
- **Narrowing.** A condition whose value is a reader's binds `$_` on both edges to the reader's RBS return narrowed as that value is: `String` on the truthy edge, and `nil` on the falsey edge of `gets`. The condition is the call itself (`while $stdin.gets`, `if STDIN.gets`, `$stdin.gets or break`, `next unless $stdin.gets`, `!$stdin.gets`) or a variable written with it (`while (line = STDIN.gets)`); `until (line = STDIN.gets).nil?` and other readings of the value do not narrow `$_`. The body of `while $stdin.gets` reads `$_` as `String` on every pass unless a `redo` targets the loop (a `redo` runs the body again without testing the predicate), and after the loop `$_` is `nil` unless the body can `break`. The arms of a reader condition bind `$_` differently, so where they join — after `if $stdin.gets … end`, `$stdin.gets ? a : b` or `(1 if $stdin.gets)` — `$_` MUST be unbound, not `String?`; a `String?` there would report correct code that proves the line through a flag it set in the arm (`ok = $stdin.gets ? true : false; return unless ok; line = $_; line.chomp`). The reader MUST be certain:
  - `gets` or `readline` without a block on the `Kernel` module, or on `$stdin` or `$<` while the file binds them to nothing or to a single such receiver's type;
...
- **Subprocesses.** A backtick or `%x` command, `system` on implicit `self`, `self.` or `Kernel`, and `wait`, `waitpid`, `wait2` or `waitpid2` on the core `Process` module with at most one plain argument, the child's pid, wait for their child and leave `$?` a `Process::Status`; `system` does so also when the command cannot run (exit status 127). The scope past a statement that certainly ran one MUST bind `$?` to `Process::Status`: one that is the statement's call, sits in its receiver chain (`` head = `git rev-parse HEAD`.strip ``), or is a positional argument of a call that is not a safe-navigation call (`puts(%x(date))`), at any depth of those. One in a block, a branch or an `&&` operand may not run, so the paths that skip it join with `$?` unbound. `spawn` alone, `IO.popen`, `Open3`, `Process.detach` and a `Process.wait` given a flags argument do not bind it, and `Process.last_status` reads `$?` rather than setting it. A program that defines a method of a setter's name anywhere (`def system`, ``def `(cmd)``, a `wait`) may run Ruby code in its place, so that name binds nothing.
- **Clearing.** Ruby sets `$?` back to `nil` in two cases: a `wait`, `waitpid`, `wait2` or `waitpid2` with the `WNOHANG` flag that finds no exited child, and `Process.waitall` with no child. Either can run inside any method the thread calls or in a signal handler, so a file that holds a `wait`-family call with more than one argument, a splat, keywords or a forwarded `...`, on any receiver, or a `waitall`, or a `send` naming either, MUST bind `$?` nowhere. Elsewhere `$?` stays a `Process::Status` once set, so the binding survives later calls, blocks and loops until a join with a path that did not set it: `system("make"); log("built"); $?.success?` reads it. A called method's subprocess also sets its caller's `$?`, which the analysis does not follow, so `def run = system("make"); run; $?` leaves it unbound.
- **Gaps**: a `WNOHANG` wait or a `waitall` run between a subprocess and a read of `$?` from another file, a gem or a signal handler set up elsewhere, or through a computed `send`; a block a method runs on another thread (a thread pool's `post { $?.success? }`) or keeps and runs after the rescue clause exited (`on_error { $! }`), which reads the bindings where it is written; a gem's Ruby `system` reached through implicit `self`, including a DSL block's rebound `self`; a gem's exception class that redefines `backtrace`, and a class below `Exception` that redefines `self.===`; and the `English` aliases `$ERROR_INFO`, `$ERROR_POSITION` and `$CHILD_STATUS`, which are not read as `$!`, `$@` and `$?`.
