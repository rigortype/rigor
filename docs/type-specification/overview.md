...
Where an inline annotation and a standalone `.rbs` declare the **same member** of the same class, Steep 2.x and rbs 4.x define no precedence — both sources are merged into one class entry and the duplicate is reported as a signature error, leaving the class without a method surface — so the SHOULD above resolves to nothing and Rigor decides. The `.rbs` declaration wins **per member**: the inline declaration of that member is dropped, every other annotation in the file still applies, and the class still builds. The drop MUST be reported (see [ADR-32](../adr/32-rbs-inline-comment-ingestion.md) WD12: an annotation Rigor parses and does not honour is never swallowed).

> **Accepted, not yet implemented — [ADR-112](../adr/112-extrbs-comment-channel.md) WD5 ([#1075](https://github.com/rigortype/rigor/issues/1075)).** The per-member precedence above is replaced by a consistency rule. Consistent declarations merge to the more precise side, and a contradiction is an error. This paragraph is rewritten when that lands.
