...
Governance: [ADR-31](adr/31-contribution-and-supply-chain-policy.md) is the project-wide contribution and supply-chain policy. It applies to **all shipped code in the `rigortype` gem** (engine `lib/rigor/`, `plugins/`, `examples/`, `ext/`, `exe/`, `sig/`, gemspec) — no external pull requests are accepted into any of these paths; the supply-chain rationale is that `rigortype` runs in every analysed user's CI / dev environment so the blast radius is the same regardless of whether a malicious commit lands in the engine or a plugin. Out-of-scope paths (docs, specs, fixtures, tooling) follow conventional OSS PR review. Plugin-specific worked paths recorded under WD2–WD5: (1) **third-party `rigor-<gem>` gem** in the author's own repo depending on `gem "rigortype"` (ADR-31 WD4 — Larger Work under [MPL §3.3](../LICENSE), fully supported, default expectation); (2) **promotion-for-bundling via issue** with `Co-authored-by:` attribution when the wrapped gem reaches community-recognition (ADR-31 WD2, criterion intentionally vague per WD3); core engine proposals follow the same WD2 issue-driven shape, just without the WD3 adoption-evidence requirement. Subtree merge of a proven third-party plugin is reserved as an optional path (ADR-31 WD5) — not a path third-party authors should plan around.
