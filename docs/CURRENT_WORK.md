...
- **Include-aware dispatch for module catalogs.** Comparable and Enumerable module catalogs landed in v0.0.5 but their data is dormant — `MethodDispatcher::ConstantFolding::CATALOG_BY_CLASS` only routes by concrete receiver class. A future slice walks the receiver's ancestor chain (`receiver_class.ancestors` mapped through Rigor's class hierarchy) and consults `COMPARABLE_CATALOG` / `ENUMERABLE_CATALOG` for inherited methods. Should also generalise to user classes that `include Comparable` so e.g. a `Coordinate` with `<=>` resolves Comparable's catalog-classified methods.
