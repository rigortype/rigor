...
The unit of persistence is the **per-file `FileCollection`** — that is what per-file reuse is sound over, and it is all the ADR-46 incremental snapshot may carry: a recheck re-collects a partial closure, which a stored table cannot serve. The **whole-run slot additionally stores the propagated table beside the collections** (`[collections, table]`), because its invalidation already is what a stored closure needs — every analyzed file through the post-run dependency descriptor, every meaning input through the effects identity — so a warm hit adopts both and re-runs neither the merge nor the fixpoint. A pre-table entry (a bare collections Hash) fails the reader's shape guard and re-collects once; an empty table stored over non-empty summaries would freeze one run's fail-soft propagation failure into every warm hit, so the writer stores `nil` in that corner and the hit re-runs the fixpoint instead.
...
A run with collection **on** is no longer excluded from the whole-run result cache. It consults the effects slot first, because that is the slot that can force work: a hit adopts the collections and the stored table (re-running the fixpoint only when the entry carries none), a miss re-analyzes (collection is observational, so the only way to collect is to analyze) and writes the slot. Either way the diagnostics slot decides for itself — an effects miss whose diagnostics entry is warm serves the warm diagnostics, and the effects entry's validation uses the same post-run dependency descriptor, so exactly the file set that invalidates diagnostics invalidates summaries.
