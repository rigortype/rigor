...
| ADR-48 | [Struct / Data value folding (member-shape carriers)](48-data-struct-value-folding.md) | Proposed — `Data.define` first. Two new carriers (a member-class `Type::DataClass` + a member-instance `Type::DataInstance`, HashShape-shaped but nominal/class-tagged) so `Point = Data.define(:x, :y); Point.new(1, 2).x` folds to `Constant[1]`. Reuses the existing `ScopeIndexer` `data_define_call?` / `meta_member_names` recognition (today only registers member *existence*). Precision-additive only — no diagnostic, no FP surface, degrades to today's `Dynamic[top]` on any uncertainty (block body / non-literal members / arity mismatch). `Struct` deferred behind its mutation-soundness story (setters/`[]=` invalidate the instance member map; `Data`'s frozen instances are why it is first). Four slices: DataClass → DataInstance + member-read folding → the `class Point < Data.define(...)` subclass idiom → block-body degradation hardening. Grounded in the Phase-5 `struct-encoding-coverage` audit |
