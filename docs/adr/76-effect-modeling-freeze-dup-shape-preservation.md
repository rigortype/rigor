...
Status: **Accepted — WD1 implemented 2026-06-24 (`2751bc78`); WD2 fully
landed 2026-06-26 (HashShape + Tuple preservation, once the ADR-78 WD1
block-form over-fold guard removed the residual reflexive firings).**
Records a split decision on effect / mutation modeling. The
...
real fold gap — **fully landed 2026-06-26** for both `HashShape` and
`Tuple` (`shape_self` in `ShapeDispatch`'s handler tables). The Tuple half
required first fixing the residual over-fold it exposed — a block-form
fold (`CONST = [...].freeze; CONST.any? { … }` folding the no-block result,
ignoring the block), root-fixed by [ADR-78](78-reflexive-overfold-always-truthy.md)
WD1's block guard in `ShapeDispatch.try_dispatch`. Diagnostic-identical to
master on `lib` + the mail / kramdown corpora. This ADR fixed the scope
and the ordering so the precision fix was not re-attempted cold.
