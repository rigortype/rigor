...
- If an annotation conflicts with the RBS signature, Rigor MUST report a diagnostic. A `rigor:v1:return:` or `rigor:v1:param:` refinement exceeds the ordinary RBS contract when the type its own member declares at that position provably does not accept it (`%a{rigor:v1:return: positive-int}` on `def label: () -> String`). Rigor reports it as `rbs.contradicting-signature`, an error, from the comparison that checks a `sig/` declaration against an inline one ([overview.md](overview.md), [ADR-112](../adr/112-extrbs-comment-channel.md) WD5). A declared `untyped`, `void` or `top` accepts every refinement, and so does a declared type Rigor cannot compare faithfully (a type alias, an interface, a type variable), since nothing about it is proven.
