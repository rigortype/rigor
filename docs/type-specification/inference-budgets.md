...
|---|---|---|---|
| Recursion re-entry | effective depth 1 → `Dynamic[top]`; value-pinned args unroll up to fuel 32 → folded constant ([ADR-55](../adr/55-recursive-return-precision.md) slice 1) | `ExpressionTyper#infer_user_method_return` (`BudgetTrace::RECURSION_GUARD`, `BudgetTrace::RECURSION_UNROLL_FUEL`) | No — a `(receiver, method)` re-entry guard; when every argument is a `Constant` / `Tuple`-of-`Constant`, the key extends with the argument *values* so distinct constant frames recurse under a hard 32-frame fuel + 64-node value-size cap, exhausting back to the plain guard (today's `Dynamic[top]`). Distinct from the configurable `recursion_depth` precision-unroll the table envisions |
