...
- A `gets` / `readline` condition, which binds `$_` on its edges (see [Last-line (`$_`) narrowing](#last-line-_-narrowing)).
...
- **Narrowing.** A condition whose value is a reader's binds `$_` on both edges to the reader's RBS return narrowed as that value is: `String` on the truthy edge, and `nil` on the falsey edge of `gets`. The condition is the call itself (`while gets`, `if gets`, `gets or break`, `next unless gets`, `!gets`) or a variable written with it (`while (line = gets)`); `until (line = gets).nil?` and other readings of the value do not narrow `$_`. The body of `while gets` reads `$_` as `String` on every pass unless a `redo` targets the loop (a `redo` runs the body again without testing the predicate), and after the loop `$_` is `nil` unless the body can `break`. The arms of a reader condition bind `$_` differently, so where they join — after `if gets … end`, `gets ? a : b` or `(1 if gets)` — `$_` MUST be unbound, not `String?`; a `String?` there would report correct code that proves the line through a flag it set in the arm (`ok = gets ? true : false; return unless ok; line = $_; line.chomp`). The reader MUST be certain:
  - `gets` or `readline` without a block, on implicit `self` or `self.` in the top-level script body, outside every block, whose `self` is `main`, while the program mixes no module into `main` or `Object`. Such a mixin is an `include`, `extend`, `prepend` or `using` (or a `send` that may name one) on implicit `self` or `self.` outside a class or module body, one on `Object`, `Kernel` or `BasicObject` anywhere in the file, or a recorded `include`, `prepend` or `extend` of one of those classes: after `include Readline`, `readline` is Reline's Ruby method. A top-level method does not count, since it is a private method of every object and its `self` may be one whose `gets` is Ruby's (`class Importer < DelegateClass(File); def import = helper; end`), and neither does a block, whose `self` its method may rebind (`instance_exec`). Inside a class or module an implicit-self reader counts only where RBS places the method in `IO` or one of the classes below (a reopened `IO`): the class's ancestry may hold a Ruby reader the analyzer does not see — a class written `class W < DelegateClass(File)` records no superclass, and RBS answers `Kernel` for readers it leaves out, such as `CSV#gets` (an alias of its Ruby `shift`) or `OpenSSL::Buffering#gets`;
  - on the `Kernel` module, or on `$stdin` or `$<` while the file binds them to nothing but such a receiver;
  - or on a receiver whose inferred type is a class whose reader RBS places in `IO`, `StringIO`, `ARGF` or `Zlib::GzipReader`. `STDIN` and `ARGF` count this way, by their types, so a project constant that shadows either (`module App; ARGF = CSV.new($stdin)`) is read as what it holds. `Tempfile` is not one: RBS declares it below `File`, but its `gets` is a Ruby forwarder (`DelegateClass(File)`), which sets the forwarder's `$_`. `Kernel`'s reader is private, so an explicit receiver that only inherits it raises instead of reading.

  A program that defines or patches in a method named `gets` or `readline` anywhere gets no narrowing from that name, because the analyzer does not follow which of the two a call reaches. A safe-navigation reader (`io&.gets`) narrows its truthy edge only: when `io` is `nil` it reads nothing.
- **Other readers.** A reader that is not a condition leaves `$_` unbound rather than binding it to `String?`: nothing proves the line non-nil there, and a nilable `$_` would report correct code that checks the reader's value another way before it reads `$_` (`line = gets; return unless line; copy = $_; copy.chomp`). A reader on a receiver the rule above does not name, a `Dynamic` one included, leaves `$_` unbound the same way, since it may be a method defined in Ruby, which leaves the caller's `$_` alone.
