...
- [ADR-51](docs/adr/51-ci-diagnostic-output-formats.md) — CI-native diagnostic output formats (accepted — partially implemented v0.1.18; `rigor check --format sarif|github|gitlab|checkstyle|junit`: SARIF 2.1.0 cross-platform anchor (+ reviewdog `-f=sarif`) + GitHub Actions workflow commands (inline PR annotations, no upload step) + GitLab Code Quality JSON (MR widget) + Checkstyle XML (reviewdog `-f=checkstyle` → *any* reviewdog reporter — GitHub PR review/GitLab MR/Gerrit/Bitbucket/Gitea — + Jenkins) + JUnit XML (test-report CIs), each a pure presentation layer over the same `Analysis::Diagnostic` fields `--format json` exposes — no new analysis; criterion = *a CI platform renders the stream inline from it*; comparator PHPStan ships github/gitlab/checkstyle/junit/teamcity but **no SARIF**; WD2 fixes the severity/identifier contract table (error/warning/info → SARIF error/warning/note · GitHub error/warning/notice · GitLab major/minor/info · Checkstyle error/warning/info · JUnit failure-type; rule → `ruleId`/`title=`/`check_name`/Checkstyle `source`/JUnit `classname`), WD3 the stable GitLab fingerprint (SHA256 of `(path, rule, line, column, message)`, not index-based), WD4 leaves the exit code unchanged, WD5 maps every diagnostic to a JUnit `failure` (surfacing not gating, severity in `type`); new output format is public contract under ADR-50 WD1; WD6 ships ADR-27 § WD3's queued copy-paste CI setup templates (+ a reviewdog `action-setup` variant) and the bundled `rigor-ci-setup` skill; `--output FILE` / reviewdog `rdjson` / TeamCity / rich SARIF rule metadata deferred)
