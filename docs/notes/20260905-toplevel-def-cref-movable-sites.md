...
**Mastodon's 66 are all Rails migrations.** The nesting owners are `FixAccountsUniqueIndex`, `RemoveFauxRemoteAccountDuplicates`, `AddSilencedAtSuspendedAtToAccounts` and friends — migration classes that declare a local shadow model (`class Account < ApplicationRecord`) inside the migration body. `Account` (20 reads), `User` (10), `Status` (7) and `IpBlock` (4) are the head of that distribution. This is the canonical shape of the defect rather than an artifact: a top-level helper called from a migration, whose `Account` Ruby resolves to `::Account` and Rigor to the migration's shadow.
