...
| ADR-43 | [RBS-complete ancestor resolution (allow-list inherited-method dispatch)](43-rbs-complete-ancestor-resolution.md) | Proposed (measurement-gated — lets `rigor check` resolve a Ruby subclass's inherited calls against an allow-listed RBS-complete ancestor (seed `Plugin::Base`) so contract misuse warns standalone, without Steep's own-helper FP wall; blanket resolution rejected on Rails-controller FP grounds; inject at `rbs_dispatch.rb` `lookup_method`) |
