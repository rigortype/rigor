...
| ADR-45 | [Unchanged-project fast path (run-result cache)](45-unchanged-project-fast-path.md) | Proposed — a pre-analysis whole-run diagnostic fingerprint is **rejected as unsound** (plugins like Pundit read project files *during* analysis, so a fingerprint built before the analysis misses them and serves a stale result — caught by the cross-process `pundit_plugin_spec` regression test); the sound design is record-the-dependencies-then-validate-on-next-run (stable key + re-digest recorded reads), staged behind a small `Cache::Store` addition; gate must go `--no-cache` when it lands |
