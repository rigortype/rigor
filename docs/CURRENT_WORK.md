...
**v0.1.1 in flight.** Four parallel tracks scoped; **Track 1 slice 1 (regex pattern → refinement-name recogniser) and Track 4 sig drifts (`Trinary#negate`, `IntegerRange#lower`, `IntegerRange#upper`) landed unreleased on the work branch.** Working state: 2090 RSpec examples / 0 failures, RuboCop 210 files / 0 offenses, `bundle exec exe/rigor check lib` reports `No diagnostics`. Remaining: Track 1 slices 2 / 3 / 4 / 5 (`numeric-string` propagation through `Integer(s)`, `self`-narrowing in `predicate-if-*`, `String#start_with?` / `#end_with?` / `#include?` predicate narrowing, additional `literal-string` propagation methods); Track 2 (cross-plugin API per [ADR-9](adr/9-cross-plugin-api.md) + return-type contributions); Track 3 remaining items (demo cache directory handling, examples RuboCop relaxation; helper extraction landed in commit `ce64bb6`); Track 4 items 12 (`node_locator_spec.rb:82` cleanup) and 13 (`numeric.yml` `Integer#ceildiv`). Full slice list in [`docs/MILESTONES.md`](MILESTONES.md) § "v0.1.1 — Planned".
...
- **Track 4 maintenance** items (`node_locator_spec.rb:82`, `Integer#ceildiv`) are pick-up-anytime cleanups. The three sig drifts are already closed.
