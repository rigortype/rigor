...
- The ADR-2 plugin-contract surface stabilised enough to support
  ...
  plugin-contract namespaces (and the wider public surface) — is now
  **drafted** in [`docs/compatibility.md`](compatibility.md) (the ADR-50
  WD1 surface document, authored in the v0.1.19 preview window); it
  binds as a trial at v0.2.0 and freezes at v1.0.0.
...
### Performance / scalability — compiled plugin contribution dispatch (ADR-52, accepted — slices 1+2+3-partial+4-partial landed; next = rigor-sorbet)

**[ADR-52](adr/52-compiled-plugin-contribution-dispatch.md)** — the structural successor to ADR-44's spot fixes on the plugin consumption path, grounded in the [2026-06-10 structural audit](notes/20260610-plugin-architecture-perf-audit.md). Criterion: every per-call / per-def / per-file / per-node plugin consultation is gated by a key the engine already holds, via a table compiled once per run at registry build; plugin code runs only on candidate hits. Slices: (1) compiled contribution table + engine call-site rewiring (no contract change) — **LANDED** (`67a552de` + `1deecb2f`; corpus byte-identical, wall-time-neutral until the legacy hooks migrate); (2) static method-name-only `dynamic_return` (receiver-less) → migrated `rigor-units` — **LANDED** (`c3550b00` + `cd5d5990`); audit correction: no production plugin fit the static gate (activesupport-core-ext ships no hook, sorbet's catalog is a run-time name set); (3) run-time **receiver-set** callable (lazy `instance_exec` memo, resolved in the instance dispatch path, set is a safe over-approximation of the block filter) → migrated `rigor-activestorage` (GitLab byte-identical) — **engine + activestorage LANDED** (`fb5aea04`/`0f1a64b2` + `be4c532c`); **rigor-activerecord is BLOCKED** (its class-side dispatch reads the AST constant / `self_type`, not the receiver type, and project model constants type as `Dynamic` — a migration attempt was reverted; needs an AST-keyed gate form or engine Singleton-typing of discovered classes, see the ADR's "rigor-activerecord blocker"); (4) run-time **method-name** set (touches the ContributionIndex name gates) — **engine + the config-gated lisp-eval/pattern examples LANDED** (`79dc790d` + `46b14280`, end-to-end-spec/demo byte-identical); **rigor-sorbet next** (verified migratable 2026-06-11 — its catalog gates on method names not receiver type, so the AR `Dynamic`-constant blocker does NOT apply; needs a `Catalog#method_names` enumerator + the hook's side effects relocated into the block + a Sorbet-using corpus for the byte-identical gate, see CURRENT_WORK § "ADR-52"), then rigor-rspec (per-file); (5) **delete** `flow_contribution_for` (deliberate pre-1.0 BC break; ADR-2/37 status updates + plugin-author skill + CHANGELOG migration note); (6) single engine-owned node-rule walk per file (independent). Every slice gates on byte-identical diagnostics over the Mastodon (6-plugin) / GitLab (11-plugin) corpora + stackprof deltas + `make bench-perf`. Slice 5 must land **before the v1.0 contract freeze** (ADR-50) — that timing asymmetry is the reason this is one coherent restructuring rather than further demand-gated spot fixes.
