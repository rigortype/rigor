...
- the receiver must be a class the project defines — it has a method, a declared superclass, or a declared `include`. A gem class the project only calls `new` on is as undescribed as it was;
...
- **`extend` is invisible.** The collection records `include` / `prepend` and nothing else, so a singleton `new` supplied by `extend M` is not found by the `def self.new` step and the rule resolves to `#initialize` instead. The same blindness already bounds singleton `super` resolution (§ Propagation, `resolve_super`).
