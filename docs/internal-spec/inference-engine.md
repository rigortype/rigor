...
- The block body of a class-creating meta call — `Class.new`, `Module.new`, `Struct.new`, `Data.define`, each with a literal block and a receiver that is the bare constant (or its `::`-rooted spelling) — is a class body: Ruby evaluates it with `self` bound to the class the call creates. `self_type` MUST be `Singleton[<anonymous-name>]` and the class context MUST carry one non-singleton frame under the same name, so a `def` in the body binds an instance method on that class. Unlike a `class` keyword body the block is a closure, so the outer scope's locals MUST stay visible. The `<anonymous-name>` is the per-call-site synthetic name defined below. A call at `Const = ...` rvalue position is out of scope for this boundary: it is typed through the expression typer rather than reached here, and its block body's methods stay keyed by the constant.
...
`ScopeIndexer` MUST register the block body's methods, def nodes and (for `Class.new(Parent)`) superclass under that name, and `MethodDispatcher`'s `Class.new` lift MUST answer `Singleton[<anonymous-name>]` for the parentless block form. The parented form (`Class.new(Parent) do ... end`) MUST keep answering `Singleton[Parent]`. A `def` in an anonymous body MUST additionally remain in the `<toplevel>` def table (never clobbering a real top-level `def` of the same name): the body is a DSL block whose methods routinely reach some other object's `self`, and the leniency predates the name.
