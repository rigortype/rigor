...
# Issue #1415 — `TOPLEVEL_BINDING.receiver` is `main`, so its `extend` puts `RubyReader` in `main`'s singleton class,
# and no implicit-self reader in the file narrows `$_`. Run with `read` as the first argument and "a\nb\n" on standard
# input; each case cites what Ruby 4.0.5 answers.

TOPLEVEL_BINDING.receiver.extend(RubyReader)
