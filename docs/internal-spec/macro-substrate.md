...
- **Carrying the index (#1038).** `Analysis::ProjectScan` carries the compiled
  ...
  Four things refuse a reuse: a different root, a different set of claimed
  globs, a different set of glob-claiming plugins (each of those three rebuilds
  the index wholesale — the transform itself may have changed), and the path the
  editor's buffer is bound to, which is recompiled from the buffer's bytes on
  every publish and whose unit never enters a shared index. The snapshot is
