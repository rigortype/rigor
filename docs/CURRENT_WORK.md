...
`make verify` is clean. **`[Unreleased]` holds the now-substantially-complete plugin-mechanism interface-segregation + ergonomics effort** (ADR-37 / ADR-38 / ADR-39 — see § "Status"); the release gates are met and all three ADRs are Accepted. Branch D below is the residual (non-gating) ergonomics — its remaining list is short (Source::Literals adoption blocked on a symbol-variant prerequisite; 0d needs a small ADR; the rest is demand-driven). Branches A–C are the other queued tracks. **Next-session entry point: pick any Branch-D remaining item (they're independent), or move to Branch A (v0.2.0 gates 1+2) which is the bigger strategic lever.**
...
1. **Boilerplate 0a — `Source::Literals` adoption** (item 5 in ROADMAP). Zero plugin adopters today; ~24 files hand-roll `is_a?(Prism::SymbolNode)` extraction. **Blocked on a prerequisite:** add **symbol-only variants** first — plugin helpers return a `String` (`node.value`) but `Source::Literals.symbol_or_string` returns a `Symbol`, so a String-returning variant is needed before migrating to preserve semantics. That prerequisite is its own slice.
2. **Boilerplate 0d — `config_schema` `{kind:, default:}`** so `Base#config` merges declared defaults, retiring the `DEFAULT_*`-constant idiom (~17 plugins). **Needs a small ADR.**
