...
- **ADR-28 path-scoped protocol contract — ecosystem opportunities.** [ADR-28](adr/28-path-scoped-protocol-contracts.md) (2026-05-23) introduces the provide-and-check extension point. The mechanism is most beneficial when a framework demands a **uniform method signature** across all files in a conventional directory. Ecosystem audit of existing plugins, ordered by fit:
  - **`rigor-hanami`** (new plugin — **LANDED in v0.1.9 cycle**): `app/actions/**/*.rb`, `#handle(Hanami::Action::Request, Hanami::Action::Response) → void`. The canonical production-plugin ADR-28 consumer. Typed parameter provision into action bodies surfaces `request` / `response` misuse as core `call.undefined-method` diagnostics. Return-type check is intentionally skipped (`return_type_name: nil`) because `#handle` is void — the response is mutated in-place. Config override: `action_path:`.
  - **`rigor-actioncable` enhancement** (demand-driven): The `#receive(data)` method on `app/channels/**/*.rb` currently receives `data` as `Dynamic[Top]`. A contract with `method_name: :receive, param_types: [{index: 0, type_name: "Hash"}]` would type `data` as `Hash` inside every channel's receive body. The `#subscribed` / `#unsubscribed` callbacks have no params and void return — not a practical ADR-28 use case (no params to provide, no return to check).
  - **`rigor-activejob` / `rigor-sidekiq`** (not a good fit): `#perform` signatures vary per job/worker class — the positional-index model requires a uniform signature. Both plugins already cover call-site arity via their index-based analyzer; the ADR-28 contract's added value here would only be void return-type checking, which is low-priority in practice.
  - **`rigor-graphql` / `rigor-pundit`** (not applicable): graphql-ruby 2.x resolver methods use keyword arguments (not positional); the ADR-28 model maps positional indices only. Pundit policy method types (`user`, `record`) are project-specific and cannot be declared generically in the plugin manifest.
  - `ProtocolContract#return_type_name` made optional (nil = void, skip return check) as part of the `rigor-hanami` landing. `ProtocolChecker` (rigor-web) guards nil before attempting nominal lookup.
