...
(base `6e4929d3`, which carries the #392 template-unit seam), Ruby 4.0.5, stdlib `ERB` 6.0.1.1 as
the compiler (neither corpus copy had an installed bundle for Erubi to resolve from).
...
note is that measurement, and it is also how three of the shipped decisions were arrived at rather
than argued.
...
| Project | `.erb` files | units built | diagnostics before | after | **new** | removed |
| --- | ---: | ---: | ---: | ---: | ---: | ---: |
| redmine | 506 | 502 | 433 | 435 | **2** | 0 |
| mastodon | 46 | 44 | 1180 | 1180 | **0** | 0 |
...
`rigor effects` after the change: redmine reports 502 `view:` units (the four it does not are the
layouts — see § 3), 35 of which carry an
`io.db.*` row —
...
Three of the shipped decisions are consequences of a run, not of a design argument. Each is recorded
...
- **Erubi.** Neither corpus copy carries an installed bundle, so `Isolation.require_with_target_bundle`
  found no Erubi and stdlib `ERB` compiled both runs. The Erubi path is exercised by the compiler's
  own probe (which measures the line offset rather than assuming it, and demotes a compiler whose
  numbering it cannot confirm), not by this corpus run.
