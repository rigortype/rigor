...
RSpec.describe "RBS name Marshal hooks (rbs_environment_marshal_patch)" do
  ...
  # Issue #799 — `rbs_extended.unsatisfied-conformance` and `effect.unknown-label` are positioned at the
  # `%a{...}` the author wrote, so an annotation's location is the one position in a cached environment that
  # a diagnostic reads. Through `RBS::Location#_dump` it collapsed to `1:1`, which moved the row on a warm
