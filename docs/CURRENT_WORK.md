...
- **Slice 2b (walker + dispatcher tier)** queued — walker walks each `Index#resolved_gems` entry's `roots:`, indexes class definitions, infers method return types via the existing engine, and contributes them as `Dynamic[T]` through a new dispatcher tier slotted between `RbsDispatch` and `try_user_class_fallback`. ADR § "Hard exclusions" enforced in the loader.
