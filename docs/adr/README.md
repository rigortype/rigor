...
| ADR-107 | [Checked types and typeless comments in Rigor's own tree](107-checked-types-and-typeless-comments.md) | Accepted (corpus rewrite + contract + G1 gate in #822; G2 `--fail-on=warning` in #827; G3 #825 open) |
...
| ADR-109 | [Ruby range literals as the notation and the semantics of numeric range refinements](109-ruby-native-range-notation.md) | Accepted (slice 1: `Integer[1..10]` display + grammar, `int<a, b>` deprecated input alias; slices 2–3 `Float[R]` reserved, not built) |
