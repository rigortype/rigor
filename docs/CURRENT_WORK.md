...
> 0. **v0.1.18 — CI-environment support** (themed cut before v0.2.0; full surface in [`ROADMAP.md`](ROADMAP.md) § "v0.1.18 — CI-environment support"). **The CI diagnostic-output + templates + skill half LANDED** ([ADR-51](adr/51-ci-diagnostic-output-formats.md)): `rigor check --format sarif|github|gitlab|checkstyle|junit` (five CI-native renderings over the `--format json` fields, `lib/rigor/cli/diagnostic_formats.rb`; `checkstyle` is the reviewdog bridge → any reviewdog reporter) + the copy-paste CI setup templates under `docs/manual/ci-templates/` (incl. a reviewdog `action-setup` variant) + the bundled `skills/rigor-ci-setup/` skill + the `docs/manual/11-ci.md` write-up. Scoping calls resolved: new ADR (not an ADR-27 amendment), all five formats (SARIF the recommended default; checkstyle/junit added for reviewdog/Jenkins/test-report reach; comparator = PHPStan, no SARIF). **Remaining for the cut:** the ergonomics ADR-51 defers (`--output FILE`, reviewdog `rdjson`, TeamCity, richer SARIF rule metadata — all demand-gated), the ADR-27 § WD3 `setup-ruby` prebuilt-4.0 timing caveat, and whatever else the standing backlog contributes before tagging v0.1.18. **Do the rigor-release-prep skill for the actual cut.**
