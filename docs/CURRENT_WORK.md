...
**v0.1.3 in progress, release pending.** Theme: **deliver [ADR-10](adr/10-dependency-source-inference.md) end-to-end, absorb ADR-11 / Rails plugin Phase work, and land ADR-13.** Five tracks closed; **ADR-13 slices 1–7 all landed on `master`** — nothing ADR-13-shaped deferred:
...
3. **ADR-14 follow-up: kind tracking + `module_function` + nested-class emission.** The third-round `lib/rigor/analysis/` dogfood surfaced four interacting gaps the generator did not handle without hand-edits. Two are closed; three remain (one new gap surfaced during the kind-tracking work).
   - ~~**(a) module-vs-class detection on intermediate segments.**~~ **Closed by commit `b6d3286`.** `Generator#walk_defs` now records every walked `ClassNode` / `ModuleNode` into `@namespace_kinds[qualified_name] => :class | :module`, threads through `MethodCandidate#namespace_kinds`, and `Writer#wrap_in_modules` emits per-segment kinds with `module` as the safe default.
   ...
   - **(c) Nested Data-define classes inside another class — STILL OPEN.** `GemResolver` contains `class Resolved < Data.define(...)`. Sig-gen emits `Resolved`'s methods in a SEPARATE nested wrap (with `GemResolver` re-opened as a sibling block) rather than nesting `class Resolved` inside the `class GemResolver` block that holds GemResolver's own methods. The writer needs to recognise when a candidate's `class_name` is a strict prefix of another candidate's `class_name` and emit a single tree (outer class block containing the inner class block). Implementation sketch: in `Writer#render_new_file`, group candidates by SHARED parent class first, then nest. The current `group_by(&:class_name)` flattens this. Sized ~50 LoC + ~3 spec examples.
   - **(d) `&:freeze` block-pass not recognised by the dispatcher — STILL OPEN.** `Hash#transform_values(&:freeze)` dispatches against the no-block overload (returns `Enumerator[V, Hash[K, untyped]]`) instead of the block overload (returns `Hash[K, U]`). This is an Inference engine gap (`Rigor::Inference::MethodDispatcher` doesn't treat `&:symbol` as block-presence). Affects sig-gen via `ivar_write_collector.rb`'s `collect` body. Implementation sketch: in `MethodDispatcher::OverloadSelector#overload_matches?` (or wherever block-presence is checked), treat `Prism::BlockArgumentNode` whose expression is a `Prism::SymbolNode` (or a `Proc` carrier) as block-present. Out of sig-gen scope; broader analyzer fix.
   - **(e) `Const = Data.define(:fields)`-assigned classes — STILL OPEN.** Surfaced during the `b6d3286` dogfood: `GemResolver` contains `Unresolvable = Data.define(:gem_name, :reason)` (an assigned constant whose RHS is `Data.define`). Sig-gen doesn't recognise this as a class declaration, so `class Unresolvable` is missing from the generated sig and the union return type `Resolved | Unresolvable` fails to resolve in Steep. Hand-fixed for `gem_resolver.rbs` with a stub `class Unresolvable; end`. The fix: `walk_defs` (or a sibling walker) should record `ConstantWriteNode` whose value is a `CallNode` matching `Data.define(...)` (or `Struct.new(...)`) as a class declaration, and emit the corresponding empty class block.
