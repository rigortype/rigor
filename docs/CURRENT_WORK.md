...
3. **ADR-14 follow-up: kind tracking + `module_function` + nested-class emission.** The third-round `lib/rigor/analysis/` dogfood surfaced four interacting gaps the generator does not yet handle, each of which had to be worked around with a hand-edit when applying the proposed sigs: **(a) module-vs-class detection on intermediate segments.** `walk_defs` records the joined qualified path (e.g. `Rigor::Analysis::DependencySourceInference::GemResolver`) but not whether each segment was originally declared `module` or `class`. The writer's nested-syntax emission defaults to `module` for every intermediate segment, which fails when the source has `module Foo; module Bar; class Foo::Bar::Baz; end`-shaped layouts. The fix needs `collect_method_definitions` to also record `kind_for_segment` info per declared name, and `wrap_in_modules` to consult it. **(b) Leaf-kind detection for `module Foo; module_function`.** Sig-gen always emits the leaf as `class`, but in Rigor `Analysis::DependencySourceInference::GemResolver` is a `module` with `module_function`. The correct RBS spelling is `module GemResolver; def self?.resolve: ...` (matching the dual instance + singleton dispatch). Hand-fixed for now. **(c) Nested Data-define classes inside another class.** `GemResolver` contains `class Resolved < Data.define(...)` AND `Unresolvable = Data.define(...)`. Sig-gen emitted `Resolved` as a separate top-level module-wrapping (sibling to GemResolver) rather than nesting it inside `class GemResolver`. The writer needs to recognise when a candidate's `class_name` is a prefix of another candidate's `class_name` and nest accordingly. **(d) `&:freeze` block-pass not recognised by the dispatcher.** `Hash#transform_values(&:freeze)` dispatches against the no-block overload (returns `Enumerator[V, Hash[K, untyped]]`) instead of the block overload (returns `Hash[K, U]`). Skipped `ivar_write_collector.rb` because the generated `Enumerator` return type didn't round-trip through `rigor check`. Each gap is a focused fix; together they'd let the analysis-dir dogfood land without hand-edits.
