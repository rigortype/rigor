...
- [ADR-47](docs/adr/47-narrowing-driven-clause-reachability.md) — Narrowing-driven clause reachability (accepted — WD1 + WD2 implemented v0.1.17; Elixir-v1.20-inspired `flow.unreachable-clause`: extends the existing `if`/`unless` reachability rules to `case`/`when`, reusing the `falsey_scope` the flow engine already threads through `eval_case_when_branches` — a clause is dead when its `body_scope` narrows the subject to `bot` (per-clause disjointness) or the entry `falsey_scope` already carries a `bot` subject (prior-exhaustion); inherits `always-truthy-condition`'s FP envelope, never fires on `Dynamic[T]`; `in`/pattern clauses gated behind `InNode` exhaustiveness; corpus FP gate before default-on)
