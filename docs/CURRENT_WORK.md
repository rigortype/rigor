...
### ADR-52 — compiled plugin contribution dispatch (slices 1+2 LANDED; next = slice 3)
...
**AUDIT CORRECTION discovered while implementing slice 2** (recorded in the ADR + audit note): the audit's slice-2 targets were both wrong — **activesupport-core-ext ships no `flow_contribution_for`** (pure RBS bundle, not a target) and **sorbet's catalog path matches arbitrary `def` method names** (a *run-time* name set, not static). No production plugin fits the static gate; units was its first consumer. **Remaining `flow_contribution_for` users = production: activerecord, activestorage, sorbet, rspec; examples: lisp-eval, pattern** — all need a run-time set.

**Next-session entry: slice 3** — the run-time-set callable forms: `dynamic_return receivers: -> {...}` / `methods: -> {...}` evaluated once after `#prepare` and frozen into the table. **Design judgment still open** (flagged in the ADR): the ContributionIndex is built at `Registry.new` (before `#prepare`), so a run-time set needs the table built/finalised *after* prepare — decide between lazy-build-on-first-dispatch vs a runner-driven `finalize!` step. Then migrate AR + activestorage (receiver sets), sorbet (catalog method-name set), rspec (per-file), the config-gated examples; slice 5 deletes the hook (pre-1.0 BC break, before the ADR-50 v1.0 freeze); slice 6 (single node-rule walk) is independent. Gate per slice: corpus byte-identical (cwd=target, project `.rigor.yml`) + stackprof deltas in a notes entry; examples gate on their integration spec + a before/after demo diff.
