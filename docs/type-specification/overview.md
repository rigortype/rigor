...
- They **contradict** only when the contradiction is **proven** — no value and no call satisfies both sides: some position a call must fill holds two **proven-disjoint** types, the two accept positional-argument counts that cannot meet while neither takes keywords, or one requires a keyword the other cannot take in any form (it has no `**rest`, no `*rest`, and no positional parameter a keyword Hash could fill). A position a call may leave empty — an optional or rest parameter, an optional keyword, every position of an optional block — never contradicts, since the call that omits it satisfies both sides. Two types are proven disjoint only from the **RBS class hierarchy** the analysis uses, never from the Ruby constants the analyzer process happens to have loaded: two distinct literal values, a literal whose class RBS places nowhere below the other side's class, or two classes RBS declares (as classes, not modules) neither of which is an RBS ancestor of the other. A name RBS does not declare as a class — including a stub Rigor synthesized for a referenced but undeclared name — proves nothing, and neither does a relative name the project's Ruby source defines. A module or an interface on either side never proves disjointness, since any class may include it; nor do two element types of one generic class (an empty `Array` is both an `Array[Integer]` and an `Array[String]`); nor does a subset, which is a refinement (`TrueClass` of `bool`). A block's parameter count never contradicts, since a block takes what its caller passes. The `.rbs` declaration binds, so the class still builds, and Rigor MUST report `rbs.contradicting-signature` as an error ([diagnostic-policy.md](diagnostic-policy.md)). A stale generated signature is the usual cause.
