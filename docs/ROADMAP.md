...
- **Indexed-collection narrowing through `Hash[k] ||= default`.** A common Ruby idiom is `params[:f] ||= []; params[:f] << item`, where the `||=` guarantees the subsequent indexed read is non-nil. Today Rigor types each `Hash#[]` independently and the `<<` / `[]=` call fires `call.undefined-method` for `nil`. Redmine 6.1.2's `app/models/query.rb#as_params` is the canonical site (6 errors against this single shape across `params[:f]` / `params[:op]` / `params[:v]`). Cleanest path: extend the existing local-var `||=` narrowing to recognise the `receiver[key] ||= default; receiver[key].mutate` pair when both `receiver` and `key` are stable, contributing a non-nil flow-fact on the second indexed read. Soundness gap: a `Hash#[]=` between the two reads could re-introduce nil; narrowing must invalidate on any intervening `[]=` to the same key. Demand-driven; surfaces in any project that uses `params[:foo] ||= []` patterns. Surveyed (2026-05-28) on Redmine 6.1.2.
- **Method-call receiver narrowing across stable receivers.** The pattern `if x.last.is_a?(Array); x.last << item; end` re-evaluates `x.last` in the body and loses the narrowing the type-test established on the predicate's receiver. Narrowing today only works when the receiver is a local-variable read. Redmine 6.1.2's `lib/redmine/views/builders/structure.rb#method_missing` is the worked site (4 errors of "`<<` for `{}`" against this exact shape). The fix would extend `Inference::Narrowing` to record narrowing keyed on a structural fingerprint of the method-call chain (receiver + method-name path, no args, no block) and apply it to identical chains in the dominated body, invalidating on any intervening method call against the same receiver. Soundness gap: a second call could in principle return a different value; rule is purely heuristic. Demand-driven, gated on a soundness story. Surveyed (2026-05-28) on Redmine 6.1.2.
- **Tuple / HashShape widening for ivar-seeded literals after mutation.** The v0.1.x flow-folding fix widens a *local* variable seeded as `Tuple` / `HashShape` to its `Nominal[Array]` / `Nominal[Hash]` parent when a mutator runs against it. The same widening should apply to ivars: `@struct = [{}]; @struct << ...` seeds `@struct` as `Tuple[Constant[{}]]` via the class-ivar pre-pass, then `<<` / `pop` / `push` reads still see the original Tuple even though the runtime value has changed shape. Worked site: Redmine 6.1.2's `Redmine::Views::Builders::Structure` (where `@struct.last` folds to `Constant[{}]` even though the body alternates `<< []` and `<< {}`). Likely a small extension to `Inference::MutationWidening` to consult ivar reads against the pre-pass-seeded type and widen the class-wide accumulator entry on first observed mutation. Demand-driven. Surveyed (2026-05-28) on Redmine 6.1.2.
- **Defensive ivar-init with nil / false rvalue.** Follow-up to the polarity-aware defensive-init guard fix (tdiary-core `Configuration#configure_attrs` cluster). When the guarded rvalue is itself a falsey constant (`nil`, `false`), `union(rvalue, Constant[nil])` collapses back to a single `Constant[nil]` and the predicate still folds — so `@x = nil unless @x` (a no-op-but-documented-default idiom) keeps the `flow.always-truthy-condition` warning. Cleanest fix: when guarded AND rvalue is a falsey Constant, widen further (skip the seed entirely, matching `||=`'s no-seed behaviour, or seed `Dynamic[Top]`). Low priority — 1 remaining FP in the 24-warning tdiary-core cluster the main fix already silenced. Surveyed (2026-05-28) on tdiary-core HEAD `ee40c2b`.
