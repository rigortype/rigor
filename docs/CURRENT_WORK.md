...
- **Slice 2b-ii (dispatcher tier)** queued — new tier in `Inference::MethodDispatcher` that consults `Index#contribution_for` AFTER `RbsDispatch.try_dispatch` and BEFORE `try_user_class_fallback`. On a hit, return `Type::Dynamic.new(top)` so the call site stops emitting `call.undefined-method` and the value carries the dynamic-origin marker per ADR-10 § "Inference contract". Threading: add `dependency_source_index:` keyword to `Environment.for_project` (and the Environment instance accessor) so the dispatcher reads it via `scope.environment.dependency_source_index`. Drift snapshot updates in the same commit.
- **Slices 3–5** queued (cache descriptor + per-gem-version invalidation; per-gem budget pool + `dynamic.dependency-source.budget-exceeded`; documentation update).
...
Read [ADR-10](adr/10-dependency-source-inference.md) § "Implementation slicing" for the binding slice list. With Slices 1 and 2a landed, recommended entry order for the next session:

- **Slice 2b** — walker + dispatcher tier. Implement `DependencySourceInference::Walker` that visits each `Index#resolved_gems` entry's `roots:`, parses the gem's `.rb` files, indexes class definitions, runs the existing `Inference::ExpressionTyper` machinery against each method body, and populates `Index#contribution_for(class_name:, method_name:)` with the inferred return type. Wire a new tier into [`lib/rigor/inference/method_dispatcher.rb`](../lib/rigor/inference/method_dispatcher.rb) between `RbsDispatch.try_dispatch` and `try_user_class_fallback`; the tier consults `scope.environment.dependency_source_index.contribution_for(...)` (or threads the index through Services) and wraps any non-nil answer in `Type::Dynamic`. Hard exclusions (`spec/` / `test/` / `bin/`, C extensions, plugin-owned receivers) per the ADR's "Hard exclusions" section live in the walker, not the parser. Plugin veto is the open question to revisit.
