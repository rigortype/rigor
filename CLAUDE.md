...
- [ADR-44](docs/adr/44-dispatch-allocation-churn.md) — Per-dispatch / per-narrow allocation churn (accepted — incremental slice landed; mutable pooled Scope/CallContext rejected on re-entrancy/FP grounds; `ProjectScope` field-regrouping sanctioned but staged)
