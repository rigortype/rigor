...
RSpec.describe "plugins/rigor-activejob" do
  ...
  # Issue #1056 — the "job index did not load" notice is a RUN-scoped disclosure, not a per-file
  ...
  describe "the job index load error is disclosed once per run (#1056)" do
    before { allow(Rigor::Plugin::Activejob::JobDiscoverer).to receive(:new).and_raise(RuntimeError, "boom") }
    ...
    it "emits one row at .rigor.yml:1:1, not one per analysed file" do
      ...
      expect(rows.first.message).to include("failed to discover jobs")
    end
  end
end
