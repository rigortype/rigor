...
# Issue #1415 — `Kernel.prepend` puts `RubyReader` ahead of `Kernel`'s own reader, so no implicit-self reader in the
# file narrows `$_`. Run with `read` as the first argument and "a\nb\n" on standard input; each case cites what Ruby
# 4.0.5 answers.

Kernel.prepend(RubyReader)
