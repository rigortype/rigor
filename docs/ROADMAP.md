...
- **`skills/rigor-baseline-reduce/`** ([ADR-22 § "WD8"](adr/22-baseline-and-project-onboarding.md), [§ "SKILL: rigor-baseline-reduce"](adr/22-baseline-and-project-onboarding.md)). Ongoing-quality workflow: walk `.rigor-baseline.yml` rule-by-rule in priority order (smallest count first → likely real bugs or contained patterns) → sample 3-5 sites per rule → classify each (real bug / stylistic-safe / FP) → apply fix / `# rigor:disable` / open a GitHub issue against rigor → refresh baseline. Audience: same as project-init; the natural next-session SKILL once a project has a baseline.

- **`skills/rigor-plugin-author/`** (external-author variant — distinct from the `.claude/skills/rigor-plugin-author/` contributor SKILL). The plugin-authoring workflow re-targeted for users writing a standalone `rigor-foo` gem in their own repo (depending on `gem "rigortype"`) or maintaining a project-private plugin under their own `lib/`. Covers (a) gemspec authoring against the published `rigortype` API surface, (b) test framework choice (RSpec / Minitest) without rigor's `plugin_helpers.rb`, (c) project-private plugin pattern (no gemspec, loaded via `Dir["lib/**/*.rb"]` or similar), (d) version pinning + breaking-change strategy, (e) `rbs_collection` integration for project-side types.
