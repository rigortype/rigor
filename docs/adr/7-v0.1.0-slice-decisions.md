...
Built on top of [ADR-2 (Extension API)](2-extension-api.md);
...
[`docs/CURRENT_WORK.md`](../CURRENT_WORK.md) and
[`docs/ROADMAP.md`](../ROADMAP.md); this ADR pins the
...
> **Partially superseded by [ADR-54](54-cache-slimming.md) (2026-06-10).**
> The definitions caching this slice deferred eventually shipped as the
> `rbs.instance_definitions` / `rbs.singleton_definitions` single-blob
> producers — and ADR-54 WD1 then *retired* them: given the cached env,
> `Marshal.load`-ing every definition measured slower than rebuilding on
> demand. The single-blob-over-per-class-disk-entry finding below stands;
> the blob tier itself does not.
...
- [ADR-2 (Extension API)](2-extension-api.md) — design surface
...
- [`docs/internal-spec/plugin-trust.md`](../internal-spec/plugin-trust.md)
  — slice-2 trust / I/O policy spec (already landed).
- [`docs/CURRENT_WORK.md`](../CURRENT_WORK.md) — in-flight
