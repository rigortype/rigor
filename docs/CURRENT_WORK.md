...
### ADR-52 — compiled plugin contribution dispatch (slice 1 LANDED; next = slice 2)

[ADR-52](adr/52-compiled-plugin-contribution-dispatch.md) + the grounding [structural audit note](notes/20260610-plugin-architecture-perf-audit.md) (2026-06-10). **Slice 1 (WD1) LANDED** (`67a552de` + `1deecb2f`): the compiled contribution table (per-plugin + global method-name gates, verb-keyed `block_as_methods`, `owns_receivers` union + per-run ancestry memo, `for_file_diagnostics` subset, frozen registry aggregates + per-path contract memo) and the six engine call-site rewirings. WD6-gated: **byte-identical diagnostics** on Mastodon `app/models` (6 plugins) + GitLab configured subset (11 plugins, 2,332 output lines) via control-vs-HEAD worktree comparison. GOTCHA the gate caught: the `&:symbol` block path dispatches with the `Prism::BlockArgumentNode` as `call_node` — a bare `call_node.name` raised, the raise was absorbed by `block_return_type_for`'s rescue, and `select(&:presence)`-style calls silently flipped onto no-block Enumerator overloads (6 new GitLab errors). Always derive gate names via `respond_to?(:name)`; regression specs pin it. Wall time is **neutral at this slice by design** — the global gate stays inert while legacy `flow_contribution_for` plugins load, so throughput arrives with WD3. **Next-session entry: slice 2** — `dynamic_return methods:`-only gating (no `receivers:`) + migrate rigor-sorbet + rigor-activesupport-core-ext off `flow_contribution_for`; then slice 3 (run-time receiver sets, AR/activestorage), slice 4 (per-file name sets, rspec), slice 5 (**delete** the hook; pre-1.0 BC break, before the ADR-50 v1.0 freeze), slice 6 (single node-rule walk; independent, any time). Gate per slice: the same corpus comparison (cwd=target, project's own `.rigor.yml`) + stackprof deltas in a notes entry.
