...
- `Float` comparison narrowing — **Reserved (as of this writing)**, designed in ADR-109 WD5 and tracked in [#831](https://github.com/rigortype/rigor/issues/831). When it lands it MUST narrow the truthy edge only (`x > c` to the closed `Float[c..]`, `x < c` to `Float[...c]`) and MUST keep the falsy edge's entry type, because `!(x > c)` admits `NaN`.
