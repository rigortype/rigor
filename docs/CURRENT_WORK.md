...
5. **Plugin diagnostic provenance.** ⚠️ partially landed — qualified-rule text rendering shipped (`Diagnostic#to_s` appends `[<source_family>.<rule>]` for non-default source families). Plugin emission protocol on `Rigor::Plugin::Base` deferred until the protocol-shape decision lands.
