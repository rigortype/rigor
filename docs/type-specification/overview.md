...
Where an inline annotation and a standalone `.rbs` declare the **same member** of the same class, Steep 2.x and rbs 4.x define no precedence — both sources are merged into one class entry and the duplicate is reported as a signature error, leaving the class without a method surface — so the SHOULD above resolves to nothing and Rigor decides ([ADR-112](../adr/112-extrbs-comment-channel.md) WD5, replacing [ADR-32](../adr/32-rbs-inline-comment-ingestion.md) WD13's per-member precedence). The two declarations are **compared**, one method name and one overload at a time, position by position — each parameter, the return type, and a block's parameters and return:

- They are **consistent** when, in every position, one side's type is a subtype of the other's. `untyped`, `void` and `top` are consistent with everything and are the least precise answer a position can give; RBS defines the three as the same top type, so two of them are equal. Consistent declarations **merge to the more precise side**: when every position of the inline declaration is at least as precise as the `.rbs` one and at least one is strictly more precise, the inline declaration binds and the `.rbs` member stands down; otherwise the `.rbs` declaration binds. Rigor MUST NOT report a consistent pair. A `rigor:v1:return:` / `rigor:v1:param:` refinement ([rbs-extended.md](rbs-extended.md)) takes part as the type of the position it refines, so `String` in `sig/` and `non-empty-string` inline read as `non-empty-string`.
- They **contradict** when some position's two types are each provably outside the other, when the two accept disjoint numbers of positional arguments, or when one requires a keyword the other cannot accept. The `.rbs` declaration binds, so the class still builds, and Rigor MUST report `rbs.contradicting-signature` as an error ([diagnostic-policy.md](diagnostic-policy.md)). A stale generated signature is the usual cause.
- Any other pair is **undecided**: a position Rigor cannot compare faithfully (a type alias, an interface, `self`, a type variable, a proc, or two classes whose relation it cannot prove), a different overload count, parameter lists of different shapes that still overlap, or each side more precise in a different position. The `.rbs` declaration binds, and the dropped inline declaration MUST be reported (see [ADR-32](../adr/32-rbs-inline-comment-ingestion.md) WD12: an annotation Rigor parses and does not honour is never swallowed).
