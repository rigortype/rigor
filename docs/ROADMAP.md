...
| O2 | substrate floor LANDED (Tier A/B/C engine + Tier D contract + Concern re-targeting); slice 5b (Tier D engine) + slice 6 (precision promotion) queued | Macro-template / heredoc-Ruby expansion. Substrate from [ADR-16](adr/16-macro-expansion.md) shipped through slices 1–5a + slice 7 docs (commits 584ae85…56706a5). Three worked consumers — `rigor-sinatra` (Tier A), `rigor-dry-struct` (Tier C), `rigor-devise` (Tier B) — exercise the substrate end-to-end. Tier D engine integration and precision promotion via ADR-13's resolver chain stay demand-driven. |
...
- **O2 follow-ups** — slice 5b (Tier D engine integration: external-file inclusion under declared `self`) and slice 6 (precision promotion: route Tier C `returns:` strings through ADR-13's `Plugin::TypeNodeResolver`) stay demand-driven. Substrate floor + Tier A/B/C engine landed in v0.1.x.
...
- **O2 — macro-template + heredoc-Ruby expansion.** Substrate floor delivered through [ADR-16](adr/16-macro-expansion.md) slices 1–5a + 7 (commits 584ae85…56706a5): Tier A (block-as-method) + Tier B (trait-inlining registry) + Tier C (heredoc template) engine integration through a new `SyntheticMethodIndex` + pre-pass scanner; Tier D (external-file inclusion) ships contract only with engine integration deferred per the slice-5a deferment; Concern (`included do`) re-targeting handled in the scanner. Three worked consumers landed: `rigor-sinatra` (Tier A), `rigor-dry-struct` (Tier C), `rigor-devise` (Tier B). Remaining items: **slice 5b** (Tier D engine — narrows top-level `self_type` and pre-binds `bound_ivars` for matched external files; queued, demand-driven), **slice 6** (precision promotion — routes Tier C `returns:` strings through ADR-13's `Plugin::TypeNodeResolver` so synthetic methods return precise types instead of `Dynamic[T]`; queued, ceiling work per WD13). Grounding survey at [`docs/notes/20260515-macro-expansion-library-survey.md`](notes/20260515-macro-expansion-library-survey.md).
