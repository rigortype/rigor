...
# marked QUIET-1367 is a write Ruby accepts, or one the rules cannot prove it rejects. The writes sit in methods, so
# the file loads.

# The setter rejects the value's type: TypeError.
...
def last_match_string = ($~ = "x") # FIRES-1367 global.write-type-mismatch — wrong argument type String (expected MatchData)
...
def stdout_time = ($stdout = Time.now) # FIRES-1367 global.write-type-mismatch — $stdout must have write method, Time given
...
def every_member_rejected(flag) = ($/ = flag ? 1 : :lf) # FIRES-1367 global.write-type-mismatch — either arm raises
...
# Values the rules cannot prove the setter rejects, and writes they do not check.
...
def union_with_accepted(flag) = ($/ = flag ? 1 : "\n") # QUIET-1367
