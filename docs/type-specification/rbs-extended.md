...
- If an annotation conflicts with the RBS signature, Rigor MUST report a diagnostic. A member-level `rigor:v1:return:` or `rigor:v1:param:` refinement exceeds the ordinary RBS contract when it is proven disjoint from the type its own member declares at that position, in every overload the refinement applies to (`%a{rigor:v1:return: positive-int}` on `def label: () -> ::String`). Rigor reports it as `rbs.contradicting-signature`, an error, from the comparison that checks a `sig/` declaration against an inline one, and "proven disjoint" means what it means there — only absolutely written core or stdlib classes prove it ([overview.md](overview.md), [ADR-112](../adr/112-extrbs-comment-channel.md) WD5). A declared `untyped`, `void` or `top` holds every refinement, and a declared type Rigor cannot compare faithfully (a type alias, an interface, a type variable) proves nothing. An annotation written on one overload (`def m: %a{…} (Integer) -> void | …`) is not carried by `RBS::Definition::Method#annotations`, so call sites do not honour it and the check does not read it.
...
> **Partly implemented as of this writing.** The exceeds check runs on every member an rbs-inline source declares and on each `sig/` member an inline declaration also declares ([#1075](https://github.com/rigortype/rigor/issues/1075)). A refinement on a member only `sig/` declares is not yet compared with its declared type.
