...
### Plugin contract — interface segregation + ergonomics (ADR-37 / ADR-38) — RELEASE GATES MET

A pre-1.0 plugin-mechanism review ([`docs/design/20260601-plugin-mechanism-pre-1.0-review.md`](design/20260601-plugin-mechanism-pre-1.0-review.md)) drove a large interface-segregation effort. The goal: an AI-legible, per-interface-testable plugin contract where the engine owns the AST walk and gates each narrow extension declaratively (PHPStan-style), with the old fat hooks kept as deprecated escape valves. **The release gates are now met** — every bundled walker plugin is migrated and both ADRs are Accepted; only the non-gating ergonomics follow-ons below remain. Phased plan: [`docs/design/20260602-plugin-boilerplate-reduction-plan.md`](design/20260602-plugin-boilerplate-reduction-plan.md).

**Landed this cycle (see `CHANGELOG.md` § `[Unreleased]`):**

- **[ADR-38](adr/38-additional-initializers.md)** `additional_initializers:` manifest field (def-form) + `rigor-minitest` consumer. **Accepted** (block-form deferred).
- **Author-helper layer** (boilerplate plan Phase 0a/0b): `Rigor::Source::Literals`, `Diagnostic.from_node` / `.from_location`, `Plugin::Base#diagnostic`.
- **[ADR-37](adr/37-plugin-interface-segregation.md) Slice 1/1c/1d**: `node_rule` (engine-owned walk) + `node_file_context` (same-file two-pass) + `NodeContext` (lexical ancestors — the long-promised ADR-2 `ContextInfo`). **All 14 bundled diagnostic-emitting plugins migrated** off the `diagnostics_for_file` walker — including `rigor-actionpack` (the last and most complex: 4 phases, namespace-qualification-sensitive, controller name re-derived from `NodeContext.ancestors`; behaviour-preserving against its golden-master spec).
- **Slice 2**: `dynamic_return(receivers:)` + `type_specifier(methods:)` engine surface (additive, back-compat) + cleanly-fitting consumers migrated (mangrove → `dynamic_return`; minitest + rspec-matcher → `type_specifier`).
- **Slice 3**: `FactProvider` naming + the machine-readable **`rigor plugins --capabilities`** catalogue (per plugin: node_rule node types, dynamic_return receivers, type_specifier methods, produced/consumed facts) — the AI-legibility finale, a differentiator PHPStan lacks.
- **[ADR-37](adr/37-plugin-interface-segregation.md) Accepted**, validated against the full migrated set; `spec/rigor/public_api_drift_spec.rb` pins every new surface.

**Remaining (all non-gating, demand-driven ergonomics):**

1. **Boilerplate plan Phase 0c–0e** (decisions already made — do not re-litigate; each is its own behaviour-preserving slice with per-plugin spec churn, so verify each before landing): **0c** `Plugin::Base#suggest` backed by `DidYouMean::SpellChecker`, replacing the hand-rolled levenshtein in statesman/rails-routes/activerecord (accepts the suggestion-output change + spec updates); **0d** `config_schema` `{kind:, default:}` extension so `Base#config` merges declared defaults, retiring the `DEFAULT_*`-constant idiom (small ADR); **0e** `Plugin::Inflector` unifying the routes/activerecord/actionmailer/actionpack inflector copies — **note the FP caveat:** the routes/activerecord `singularize`/`pluralize` copies feed route-helper name resolution, so unifying them can change *which* diagnostics fire; diff each implementation and re-verify the rails-routes survey before landing (the pure `underscore` casing transform is the safe subset). And the **`Source::Literals` symbol-only variants** (Q1 decision: add them, migrate plugins preserving semantics — `.value`/`.unescaped` are equivalent for literal symbols).
2. **`dynamic_return` generalisation** (optional `methods:` gate / dynamic-receiver predicate) — the path to migrating the escape-valve consumers (rspec `let`-binding, sorbet, activerecord, activestorage). `flow_contribution_for` is the supported deprecated escape valve and those consumers work unchanged; this only widens the narrow surface.
3. **[ADR-38](adr/38-additional-initializers.md) block-form** `additional_initializers` (rspec `before`/`let` whose ivar writes live in a call block, not a `DefNode`) — needs the ivar write-collector to descend declared call blocks.
4. **Per-interface test harnesses** (`NodeRuleTest` / `DynamicReturnTest`) — deferred until a plugin author needs them; the `Analyzer.*_violations_for` split already makes per-node logic unit-testable.

The escape-valve consumers (sorbet / activerecord / activestorage / rspec-let), the dry-rb/graphql pure-FactProvider plugins (no `diagnostics_for_file`, nothing to migrate), and hanami/web (ADR-28 ProtocolContractChecker — a separate common-base axis) are **out of scope of the node_rule/Slice-2 migration** and stay as they are.
