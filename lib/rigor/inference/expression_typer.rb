...
module Rigor
  module Inference
    ...
    class ExpressionTyper
      ...
      # {BodyFixpoint::CAP} passes, so a rebind a counter guards (`row = [v, v] if count > 3`) runs in none of
      # them, and the widen on the capped pass erases a `Constant`'s value but leaves a `Tuple` / `HashShape` as
      # it is. `row = []` therefore came back on its seed, and `sizes.last == 2` folded always-falsey on a
      # runtime `2`, so a shape carrier counts as pinned ({#value_pinned?}). For a shape the unmoved test is
      # type equality, so any rebind that keeps the type is floored with it, not only one restoring the entry
      # value: `qr = n.divmod(3)` rebound to another `divmod` reads `Dynamic[top]` too.
      #
