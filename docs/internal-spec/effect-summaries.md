...
- an ancestry the scan could not read declines outright. `FileCollection::OPAQUE_ANCESTOR` (`"<opaque>"`) is recorded for a superclass expression that is not a constant path (`class K < Struct.new(:a)`, `< Data.define(:a)`, `< DelegateClass(X)`, `< Sequel::Model(:t)`) and for a class body that aliases `initialize` (`alias initialize setup`, `alias_method :initialize, :setup` — the scan models no aliases at all). It matches no summary key, so every other walk steps over it; the constructor rule asks for it by name, because a load-time-built or aliased constructor is the opposite of an absent one.
