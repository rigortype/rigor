...
| ADR-51 | [CI-native diagnostic output formats](51-ci-diagnostic-output-formats.md) | Accepted — partially implemented (v0.1.18). Five CI-native renderings of the existing diagnostic stream behind `rigor check --format`: **`sarif`** (SARIF 2.1.0, the cross-platform anchor — GitHub code-scanning + any SARIF tool + reviewdog `-f=sarif`), **`github`** (GitHub Actions `::error file=…,line=…::` workflow commands → inline PR annotations, no upload step), **`gitlab`** (GitLab Code Quality / CodeClimate-subset JSON → the MR widget), **`checkstyle`** (Checkstyle XML → reviewdog `-f=checkstyle` posts to *any* reviewdog reporter — GitHub PR review / GitLab MR / Gerrit / Bitbucket / Gitea — and Jenkins), **`junit`** (JUnit XML → GitHub/GitLab/CircleCI/Jenkins test reports). Each is a pure presentation layer over the same `Analysis::Diagnostic` fields `--format json` exposes — no new analysis. Criterion for a `--format` value: *a CI platform renders the stream inline from it* (so CSV / generic shapes stay out — `json` serves them). WD1 adds the broad-reach pair on a *reach-per-cost* basis; comparator = PHPStan (ships github/gitlab/checkstyle/junit/teamcity, **no SARIF** — Rigor leads with SARIF, PHPStan's gap). WD2 fixes the severity + identifier **contract table** (error/warning/info → SARIF error/warning/note · GitHub error/warning/notice · GitLab major/minor/info · Checkstyle error/warning/info · JUnit failure-type error/warning/info; qualified rule → `ruleId`/`title=`/`check_name`/Checkstyle `source`/JUnit `classname`); WD3 the GitLab fingerprint (SHA256 of `(path, rule, line, column, message)` — stable, not index-based); WD4 leaves the exit code unchanged (file output via `>` redirect + `if: always()` upload); WD5 maps every diagnostic to a JUnit `failure` (surfacing, not gating — severity preserved in `type`). A new output format is public contract under [ADR-50](50-release-engineering-and-stability-strategy.md) WD1. WD6 also ships [ADR-27](27-tool-distribution-model.md) § WD3's queued copy-paste CI setup templates (`.github/workflows/rigor.yml` + `.gitlab-ci.yml` + a reviewdog `action-setup` variant) and the bundled `rigor-ci-setup` skill. Deferred: `--output FILE`, reviewdog native `rdjson`, TeamCity, richer SARIF rule metadata |
