...
module Rigor
  module Inference
    module MethodDispatcher
      ...
      # a literal with a `**splat` entry (the literal's own type leaves the splat out), and an empty closed shape,
      # which is what a mapping filled through an alias the engine does not track still reads as. A block the
      # call carries but the block pass could not type contributes `Dynamic[top]` too.
      #
      ...
      module HashTransformKeysFolding
        ...
        def mapping_arm(context, block)
          ...
          splatted_literal?(arguments.first) ? Type::Combinator.union(renamed, unknown) : renamed
        end
        ...
        # (`{ **o, b: :y }` reads `Hash[:b, :y]`), so the splatted mapping's values are missing from it.
