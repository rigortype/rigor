...
| **Cache schema version** | `Descriptor::SCHEMA_VERSION` + `Store::FORMAT_VERSION` in [`lib/rigor/cache/`](../lib/rigor/cache/) (marker `4.2`) | [Manual ch. 12 — Caching](manual/12-caching.md) | **yes** — a schema/format bump invalidates the cache, never silently mis-reads it |
...
| Persistent cache | `Cache::Descriptor::SCHEMA_VERSION`.`Cache::Store::FORMAT_VERSION` (the `schema_version.txt` marker) | `4.2` |
