...
**ADR-50 remaining (post-v0.2.0):** the versioning model + support line (WD5 — edition-style post-1.0: soft-break minors behind bleeding-edge / baseline, hard-break majors; a maintenance line only for the release behind the most recent *hard* break — previous minor pre-1.0, previous major post-1.0 — with backports on-demand-not-guaranteed) and a `rigor upgrade` migration command (WD7, deferred until a concrete BC gives it a target). The first bleeding-edge `FEATURES` entry has landed (`reject-unparseable-signatures`, [PR #96](https://github.com/rigortype/rigor/pull/96)); further disciplines land as single feature entries.
...
1. **`dynamic_return` generalisation + `flow_contribution_for` retirement — DONE via [ADR-52](adr/52-compiled-plugin-contribution-dispatch.md).** This was the ADR-37 sketch of the path; ADR-52 built it out (run-time `methods:` / `receivers:` / `file_methods:` gates, slices 2–5a) and then **deleted** the fat `flow_contribution_for` hook (slice 5b, a deliberate pre-1.0 BC break — defining it now raises at load). All five escape-valve consumers (rspec `let`-binding, sorbet, activerecord, activestorage, rspec-rails) are migrated onto the narrow gated surface. No longer open.
...
The dry-rb/graphql pure-FactProvider plugins (nothing to migrate) and hanami/web (ADR-28 ProtocolContractChecker — a separate common-base axis) are **out of scope of the node_rule/Slice-2 migration** and stay as they are. (The former escape-valve consumers — sorbet / activerecord / activestorage / rspec-let — were migrated off `flow_contribution_for` by ADR-52, item 1 above.)
...
### Sig-gen (ADR-14)
- **LANDED — sig-gen cannot emit unparseable RBS (the output-validity guard).** Two bugs had shipped RBS that
  `rbs` itself rejects: a non-identifier record key (`{ :"data-contrast" => T }`, fixed in the ADR-82 arc) and a
  `&block` constructor param rendered before the parens ([PR #51](https://github.com/rigortype/rigor/pull/51)).
  Both were found *downstream*, as a poisoned `sig/` tree — and the consequence sharpened once an unparseable
  `.rbs` became a quarantined (types-deleted) file and, under `reject-unparseable-signatures`, a build failure.
  Every rendered line is now parsed at the generator's single candidate-construction choke point (so the guard
  covers `--print` / `--diff` / `--write` / MCP at once): a line `rbs` rejects is skipped
  (`sig.skipped.unrenderable-rbs`) and reported on stderr as the Rigor rendering defect it is, rather than
  emitted. The writer keeps a second guard over the fully-assembled file — a bad merge-splice is refused, not
  written, so the user's own hand-written signatures in that file survive — and `--write` exits non-zero when a
  requested write was refused. Skipping (not failing) is the generator-side default: it keeps sig-gen useful on
  a project with one pathological method. Verified clean across Rigor's own `lib` + five survey projects.
