...
- `obj.respond_to?(:foo)` records a public existence fact for `foo` on the true branch. The true branch also drops the members of `obj`'s type whose class Rigor knows to lack `foo`, and reads `Dynamic[top]` when no member may respond, so the guarded `obj.foo` does not report ([control-flow-analysis.md](control-flow-analysis.md#class-guards), [#1429](https://github.com/rigortype/rigor/issues/1429)).
