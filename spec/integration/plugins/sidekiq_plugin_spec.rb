...
RSpec.describe "plugins/rigor-sidekiq" do
  ...
  # Issue #1056 — the "worker index did not load" notice is a RUN-scoped disclosure, not a per-file
  ...
  describe "the worker index load error is disclosed once per run (#1056)" do
    before { allow(Rigor::Plugin::Sidekiq::WorkerDiscoverer).to receive(:new).and_raise(RuntimeError, "boom") }
    ...
    it "emits one row at .rigor.yml:1:1, not one per analysed file" do
      ...
      expect(rows.first.message).to include("failed to discover workers")
    end
  end
end
