...
- [ADR-51](docs/adr/51-ci-diagnostic-output-formats.md) — CI-native diagnostic output formats (accepted — partially implemented v0.1.18; `rigor check --format sarif|github|gitlab`: SARIF 2.1.0 cross-platform anchor + GitHub Actions workflow commands (inline PR annotations, no upload step) + GitLab Code Quality JSON (MR widget), each a pure presentation layer over the same `Analysis::Diagnostic` fields `--format json` exposes — no new analysis; criterion = *a CI platform renders the stream inline from it*; WD2 fixes the severity/identifier contract table (error/warning/info → SARIF error/warning/note · GitHub error/warning/notice · GitLab major/minor/info; qualified rule → `ruleId`/`title=`/`check_name`), WD3 the stable GitLab fingerprint (SHA256 of `(path, rule, line, column, message)`, not index-based), WD4 leaves the exit code unchanged (file output via `>` redirect + `if: always()` upload); new output format is public contract under ADR-50 WD1; WD5 also ships ADR-27 § WD3's queued copy-paste CI setup templates; `--output FILE` / JUnit XML / rich SARIF rule metadata deferred)
