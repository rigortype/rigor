...
- **Containers**: `ArrayShape`, `Tuple`, `HashShape`, `Record`. Also generic envelope carriers `Result[T, E]` and `Maybe[T]` (ADR-20 slice 4 amendment). See [`rbs-compatible-types.md`](../type-specification/rbs-compatible-types.md) for the RBS-derived forms and [`rigor-extensions.md`](../type-specification/rigor-extensions.md) for the refinements (required/optional keys, read-only entries, extra-key policy).
