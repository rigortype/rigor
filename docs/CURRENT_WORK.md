...
- **Slice 5 (documentation)** — landed unreleased. New normative spec at [`docs/internal-spec/dependency-source-inference.md`](internal-spec/dependency-source-inference.md) covering the four landed slices end-to-end (configuration, resolver + index, walker, dispatcher tier, cache slice) plus the live + pending diagnostic family and the boundary contracts with ADR-2 / ADR-5 / ADR-9. `docs/internal-spec/README.md` reading-order table updated; cross-links in `special-types.md` / `inference-budgets.md` / `diagnostic-policy.md` upgraded from "ADR-10 only" to "ADR-10 (analyzer contract: dependency-source-inference.md)" so readers find the implementation contract one click away.
- **Slice 4** queued (per-gem budget pool + `dynamic.dependency-source.budget-exceeded`).
...
Read [ADR-10](adr/10-dependency-source-inference.md) § "Implementation slicing" for the binding slice list. With Slices 1, 2a, 2b-i, 2b-ii, 3, and 5 landed, only Slice 4 (per-gem budget pool + `dynamic.dependency-source.budget-exceeded`) remains. Slice 4 has open design judgments — config-key location (`dependencies.budget_per_gem` vs `budgets.dependency_source_per_gem`), the budget unit / default value, and the precise "trip" semantics. See [`docs/internal-spec/dependency-source-inference.md`](internal-spec/dependency-source-inference.md) § "Stability" for the substrate the slice will extend, and [ADR-10](adr/10-dependency-source-inference.md) § "Open questions" for the queue around it (per-receiver plugin veto, `mode: full` retention, cache size cap, configurable tier ordering).
