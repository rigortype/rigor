...
- `Rigor::Reflection.project_declared_class?(class_name, scope: nil, environment: nil)` — `true` when the class's RBS declaration lives under the project's own `signature_paths:` rather than in bundled core / stdlib / gem RBS. The distinction is one of AUTHORITY: a bundled signature describes a class the project does not own, so a project `def` on it is a monkey-patch; a project sidecar describes the very source under analysis, so a `def` in another of the project's files is that class's own definition. Attribution is by declaration buffer name and MUST answer `false` whenever it cannot be determined (no loader, no environment, an environment blob predating buffer-name preservation) — the fail-soft answer is the one that changes no diagnostic.
