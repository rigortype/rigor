...
Boundary: Slice 3 phase 2 does NOT yet thread scope through arbitrary expression interiors (`foo(x = 1)` and `[1, x = 2]` still drop `x` from the post-scope), and the CLI commands `rigor type-of` and `rigor type-scan` continue to operate against `Scope#type_of` with an empty scope. Both are deliberate Phase 2 simplifications; the StatementEvaluator surface above is stable and future commits can grow the catalogue and the CLI integration without breaking it.

Coverage uplift on `rigor type-scan lib`: unchanged from Slice 5 phase 1 (the type-scan tool still types every node in the empty scope and so does not yet observe the StatementEvaluator's scope-threading; this is the deliberate boundary above). The behavioral uplift is observable through the new RSpec coverage and the Scope#evaluate smoke probes, and it sets the stage for Slice 6 narrowing — which depends on a control-flow-aware scope being available at every program point.
