...
- The method carries **no authored bound of its own**. An envelope on the method, on its class, in `effects.envelopes:` or in an rbs-inline annotation is the author's statement about this very method; it is checked rather than propagated, so it never enters any summary's `≤` lane and an emitter reading the table alone cannot see it. Writing an inferred annotation over it would replace a contract with an inference about the body the contract was written about.
