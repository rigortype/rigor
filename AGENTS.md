...
The current implementation is the v0.1.3-in-progress inference engine on top of the v0.1.0 plugin contract. It parses Ruby with `Prism`, runs a flow-sensitive type-inference engine over each file, consults RBS signatures (bundled stdlib + project `sig/` + gem RBS) plus in-source `def` / `define_method` / `attr_*` / `Data.define` discovery, and reports a deliberately-narrow rule catalogue (the `call.*` / `flow.*` / `def.*` / `assert.*` / `dump.*` families). Worked plugin examples live under [`examples/`](examples/README.md); the count drifts as new plugins land, so consult that README for the canonical list rather than hard-coding it elsewhere. The nine-chapter end-user handbook lives under [`docs/handbook/`](docs/handbook/README.md).

v0.1.3 adds three substantive surfaces beyond the v0.1.0 plugin contract:

- **Opt-in gem-source inference** per [ADR-10](docs/adr/10-dependency-source-inference.md). Gems listed under `dependencies.source_inference:` in `.rigor.yml` have their `lib/` walked as a per-gem `Dynamic[T]`-bearing fallback, sitting strictly below RBS / plugin contracts in the dispatcher tier.
- **Sorbet input adapter** per [ADR-11](docs/adr/11-sorbet-input-adapter.md) — `examples/rigor-sorbet/` ingests `sig { … }` blocks, `T.let` / `T.cast` / `T.must` / `T.bind` / `T.absurd`, and RBI files as a plugin (translation at the plugin boundary; core stays RBS-canonical).
- **Plugin TypeNode resolver** per [ADR-13](docs/adr/13-typenode-resolver-plugin.md). `Rigor::Plugin::TypeNodeResolver` lets plugins contribute custom named- / generic-type vocabulary that resolves inside `%a{rigor:v1:…}` payloads. The parser is now a two-pass design (scan to a `Rigor::TypeNode` AST, then a `Resolver` pass walks the AST through built-in registry → plugin chain → RBS Nominal fallback). Five Rigor-canonical shape-projection type functions (`pick_of` / `omit_of` / `partial_of` / `required_of` / `readonly_of`) live on `Type::Combinator`; the opt-in `examples/rigor-typescript-utility-types/` plugin maps the TS-canonical spellings (`Pick` / `Omit` / `Partial` / `Required` / `Readonly`) onto them.
...
- Use RBS for external dependency and standard library type information by default. Rigor-specific advanced type expressions live in RBS comment extensions (`%a{rigor:v1:…}`) parsed by [`lib/rigor/builtins/imported_refinements.rb`](lib/rigor/builtins/imported_refinements.rb). The parser is a two-pass design — scan to a `Rigor::TypeNode` AST, then a `Resolver` pass walks AST through built-in registry → plugin `TypeNodeResolver` chain ([ADR-13](docs/adr/13-typenode-resolver-plugin.md)) → RBS Nominal fallback. When extending the payload grammar, edit the parser AND the spec rows in [`docs/type-specification/imported-built-in-types.md`](docs/type-specification/imported-built-in-types.md) / [`docs/type-specification/type-operators.md`](docs/type-specification/type-operators.md) in the same commit. As of v0.1.3, a per-gem opt-in (`dependencies.source_inference:` in `.rigor.yml`, per [ADR-10](docs/adr/10-dependency-source-inference.md)) lets the analyzer walk a no-RBS gem's `lib/` as a `Dynamic[T]`-bearing fallback below the RBS tier — RBS / RBS::Inline / generated stubs / plugin contracts still win on conflict.
