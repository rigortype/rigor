...
- **Bounded floats from folds** ([ADR-109](../adr/109-ruby-native-range-notation.md)). A draw, a monotone function, and a clamp all land in an interval the call names, and that interval is a `FloatRange` / `IntegerRange`: `rand(a..b)` and `Random.rand(a..b)` SHOULD return the literal range itself (the bare and `rand(n)` forms are left to RBS, since the corpus reads them as its unknown-value oracle); `Math.sqrt` / `exp` / `log` and the other monotone functions over a bounded argument SHOULD return the range between the images of its bounds, and MUST decline when a bound lies below the function's domain (the value that reaches it raises); `abs`, `clamp(lo, hi)` and `clamp(range)` on a bounded receiver SHOULD narrow to the bracket, and on a plain `Integer` / `Float` receiver SHOULD return the bracket entire — `clamp` lands inside the bracket whatever bounds the receiver carries, so `i.clamp(1, 9)` and `i.clamp(1..9)` are both `Integer[1..9]`. The bracket's endpoints MUST be literals of the receiver's own class: a mixed bracket (`1.clamp(0.5, 2.5)`) returns the receiver OR a bound, whose class is a union, and MUST decline. An exclusive end (`clamp(1...9)`) and a reversed bracket both raise at run time and MUST decline on every receiver. A bounded Float never holds `NaN`, so `nan?` on it is `false` and `finite?` is decided by its bounds; `floor` / `ceil` / `round` / `to_i` map finite bounds onto an `IntegerRange` and MUST decline on an infinite bound, which raises `FloatDomainError` for the value that reaches it. A plain `Float` receiver takes none of the others: it may be `NaN`. `clamp` is the exception, and soundly so — a `NaN` receiver raises inside the comparison, so a call that returns is inside the bracket, which no `FloatRange` holds `NaN` in either.
...
- **Override signature checking ([ADR-35](../adr/35-override-signature-compatibility.md), shipped v0.1.15)**: where this principle *biases inferred* signatures toward substitutability, the `def.override-*` rule family *verifies authored* ones across a project-defined hierarchy — `def.override-return-widened` is the return-covariance counterpart of clause 1, and `def.override-param-narrowed` the parameter-contravariance counterpart of clause 2. The rules fire only when both the override and the shadowed ancestor carry an author-supplied signature (inference-only either side stays silent, so the principle's authorship choices are never themselves flagged), and severity maps through `severity_profile:`.
