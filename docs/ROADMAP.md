...
| O2 | substrate floor + precision promotion LANDED (Tier A/B/C engine + Tier D contract + Concern re-targeting + slice 6a/6b precision); slice 5b (Tier D engine) + ADR-13 resolver-chain wiring for utility-type returns queued | Macro-template / heredoc-Ruby expansion. Substrate from [ADR-16](adr/16-macro-expansion.md) shipped through slices 1–5a + slice 7 docs (commits 584ae85…56706a5) + slice 6a-TierB / slice 6b-TierC precision promotion (commits d174fff / d7b1943). Three worked consumers — `rigor-sinatra` (Tier A), `rigor-dry-struct` (Tier C), `rigor-devise` (Tier B) — exercise the substrate end-to-end. Slice 6's precision promotion: Tier B emissions get the module's authored RBS return; Tier C plain class-name returns resolve via `environment.nominal_for_name`. Tier D engine integration and ADR-13 resolver-chain wiring for parameterised / utility-type returns stay demand-driven. |
...
- **O2 follow-ups** — slice 5b (Tier D engine integration: external-file inclusion under declared `self`) and full ADR-13 `Plugin::TypeNodeResolver` chain wiring for the synthetic-method tier (unlocks utility-type-shaped substrate returns like `Array[String]` / `Pick<T, K>`) stay demand-driven. Substrate floor + Tier A/B/C engine + slice 6 precision promotion landed in v0.1.x.
