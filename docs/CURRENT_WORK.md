...
**v0.1.0 slice 5 (partial) landed on `master` (unreleased).** Qualified-rule text rendering: `Diagnostic#to_s` now appends `[<source_family>.<rule>]` when the diagnostic carries a non-builtin source family (per [ADR-2 § "Plugin Diagnostic Provenance"](adr/2-extension-api.md)). Loader failures (`:plugin_loader` family, `load-error` rule) and any future `plugin.<id>.<rule>` / `rbs_extended.<rule>` / `generated.<provider>.<rule>` diagnostics surface their provenance in standard `rigor check` output without changing built-in-rule layout. **Plugin emission protocol intentionally deferred** — the matching hook on `Rigor::Plugin::Base` carries plugin-protocol design decisions tied to slices 4 / 6. **Working state: 1844 RSpec examples / 0 failures, RuboCop 194 files / 0 offenses, `bundle exec exe/rigor check lib` reports 0 diagnostics.**
...
4. **FlowContribution wiring through internal narrowing.** Internal-only refactor that turns the merger into the single point of integration (carry-over from v0.0.9). Working decisions pinned by [ADR-7 § "Slice 4"](adr/7-v0.1.0-slice-decisions.md): introduce a normalised `Rigor::FlowContribution::Fact` value object as the canonical slot payload (4-A); apply the merger per call-site (4-B). No user-visible behaviour change.
