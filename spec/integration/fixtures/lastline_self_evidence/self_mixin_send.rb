...
# Issue #1415 — a `send` naming `include` mixes `RubyReader` into `Object`, as `include` does, so no implicit-self
# reader in the file narrows `$_`. Run with `read` as the first argument and "a\nb\n" on standard input; each case cites
# what Ruby 4.0.5 answers.

send(:include, RubyReader)
