...
module PublicApiDriftSnapshots # rubocop:disable Metrics/ModuleLength
  SCOPE_INSTANCE = %w[
    ...
    constant_narrowing(req:key)
    constant_narrowings()
    constant_sources()
    cvar(req:name)
    cvars()
    data_member_layout(req:class_name)
    data_member_layouts()
    declaration_sourced()
    ...
    discovery()
    dynamic_origins()
    enqueue_ancestors(req:current,req:queue,req:name_memo,key:mixins)
    ...
    forget_error_info()
    forget_guard_narrowing_of(req:kind,req:name)
    forget_guard_narrowings()
    forget_last_line()
    forget_last_status()
    forget_match_globals()
    global(req:name)
    globals()
    guard_facet(req:kind,req:name)
    guard_facets()
    guard_live()
    guard_live?()
    guard_narrowed?()
    ...
    instance_def_shadows_call?(req:class_name,req:method_name,req:call_node)
    ivar(req:name)
    ...
    optimistic_ivar_miss(req:name)
    optimistic_ivars()
    optimistic_local(req:name)
    optimistic_local_miss(req:name)
    optimistic_locals()
    optimistic_origins()
    ...
    repeated_or_writes()
    run_generation()
    seed_declaration_sourced_global(req:name,req:type)
    ...
    singleton_def_shadows_call?(req:class_name,req:method_name,req:call_node)
    singleton_def_through_ancestors(req:class_name,req:method_name,key:name_memo)
    singleton_extends_of(req:class_name)
    source_path()
    struct_fold_safe?(req:name)
    struct_fold_safe_locals()
    struct_member_layout(req:class_name)
    ...
    user_singleton_def_site_for(req:class_name,req:method_name)
    void_origins()
    with_constant_narrowing(req:key,req:type,req:pre_guard)
    ...
    with_global_copy_marks(req:name,req:globals)
    with_gradual_bindings(req:entry)
    with_guard_facet(req:kind,req:name,req:type)
    with_guard_live()
    with_guarded_global(req:name,req:type,req:pre_guard,key:record)
    ...
    with_mutated_local(req:name,req:type)
    with_optimistic_ivar(req:name,req:cause,key:miss)
    with_optimistic_local(req:name,req:cause,key:miss)
    with_published_constant_mark(req:kind,req:name)
    with_repeated_or_writes(req:nodes)
    with_self_type(req:type)
    with_singleton_class_body(req:flag)
    with_source_path(req:path)
    with_struct_fold_safe(req:locals)
    without_constant_narrowing(req:key)
    without_constant_narrowings_named(req:name)
    ...
    without_local_declaration_marks(req:name)
  ...
  ENVIRONMENT_INSTANCE = %w[
    ...
    global_for_name(req:name,key:builtin)
    hkt_registry()
    hkt_scan_failure()
    missing_rbs_gem_owner(req:root_constant_name)
    name_scope()
  ...
  REFLECTION_SINGLETON = %w[
    ...
    lexical_nesting_chain(req:scope)
  ...
  PLUGIN_BASE_INSTANCE = %w[
    ...
    config()
    declared_members(req:class_name)
    diagnostic(req:node,keyreq:path,keyreq:message,key:severity,key:rule,key:location)
    diagnostics_for(req:violations,keyreq:path,key:node)
    diagnostics_for_file(keyreq:path,keyreq:scope,keyreq:root)
    disclose_once(req:key,keyreq:message,key:severity,key:rule)
    ...
    producer_error(req:id)
    producer_value(req:id,key:params)
    protocol_contracts()
  ...
  PLUGIN_MANIFEST_INSTANCE = %w[
    ...
    produces()
    protocol_contracts()
    rbs_complete_ancestors()
    rbs_complete_extends()
    signature_paths()
  ...
  PLUGIN_REGISTRY_INSTANCE = %w[
    ...
    plugins()
    protocol_contracts()
    rbs_complete_ancestor?(req:class_name)
    rbs_complete_ancestors()
    rbs_complete_extends()
    rbs_complete_extends?(req:class_name)
    resolved_gem_paths()
    signature_paths()
  ...
  COMBINATOR_SINGLETON = %w[
    ...
    data_class_of(keyreq:members,key:class_name)
    data_instance_of(keyreq:members,key:class_name)
    decimal_int_string()
    difference(req:base,req:removed)
    dynamic(req:static_facet)
    finite_float()
    float_range(req:min,req:max,key:exclude_end)
    ...
    non_zero_int()
    non_zero_int_compatible?(req:type)
    non_zero_int_difference?(req:diff)
    numeric_string()
    octal_int_string()
    ...
    result_of(req:ok_type,req:err_type)
    shape_projection_lossy?(req:type)
    singleton_of(req:class_name_or_object)
    struct_class_of(keyreq:members,key:class_name,key:keyword_init)
    struct_instance_of(keyreq:members,key:class_name)
    top()
