...
- **Invalidation.** The match globals are frame-local facts. Ruby keeps them in the special-variable slot of the method body (or class, module or file body) that runs the match. A block, and a closure created in that body — a lambda literal, `lambda {}`, `proc {}`, `Proc.new {}` — ordinarily reaches the same slot, so a match it runs rebinds the body's globals. A nested `def`, class or module body has a slot of its own, and so does every call into a method defined in Ruby: a match in the callee's body rebinds the callee's slot, never its caller's, so `log("parsed")`, `warn "debug"` or `self.log("x")` between the predicate and a global's use leaves the narrowing in place ([#1364](https://github.com/rigortype/rigor/issues/1364)). The caller's slot is reached only by C code that matches on its behalf, by code evaluated in its frame, and by a block or closure it created. An intervening call that may reach it, between the predicate and a global's use, invalidates the narrowing:
  - a call that matches on the slot's behalf, read by the method it calls and the types the flow scope gives its operands, on any receiver ([#1365](https://github.com/rigortype/rigor/issues/1365)):
    - `=~`, `!~`, `match`, `sub`, `sub!`, `gsub`, `gsub!` and `scan`, whatever their argument: `sub` and `scan` with a String pattern still set the globals, and `!~` runs `=~`;
    - `[]`, `slice`, `slice!`, `index`, `rindex`, `byteindex`, `byterindex`, `partition`, `rpartition`, `split`, `start_with?`, `any?`, `all?`, `none?` and `one?` with an argument that may be a Regexp. `grep` and `grep_v` count on the same terms, but only in their block form, since without a block they leave the caller's globals alone. `[]=`, and an index `||=`, `&&=` or `op=`, count with an index that may be one (`s[re] = v`); the value stored is not read;
    - `===` and unary `~` on a receiver that may be a Regexp: `a === b` is `a`'s method, so `String === s` runs no match, and `~n` on an Integer is a bitwise not;
    - `eval`, `instance_eval`, `class_eval` or `module_eval` with an argument, which runs a String of code in this frame (the block form of the last three is a block): `eval(src)`, `Kernel.eval(src)`, `binding.eval(src)`, `obj.instance_eval(src)`;
    - `send`, `__send__` or `public_send` whose method-name argument is not a Symbol or String literal, or names a method this rule counts with the arguments sent after it: `u.send(:=~, re)` counts, `u.send(:match?, re)` does not.

    `match?` never counts, because it never sets `$~`. An operand *may be a Regexp* when it is a regex literal, a forwarded `...`, a splat whose elements may be one, or a value whose type admits one: `Regexp` or a subclass, an ancestor such as `Object` or `Kernel`, `top`, an interface, a union with such a member, or `Dynamic[top]`. A `Dynamic` value with a static facet is read by its facet. A literal that is not a Regexp, and a value typed `String`, `Symbol`, `Integer`, `Array`, `Hash`, a class, a tuple or a hash shape, may not be one, so `val = row[:name]`, `csv.split(",")`, `list.index(3)`, `String === s` and `s.match?(re)` leave the narrowing in place, as Ruby leaves `$1`. `Dynamic[top]` counts because nothing is known about it: `row[key]` with an unannotated `key`, and `===` on a constant that does not resolve, still invalidate. That keeps the invalidation the method name alone gave before this rule wherever the analyzer cannot type the argument, at the cost of a lookup that turns out to be a Hash or Array lookup at runtime;
  - an implicit-self or `self.` call whose arguments may run a match in this frame although no call in them does: a Symbol or String literal naming one of the methods above (`inject(:=~)`), which the method may run from C in this frame, a `yield`, or any construct a block body may match with (below), under the broad reading of the next rule;
  ...
  - a C-implemented method outside the names above, such as a native extension's, and `inject(:=~)` or `reduce(:=~)` on an explicit receiver, which runs a match method from C in this frame;
  - inside a block or closure body, `[]=` with a Regexp index, unary `~`, a `send` naming a match method or with a computed name, and an `eval` of a String, which the block scan above does not read ([#1377](https://github.com/rigortype/rigor/issues/1377));
  ...
  - a read of a global later in the same statement than an operand that rebinds it (`log(s.sub(re, ""), $1)`), which reads the narrowing from before the statement, and the right operand of an `&&` or `||` condition, whose combined edge keeps the left operand's `=~` narrowing (`if s =~ re && u !~ other`, [#1376](https://github.com/rigortype/rigor/issues/1376));
