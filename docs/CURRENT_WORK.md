...
1. **Plugin registration / loading** — manifest discovery (gem metadata + project-local plugins), deterministic load ordering, and dependency-injected analyzer services (`Reflection`, type factories, configuration readers). ADR-2 § "Registration, Configuration, and Caching". Smallest viable slice; no behaviour change to the engine yet, just a registry that the rest of v0.1.0 attaches to.
