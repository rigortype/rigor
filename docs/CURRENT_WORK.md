...
**v0.1.3 in progress, release pending.** Theme: **deliver [ADR-10](adr/10-dependency-source-inference.md) end-to-end, absorb ADR-11 / Rails plugin Phase work, and land ADR-13.** Five tracks closed; two ADR-13 polish slices deferred to v0.1.4:
...
- **[ADR-13](adr/13-typenode-resolver-plugin.md) (plugin TypeNode resolver + TS-utility-type adapter) — slices 1–6 landed.** New `Rigor::Plugin::TypeNodeResolver` extension point with manifest hook + registry aggregation. Parser refactored from inline scan+resolve into "scan to AST" + sibling `Resolver` pass. Five Rigor-canonical shape-projection type functions on `Type::Combinator` (`pick_of` / `omit_of` / `partial_of` / `required_of` / `readonly_of`) — phase A on HashShape, phase B on Tuple, predicate `shape_projection_lossy?` exposed for slice-3b consumers. New opt-in `rigor-typescript-utility-types` plugin under `examples/` maps the TS spellings (`Pick`, `Omit`, `Partial`, `Required`, `Readonly`) onto the core functions.
- **handbook expansion.** Chapter 7 gains a `@phpstan-assert` correspondence table; chapter 10 (rigor-sorbet) covers all the new sorbet recognisers. TypeScript appendix update (slice 7 of ADR-13) is deferred to v0.1.4.
...
7. ⏭ **Slice 3b** (deferred) — `RbsExtended` caller threading + `dynamic.rbs-extended.unresolved` (whole-payload resolution failure) + `dynamic.shape.lossy-projection` (consults the slice-5 predicate at projection-authoring sites).
...
v0.1.3's four primary tracks are closed; two ADR-13 polish slices deferred to v0.1.4:
...
- **ADR-13**: Slices 1–6 landed (core machinery + opt-in plugin). Slices 3b + 7 deferred to v0.1.4 — see below.
...
2. **Close the ADR-13 polish track:**
   - **Slice 3b — caller threading + diagnostics.** Thread `name_scope:` through `RbsExtended.resolve_directive_rhs` and the two sibling `Builtins::ImportedRefinements.parse` call sites. Wire `dynamic.rbs-extended.unresolved` for whole-payload resolution failures (currently fail-soft to nil silently). Wire `dynamic.shape.lossy-projection` at projection-authoring sites by consulting `Type::Combinator.shape_projection_lossy?` (predicate landed in slice 5, no consumer yet). The threading touches every `Builtins::ImportedRefinements.parse(payload)` call site and the analyzer surface around them; budget a single focused commit. Likely 200-400 LoC including specs.
...
The default goal for the next session is "cut the v0.1.3 release" (substantial work accumulated, four primary tracks closed). If continuing implementation instead, the natural entry slice is ADR-13 slice 3b (caller-side diagnostic threading) — small, well-scoped, no architectural decisions left. Read in this order:
...
After those, the implementation surface for v0.1.3's remaining slices (3b / 7) is locatable from grep over:
- `lib/rigor/rbs_extended.rb` for `resolve_directive_rhs` and sibling `Builtins::ImportedRefinements.parse` call sites (slice 3b caller-threading).
- `lib/rigor/builtins/imported_refinements.rb` for the parser entry and the `Resolver` class (slice 3b diagnostic emission).
- `lib/rigor/type/combinator.rb` for `shape_projection_lossy?` (slice 3b consumer site).
