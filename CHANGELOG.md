...
- **Engine surfaces used.** `Scope#type_of(node)` for the per-call query; `Type::Combinator.literal_string_compatible?(type)` for the literal-string predicate the v0.0.9 carrier publishes; `Type::Constant#value` for exact-value extraction when the type is a `Constant<String>`. The plugin therefore inherits every literal-string improvement Rigor lands going forward (interpolation folding, `String#format`, `String#%`, `Array#join`, …) without changes — the demo confirms `"user" + "@example.com"` is folded by the analyzer's `LiteralStringFolding` tier and the plugin matches against the resulting `Constant<"user@example.com">`.
- **Diagnostics.** `:info plugin.pattern.literal-match` when a literal exact value matches the named pattern; `:error plugin.pattern.literal-mismatch` when it does not; `:info plugin.pattern.literal-unknown` when the value is provably literal-string-compatible but the exact value is not statically known (e.g. interpolation lifts to the literal-string refinement carrier without going all the way to a `Constant<String>`); `:error plugin.pattern.unknown-pattern` when the call references a pattern name not declared in the plugin config.
- **Rich `config_schema`.** Patterns are declared in `.rigor.yml` as a `Hash<String, String>` of name → regex source; `:hash` is the kind the manifest validator accepts. Invalid regexes raise from `init` and surface as `:plugin_loader load-error` diagnostics through the runner's loader-failure isolation path.
- **Demo project** under `examples/rigor-pattern/demo/` (`.rigor.yml` + `demo.rb` + `lib/validators.rb`). Run with `RUBYLIB=$PWD/../lib bundle exec rigor check` from inside the demo directory.
- **Integration spec** at [`spec/integration/examples/pattern_plugin_spec.rb`](spec/integration/examples/pattern_plugin_spec.rb) — 12 examples covering Constant<String> direct literals, engine collaboration through `LiteralStringFolding` (string concatenation + interpolation), non-literal values stay silent, unknown-pattern, shape filtering (non-Symbol first arg, < 2 args, different method name), and the configurable `method_name` override.

### Added — example plugin: `rigor-routes`

- **Reference example for the v0.1.0 plugin trust + cache surfaces** under [`examples/rigor-routes/`](examples/rigor-routes/). Where `rigor-lisp-eval` and `rigor-units` only exercise slice 5 (diagnostic emission) over the in-memory AST, `rigor-routes` is the first example to demonstrate **slice 2 (`Plugin::IoBoundary` / `Plugin::TrustPolicy`)** and **slice 6 (`Plugin::Base.producer` / `#cache_for`)**. The plugin validates Rails-style route helper calls (`users_path`, `edit_user_path(@user.id)`, …) against a YAML route table read once from `config/routes.yml`. Recognised calls surface as `:info` notes naming the resolved `METHOD /path`; unknown helpers surface as `:error` with Levenshtein-distance did-you-mean suggestions; arity mismatches list every required `:placeholder`.
- **IoBoundary integration.** `route_table` reads `config/routes.yml` via `io_boundary.read_file(@routes_file)`, scoped by the active `TrustPolicy.allowed_read_roots` (the project root by default). Read failures (`Plugin::AccessDeniedError`, `Errno::ENOENT`, `Psych::SyntaxError`) collapse into a single `:warning` `load-error` diagnostic at line 1 instead of crashing the run.
- **Cache producer.** A `Plugin::Base.producer :route_table do ... end` block parses the YAML through `RouteTable.parse`. The auto-built `Cache::Descriptor` (from `Plugin::Base#cache_for`) stitches together (a) the plugin's `PluginEntry` template (id + version + config_hash) and (b) the `IoBoundary`'s accumulated `FileEntry` digests, so the cache key invalidates whenever `config/routes.yml`'s SHA-256 changes. `--cache-stats` shows `plugin.routes.route_table` activity end-to-end.
- **The "read first, cache_for second" pattern** is the key authoring detail and lives in `Routes#route_table` with a comment pointing at the canonical example in [`spec/rigor/plugin/cache_producer_spec.rb`](spec/rigor/plugin/cache_producer_spec.rb) — "invalidates when files read via io_boundary BEFORE cache_for change between calls".
- **Demo project** under `examples/rigor-routes/demo/` (`.rigor.yml` + `config/routes.yml` + `lib/route_helpers.rb` + `demo.rb` + `errors_demo.rb`). Run with `RUBYLIB=$PWD/../lib bundle exec rigor check --cache-stats` from inside the demo directory; first run reports `1 miss / 1 write`, second run reports `1 hit / 0 writes`.
- **Integration spec** at [`spec/integration/examples/routes_plugin_spec.rb`](spec/integration/examples/routes_plugin_spec.rb) — 13 examples covering recognised helpers, unknown-route + did-you-mean, arity mismatches, the trust+cache surface (cache hit on a second run, cache invalidation when `routes.yml` content changes, graceful `:warning` when the routes file is missing).

### Added — example plugin: `rigor-units`

- **Second worked example of the v0.1.0 plugin authoring surface** under [`examples/rigor-units/`](examples/rigor-units/). Where `rigor-lisp-eval` types a literal AST argument, `rigor-units` shows how to **track types through the program** — propagating dimensional types through local-variable assignments, method chains, and arithmetic. The plugin types a units-of-measure DSL (`100.kilometers`, `2.hours`, `distance / time`, `60.kilometers.per_hour`, `speed.in_kilometers_per_hour`) and recognises four dimensions: `Distance`, `Time`, `Speed`, `Acceleration`. Recognised operations include same-dimension arithmetic, dimensional division (`Distance / Time → Speed`, `Speed / Time → Acceleration`), dimensional multiplication (`Acceleration * Time → Speed`, `Speed * Time → Distance`), chained constructors (`Distance.per_hour → Speed`, `Distance.per_second_squared → Acceleration`), and matching-dimension comparisons. Mismatches surface as `:error` diagnostics — `Distance + Time`, `speed.in_meters` (Speed has no `.in_meters`), `distance <= time` — under the `plugin.units.dimension-mismatch` / `plugin.units.in-method-mismatch` rules. Successful binding inferences and `.in_<unit>` queries surface as `:info` notes under `plugin.units.inferred-binding` / `plugin.units.in-method-result`.
- **Plugin layout** mirrors `rigor-lisp-eval` with one extra file: `lib/rigor/plugin/units.rb` (manifest + hook), `lib/rigor/plugin/units/method_table.rb` (pure dispatch table — `(receiver, method, args) → result_dim or error`), and `lib/rigor/plugin/units/analyzer.rb` (the AST walker with the local-variable binding map).
- **Demo project** under `examples/rigor-units/demo/` (`.rigor.yml` + `demo.rb` + `lib/units.rb` + `sig/units.rbs`) reproducing the canonical "speed / acceleration / gravity" worked example. Run with `RUBYLIB=$PWD/../lib bundle exec rigor check demo.rb` from inside the demo directory.
- **End-to-end integration spec** at [`spec/integration/examples/units_plugin_spec.rb`](spec/integration/examples/units_plugin_spec.rb) — 16 examples covering local-binding inference, `.in_<unit>` query results, dimensional mismatches in `+` / `-` / `*` / `/` / comparison operators, and the full demo file end-to-end (zero error diagnostics, every dimensional local present, every `.in_*` annotated). Treat as the executable contract for the README's "What the plugin recognises" section.
- **Same scope note as `rigor-lisp-eval`.** The plugin only emits diagnostics — it does NOT replace the analyzer's inferred return type for the call sites. Once the v0.1.x plugin contribution surface for return-type narrowing ships, the same `Analyzer` body moves into `FlowContribution` bundles and the `untyped` boundaries in `demo/sig/units.rbs` become precise. Future direction also captured in the demo's RBS sig and README under "Future direction — lightweight HKT".

### Added — example plugin: `rigor-lisp-eval`

- **First worked example of the v0.1.0 plugin authoring surface** under [`examples/rigor-lisp-eval/`](examples/rigor-lisp-eval/). The plugin walks literal `Lisp.eval(...)` arguments — a small S-expression-style grammar (`:+`/`:-`/`:*`/`:/`, `:<`/`:>`/`:<=`/`:>=`/`:==`, `:and`/`:or`/`:not`, `:if`) — and emits an `info` diagnostic at every call site naming the statically-inferred return type (`Integer`, `Float`, `bool`, branch unions). Ill-typed forms (`Lisp.eval([:+, 1, true])`) surface as `error` diagnostics; non-literal arguments are passed through silently. The example exercises the manifest, `config_schema` (`module_name` / `method_name` / `severity` overrides), `#init(services)` config plumbing, `#diagnostics_for_file(path:, scope:, root:)` AST walking, and `Rigor::Analysis::Diagnostic` construction. The runner auto-stamps `source_family: "plugin.lisp-eval"` per ADR-7 § "Slice 5-B" so the qualified rule renders `[plugin.lisp-eval.inferred-return-type]` / `[plugin.lisp-eval.type-error]` in `rigor check`'s output.
- **Demo project** under `examples/rigor-lisp-eval/demo/` (`.rigor.yml` + `demo.rb` + `lib/lisp.rb` + `sig/lisp.rbs`). Run with `RUBYLIB=$PWD/../lib bundle exec rigor check demo.rb` from inside the demo directory.
- **End-to-end integration spec** at [`spec/integration/examples/lisp_eval_plugin_spec.rb`](spec/integration/examples/lisp_eval_plugin_spec.rb) that drives a real `Analysis::Runner` over the example plugin and pins the diagnostic shape per recognised form. Treat the spec as the executable contract for the README's "What the plugin recognises" section.
- **Scope note (intentional).** The plugin only emits diagnostics — it does NOT replace the analyzer's inferred return type for the call site. The plugin contribution surface for return-type narrowing (a slice that would let plugins emit `FlowContribution` bundles consumed by `Inference::MethodDispatcher`) is queued for v0.1.x; once it ships, the same `Interpreter` inside this example moves into a return-type contribution and the diagnostic-side surface stays as the user-facing trace. Documented at the top of the plugin's `lib/rigor/plugin/lisp_eval.rb` and in the README.
...
### Added — v0.1.0 carry-over: per-method Reflection cache (ADR-7 § "Slice 6-D")

- **`Rigor::Cache::RbsInstanceDefinitions` / `Rigor::Cache::RbsSingletonDefinitions`.** Per-class cache producers for `RBS::Definition` objects (instance and singleton sides). Each (class_name, signature_paths, libraries) combination lands as one cache entry; subsequent calls for the same class skip `RBS::DefinitionBuilder.build_instance` / `build_singleton` and load the post-build artefact via Marshal round-trip. Marshal-cleanness was enabled by the v0.0.9 C2 `RBS::Location` patch.
- **`Rigor::Environment::RbsLoader#instance_definition` / `#singleton_definition`** now consult the cache when `cache_store` is set (with the in-memory `@instance_definition_cache` keeping the per-process short-circuit on top). The new `#uncached_instance_definition` / `#uncached_singleton_definition` public accessors provide the cache-bypass entry the producers call into. Disk footprint for `bundle exec exe/rigor check lib` is roughly 200 MiB (1179 instance + 133 singleton entries for the analysed run); users run `--clear-cache` if needed. ADR-7 § "Slice 6-D" originally descoped this; the v0.0.9 NameError that gated it is fully diagnosed and fixed here.
- **Sig drift fixes** in `sig/rigor/type.rbs`, `sig/rigor/environment.rbs`, `sig/rigor/rbs_extended.rbs`, and `sig/rigor/inference.rbs` — the cache layer's awakening surfaced a backlog of v0.0.9+ runtime additions never reflected in the RBS sigs (`Rigor::Type::Combinator` `literal_string` / `non_lowercase_string` / `non_uppercase_string` / `non_numeric_string` / `non_empty_literal_string` / `literal_string_carrier?` / `literal_string_compatible?`; `Rigor::Environment::RbsLoader` `cache_store` / `build_env_for` / `constant_names` / `each_known_class_name` / `uncached_*_definition`; `Rigor::RbsExtended.read_flow_contribution`; `Rigor::Inference::Narrowing.narrow_for_fact`). Two analyser-limitation cases (`Class#manifest` lookup on a `<` Base check, `Gem::BasicSpecification#full_gem_path`) are marked with `# rigor:disable undefined-method` for now.
...
- **`PredicateEffect#to_fact` / `AssertEffect#to_fact`.** Lift the parser-side typed Effect carriers into Facts: class-name effects fold to `Nominal[<class>]`; refinement-form effects pass their `refinement_type` through directly. The `edge` (PredicateEffect) and `condition` (AssertEffect) fields don't surface on the Fact — the slot the fact lands in encodes the edge / routing.
- **`RbsExtended.read_flow_contribution` now populates slot payloads with `FlowContribution::Fact`.** The bundle's `truthy_facts` / `falsey_facts` / `post_return_facts` arrays carry Facts instead of the parser-side `PredicateEffect` / `AssertEffect`. Side-effect: assert effects now route by condition — `:always` → `post_return_facts`, `:if_truthy_return` → `truthy_facts`, `:if_falsey_return` → `falsey_facts` — closing a v0.0.9 imperfection where every assert effect landed in `post_return_facts` regardless of condition. The internal narrowing call sites (`Inference::Narrowing` etc.) keep reading the typed Effect carriers via `read_predicate_effects` / `read_assert_effects` for now; the consumer migration to `read_flow_contribution` + `Merger.merge` is slice 4b.
- **Public-API drift snapshot for `FlowContribution::Fact`** locks the canonical surface ahead of slice 4b.
...
- **`Rigor::Plugin::IoBoundary` analyzer-side helper.** Per-plugin service plugins are expected to flow file reads and URL fetches through. `#read_file(path)` validates the absolute path against the policy, returns the bytes, and accumulates a `:digest` `Cache::Descriptor::FileEntry` so contributions stay invalidatable alongside their inputs. `#open_url(url)` always raises `Rigor::Plugin::AccessDeniedError` while the policy is `:disabled`; the hook is reserved so slices 3-6 can lift the gate without re-defining the API. `#cache_descriptor` returns a fresh frozen `Cache::Descriptor` capturing the boundary's read history.
- **`Rigor::Plugin::AccessDeniedError`.** Public exception carrying `reason:` (`:read_outside_scope` / `:network_disabled`) and `resource:` (offending path or URL). Slices that detect richer denial categories may extend the reason set without breaking the public shape.
- **`Rigor::Plugin::Services` extensions.** `Services#trust_policy` exposes the per-run `TrustPolicy`; `Services#io_boundary_for(plugin_id)` returns a fresh per-plugin `IoBoundary`. Both attach to the same `Services` constructor; the slice-1 callers continue to work without supplying the new kwargs (defaults to a network-disabled CWD-only policy).
- **`.rigor.yml` `plugins_io:` section.** New top-level key with `network:` (only `disabled` accepted in slice 2) and `allowed_paths:` (extra absolute / project-relative paths plugins may read from beyond the project root + signature_paths + trusted-gem roots). Surfaced through `Configuration#plugins_io_network` / `#plugins_io_allowed_paths`. Defaults preserve the strictest stance.
- **`Analysis::Runner` trust-policy build.** The runner derives `trusted_gems` from the gem-name half of each `Configuration#plugins` entry and `allowed_read_roots` from the project root, signature paths, each trusted gem's `Gem::Specification#full_gem_path` (when loadable), and the user's `plugins_io.allowed_paths` extras. The built `TrustPolicy` lands on the `Plugin::Services` container handed to the loader.

Documentation: [`docs/internal-spec/plugin-trust.md`](docs/internal-spec/plugin-trust.md) (normative); drift snapshots updated for `Rigor::Plugin::Services` / `Rigor::Plugin::TrustPolicy` / `Rigor::Plugin::IoBoundary`.

### Added — v0.1.0 slice 1: plugin registration / loading

- **`Rigor::Plugin` namespace.** First slice of the v0.1.0 plugin contract per [ADR-2 § "Registration, Configuration, and Caching"](docs/adr/2-extension-api.md). Public surface: `Rigor::Plugin.register(plugin_class)` (gem-side entry), `Rigor::Plugin::Base` (subclassed by plugins; declares its `manifest` and overrides `#init(services)`), `Rigor::Plugin::Manifest` (frozen value object — `id` / `version` / `description` / `protocols` / `config_schema`, with `#validate_config`), `Rigor::Plugin::Services` (DI container — `reflection` / `type` / `configuration` / `cache_store`), `Rigor::Plugin::Registry` (read-side snapshot — `plugins` / `ids` / `find(id)` / `load_errors`), and `Rigor::Plugin::LoadError` (failure carrier). Internal: `Rigor::Plugin::Loader`. Spec at [`docs/internal-spec/plugin.md`](docs/internal-spec/plugin.md); drift snapshots at `spec/rigor/public_api_drift_spec.rb`.
- **`.rigor.yml` `plugins:` extension.** Each entry is now either a bare gem-name string (`rigor-rails`) or a hash (`{ gem:, id:, config: }`). `Configuration#plugins` normalises both forms; the hash form is required when one gem registers more than one plugin or when the user supplies a config block.
- **`Analysis::Runner` plugin loading.** The runner builds a `Rigor::Plugin::Services` container with the project `Reflection` / `Type::Combinator` / `Configuration` / `Cache::Store`, hands it to `Plugin::Loader.load`, and exposes the resulting `Plugin::Registry` via `Runner#plugin_registry`. Plugins that load successfully are inert in slice 1 — protocol hooks land in slices 3–6.
- **Loader failure isolation.** Per [ADR-2 § "Plugin Trust and I/O Policy"](docs/adr/2-extension-api.md), `Plugin::Loader` collects every `LoadError` (gem-load failure, missing registration, multi-registration ambiguity, duplicate ids, config-schema violation, `#init` raise) on the resulting `Registry`. `Analysis::Runner` converts each one into a `:error` `Diagnostic` with `source_family: :plugin_loader` and `rule: "load-error"`, then continues the analysis.
