...
RSpec.describe Rigor::Type::Top do
  ...
  describe "describe" do
    it "renders as \"top\" regardless of verbosity" do
      expect(described_class.instance.describe).to eq("top")
      expect(described_class.instance.describe(:long)).to eq("top")
    end
  end

  describe "erase_to_rbs" do
    it "erases to the bare RBS top form" do
      expect(described_class.instance.erase_to_rbs).to eq("top")
    end
  end

  describe "lattice membership" do
    it "is the top lattice point and nothing else" do
      top = described_class.instance
      expect(top.top).to eq(Rigor::Trinary.yes)
      expect(top.bot).to eq(Rigor::Trinary.no)
      expect(top.dynamic).to eq(Rigor::Trinary.no)
    end
  end
  ...
  describe "value semantics" do
    ...
    it "is not equal to a different lattice extreme" do
      expect(described_class.instance).not_to eq(Rigor::Type::Bot.instance)
    end
  ...
  describe "inspect" do
    it "renders a debug string naming the class" do
      expect(described_class.instance.inspect).to eq("#<Rigor::Type::Top>")
    end
  end
end
