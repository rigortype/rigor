...
- **Narrowing.** A condition whose value is a reader's binds `$_` on both edges to the reader's RBS return narrowed as that value is: `String` on the truthy edge, and `nil` on the falsey edge of `gets`. The condition is the call itself (`while gets`, `if gets`, `gets or break`, `next unless gets`, `!gets`) or a variable written with it (`while (line = gets)`); `until (line = gets).nil?` and other readings of the value do not narrow `$_`. The body of `while gets` reads `$_` as `String` on every pass, and after the loop `$_` is `nil` unless the body can `break`. The reader MUST be certain:
  - `gets` or `readline` without a block, on implicit `self` or `self.` whose method RBS places in `Kernel` or `IO`, or in a class RBS does not know, whose ancestry reaches `Kernel` through `Object`;
  - on `Kernel`, `ARGF` or `STDIN`, or on `$stdin` or `$<` while the file binds them to nothing but such a receiver;
  - or on a receiver whose inferred type is a class whose reader RBS places in `IO`, `StringIO`, `ARGF` or `Zlib::GzipReader`. `Tempfile` is not one: RBS declares it below `File`, but its `gets` is a Ruby forwarder (`DelegateClass(File)`), which sets the forwarder's `$_`. `Kernel`'s reader is private, so an explicit receiver that only inherits it raises instead of reading.
...
- **Invalidation.** A narrowing of `$_` MUST be forgotten wherever Ruby may set the slot before the read. The code that may set it is a call to `gets`, `readline` or `foreach` on any receiver; `sub`, `gsub`, `chop` or `chomp` on implicit `self`, which `ruby -n` / `-p` defines to edit `$_`; a `send`, `__send__` or `public_send` whose name is computed or a literal naming one of the first three; an `eval`, `instance_eval`, `class_eval` or `module_eval` of a String; a write to `$_`; and a `&expr` block argument that may run one in this body (`&:gets`, `&:readline`, or a proc other than the method's own `&block`, which it only forwards). It is read anywhere but inside a nested `def`, class or module body, a `defined?` operand or a root block, and it is forgotten:
  - after a statement whose own call, receiver chain, arguments or literal values may set it, and after a call whose block may;
  ...
  - on entry to a `while`, `until` or `for` body, and to a `while` or `until` predicate, when the body may set it, because the body runs again after it ran;
