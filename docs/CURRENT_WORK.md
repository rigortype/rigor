...
2. **O2 — macro template / heredoc-Ruby expansion.** Substrate floor + precision promotion LANDED through [ADR-16](adr/16-macro-expansion.md) slices 1–5a + slice 7 docs (commits 584ae85…56706a5) + slice 6a-TierB / 6b-TierC precision (commits d174fff / d7b1943). Tier A (block-as-method) + Tier C (heredoc template) + Tier B (trait-inlining registry) engine integration shipped via a new `SyntheticMethodIndex` + pre-pass scanner; Tier D ships contract only (engine integration queued per slice-5a deferment); Concern (`included do`) re-targeting handled in the scanner; slice 6 promotes Tier B emissions via their `origin_module:` provenance through RBS dispatch and Tier C plain-class `returns:` strings via `environment.nominal_for_name`. Three worked consumers landed: `rigor-sinatra` (Tier A), `rigor-dry-struct` (Tier C), `rigor-devise` (Tier B). Remaining work — **slice 5b** (Tier D engine integration: top-level `self_type` narrow + `bound_ivars` pre-binding for matched external files) and **full ADR-13 resolver-chain wiring** for parameterised / utility-type substrate returns — stays demand-driven. Grounding survey at [`docs/notes/20260515-macro-expansion-library-survey.md`](notes/20260515-macro-expansion-library-survey.md).
