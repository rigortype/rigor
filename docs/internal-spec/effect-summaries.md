...
**Fallback selectors** ([#1065](https://github.com/rigortype/rigor/issues/1065)). An edge MAY carry `FileCollection::Edge#fallback_selectors`, copied by the rule from the row's `callee_fallbacks:` table (`docs/internal-spec/plugin.md` § `callee_fallbacks:`). `Propagator::Index#resolve` asks `targets_for` the edge as recorded and, only when that answers nothing, asks it again with each fallback selector in order, taking the **first** non-empty answer. The requested selector therefore always wins where its unit exists, and a fallback never joins a second target beside it — the framework runs one template, and a second edge would put a label on the caller that no execution of the render produces. `taint_if_unresolved` is seeded only when every candidate failed, so the step stays monotone and independent of visit order. The member is in `freeze_edges`' sort key and bumped the cache identity to `schema:5`.
