...
| `effect.*` | Effect-label contracts ([ADR-103](../adr/103-effect-labels.md); the label language is [effect-labels.md](effect-labels.md)). The whole family is inert without an `effects:` block. `effect.envelope-exceeded`, `effect.unknown-label` and `effect.annotations-unchecked` are implemented (see the rows below). Two further identifiers are reserved with no implemented diagnostic: `effect.liskov-widened` — an override proving effects the envelope it inherits from a project-defined ancestor does not admit, both-sides-authored in the [ADR-35](../adr/35-override-signature-compatibility.md) sense, tracked by [#386](https://github.com/rigortype/rigor/issues/386); and `effect.discarded-pure-result` — the non-bang footgun, gated additionally by the catalogue's `raises` facet and shipping `:off` in every profile pending a corpus gate, tracked by [#390](https://github.com/rigortype/rigor/issues/390). The identifiers are reserved jointly with Steins, the sibling PHP analyzer, so a policy naming one reads the same against both. |
