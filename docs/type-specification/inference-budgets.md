...
When the receiver belongs to a gem the user opted into via `dependencies.source_inference:` (per [ADR-10](../adr/10-dependency-source-inference.md); analyzer contract: [`docs/internal-spec/dependency-source-inference.md`](../internal-spec/dependency-source-inference.md)), the dependency-source-inference tier MAY contribute a `Dynamic[T]` answer below the RBS tier instead of leaving the call as raw `Dynamic[top]`. The wrapping preserves the dynamic-origin provenance, so consumers MUST NOT rely on the static facet `T` as a ground-truth contract — RBS / RBS::Inline / generated stubs / plugin contracts always win on conflict, and the inferred shape is never round-tripped out as authored RBS.
...
The budget table above is **normative-for-v1 intent. As of this writing the configurable `budgets:` surface is not yet wired** — no `budgets:` key is parsed and the table's rows are not enforced. The cutoffs the engine actually applies today are the four below. Note that the three hard-coded guards are deliberately **not** rows in the `budgets:` table above — that table is the *configurable* surface, and these are fixed termination floors, so they live here instead:

| Guard | Value | Where | Configurable? |
|---|---|---|---|
| Recursion re-entry | effective depth 1 → fixpoint return summary (Kleene from `bot`, cap 3) instead of `Dynamic[top]`; value-pinned args unroll up to fuel 32 → folded constant ([ADR-55](../adr/55-recursive-return-precision.md) slices 1+2) | `ExpressionTyper#infer_user_method_return` (`BudgetTrace::RECURSION_GUARD`, `BudgetTrace::RECURSION_UNROLL_FUEL`, `BudgetTrace::RECURSION_FIXPOINT_CAP`) | No — a `(receiver, method)` re-entry guard. The in-cycle re-entry returns the assumed **fixpoint summary** (slice 2: a Kleene iterate seeded `bot`, joined and re-evaluated up to 3 times, the final round widening `Constant→Nominal`; non-convergence collapses to today's `untyped` via `RECURSION_FIXPOINT_CAP`), so a recursive `String`-builder returns `String` with no `Dynamic` contribution. When every argument is a `Constant` / `Tuple`-of-`Constant`, the key additionally extends with the argument *values* so distinct constant frames recurse under a hard 32-frame fuel + 64-node value-size cap (slice 1), exhausting back to the plain guard. Both remain hard, non-configurable termination caps — distinct from the configurable `recursion_depth` precision-unroll the table envisions |
| Ancestor-walk cap | 100 nodes | `ExpressionTyper::ANCESTOR_WALK_LIMIT` (implicit-self method resolution) | No |
| HKT reducer fuel | 64 steps | `Inference::HktReducer::DEFAULT_FUEL` ([ADR-20](../adr/20-lightweight-hkt.md)) | No (per-call `fuel:` argument, not `.rigor.yml`) |
| `dependencies.budget_per_gem` | 5000 method definitions (range 1250–20000) | [ADR-10](../adr/10-dependency-source-inference.md) source-walk cap | **Yes** — `.rigor.yml` `dependencies.budget_per_gem:`, opt-in via `dependencies.source_inference:` |

The remaining table rows — including the cost-bearing `union_size` and `structural_growth` — are not yet enforced. The target design, the wiring plan (Layer 1 doc/spec hygiene, Layer 2 measurement-gated wiring), and the on-hit `static.*`-diagnostic policy are recorded in [ADR-41](../adr/41-inference-budget-design.md); the supporting survey is [`docs/notes/20260603-inference-budget-reality-survey.md`](../notes/20260603-inference-budget-reality-survey.md). `RIGOR_BUDGET_TRACE` exposes per-run counts of the wired guards, including the ADR-55 slice 1 `recursion-unroll-fuel` exhaustion counter and the slice 2 `recursion-fixpoint-cap` non-convergence counter.
