...
- **New CheckRules rule families.** ~~`flow.unreachable-branch`~~ (closed in v0.1.2 Track 2; literal-only envelope), `flow.dead-assignment`, `flow.always-truthy-condition`, `def.ivar-write-mismatch`, `def.method-visibility-mismatch`, plus `def.return-type-mismatch` for type-carrier predicates. Each needs careful false-positive triage.
