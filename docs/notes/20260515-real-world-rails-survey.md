...
| O7 | landed (2026-05-15) | RBS env-build performance falls off a cliff when a `signature_paths:` entry duplicates a stdlib RBS declaration. Root cause traced through five rounds of bisection on a Mastodon controller analysis: gem-shipped `prism/sig/prism.rbs` redeclares `Prism::VERSION: String`, clashing with Rigor's bundled stdlib RBS (Ruby 4.0+ ships prism in core). `RBS::Environment.from_loader(...)...resolve_type_names` raises `RBS::DuplicatedDeclarationError`. Pre-fix, the `@state[:env] ||= ...` memo in `RbsLoader#env` did not capture the failure, so every subsequent `env` access (one per AST node touched during analysis) re-parsed the entire signature set — 390 builds in 35 s for one controller file. Fixed by memoising the failure with an `@state[:env_loaded]` flag, emitting a single user-facing warning naming the conflicting `.rbs`, and gating the nil-env case through every consumer (`each_known_class_name`, `class_decl_paths`, `constant_names`, `compute_class_known`, `build_instance_definition`, `build_singleton_definition`). Post-fix: same workload runs in 0.15 s (~550× speedup); failure surfaces cleanly to the user. Original cliff symptom on the Diaspora 16-paths experiment (11+ min) had the same root cause — duplicated decls across the 15 gem_rbs_collection paths and rigor's bundled stdlib. |
