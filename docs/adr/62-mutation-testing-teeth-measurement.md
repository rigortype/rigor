...
| `arity_extra` in the default operator set | **Dropped to opt-in, then restored** — most Ruby methods accept an extra arg (splat/optional), so it was an equivalent-mutant noise factory; the signature-arity guard the row asked for shipped 2026-09-03 (`Protection::Mutator::ArityGuard`) and `arity_extra` is back in the default set. |
