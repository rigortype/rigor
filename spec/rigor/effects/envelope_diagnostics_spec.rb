...
RSpec.describe "effect.envelope-exceeded over the envelopes fixture" do
  ...
  describe "what does not fire" do
    ...
    # #1363 — a write to `$_` or `$~` binds only the method's own special-variable slot, and a `$@` read reads the
    # rescued exception's backtrace as `e.backtrace` would; neither is a global effect.
