...
A unit is keyed on the side Ruby defines the method on. It also carries a second bit: whether its body runs on a class. That is the singleton bit § Origins and § Ownership read. For every unit the scanner keys, the two agree: a method of the singleton class runs on the class, and a method of the class runs on an instance. `Effects::DefinitionContext` computes them from two facts that Ruby keeps apart:

- A receiver-less `def` lands on the **default definee**. That is the class the lexically enclosing body opened, or its singleton class inside `class << self`. A method body does not move it, and neither does an ordinary block. So `def inner` inside `def self.outer` defines `Class#inner`, and inside a `def` in `class << self` it defines `Class.inner`.
- `define_method` and `attr_*` are calls on **`self`**. They define instance methods of whatever module `self` is. A `define_method` inside `def self.setup`, where `self` is the class, defines `Class#x`, and one inside `class << self`, where `self` is the singleton class, defines `Class.x`. A `define_method` block keeps the default definee it closed over. The scanner synthesises `attr_*` only where a class body calls it, on the side a `define_method` there would take.
...
Only syntax that names the new `self` moves either fact. That is `class << self` on the class, and a block given to `class_eval`, `instance_eval` or one of their aliases (`class_exec`, `module_eval`, `module_exec`, `instance_exec`) whose receiver is `self`, spelled or implicit, or `singleton_class`. `class_eval` makes its receiver both `self` and the definee. `instance_eval` makes its receiver `self` and the receiver's singleton class the definee. So a `def` inside `instance_eval` in a class body is `Class.x` while a `define_method` there is `Class#x`, and inside `singleton_class.instance_eval` a `define_method` or `attr_*` is `Class.x`. These are the rules [`macro-substrate.md`](macro-substrate.md) states for `ScopeIndexer`'s tables. Those tables record fewer of these shapes (no nested `def`, for one), and where they record a shape they put it on the same side.

Where the syntax does not say which class or object a definition lands on, the definition is **not a unit**:

- the block of `Class.new`, `Module.new`, `Struct.new` or `Data.define`, which defines into a class or module the call creates;
- an eval on any other receiver (`Other.class_eval { def x }`, `base.class_eval`, `obj.instance_eval`), and `class << expr` for any `expr` but `self` on the class;
- a `def obj.x` whose receiver is neither `self` nor a constant (a `def Const.x` is keyed on the enclosing class, as it always was);
- a method on one object: `def self.x`, `class << self` or `instance_eval` in an instance method, and `def self.x` or `class << self` at the top level, where `self` is `main`;
- a `def` whose definee is the singleton class's own singleton class: `def self.x` inside `class << self`, and a `def` in `instance_eval` on the singleton class;
- a `define_method` or `attr_*` where `self` is not a class. In an instance method that raises unless the object is a module; at the top level `define_method` defines a public method of `Object`, which the scanner does not key.

The list over-drops a few spellings that do name the class: `W.class_eval { … }` and `class << W` inside `class W` itself, and `self.class.class_eval { … }` in an instance method. The scanner does not resolve a constant against the enclosing class, so it reads them as any other receiver. Dropping a unit never adds a label; its callers lose the edge, as for any other unresolved call.

Whatever is nested inside such a definition or block is unplaced too, except a `class` or `module` body, which starts afresh: `class Inner` inside a `Class.new` block in `class Host` opens `Host::Inner`, as Ruby's lexical scope does, and is keyed there. The enclosing method still stops at the definition, and the definition's body belongs to no summary; an edge to it is dropped like any other unresolved ordinary edge (§ Propagation). Filing it under the enclosing class instead would join it to that class's real method of the same name, and put its effects on that method's callers. That is the common case, not a coincidence, because a `Class.new(self)` subclass overrides its parent's methods by design.
...
A `view:` key is deliberately **not** a `MethodKey` shape — because a template has no owner class and no selector, and spelling one would put a method in the snapshot that no call site can name. The scanner takes the file's whole body as that one unit rather than minting one per `def` (`Scanner#scan_template_unit`), resolving the unit's implicit-self calls against the plugin's declared `self_type:`; `Runner#effect_sources` traces the key back to the template file the user wrote.

`MethodKey.split` cannot be used to test for one, and #393 found out the hard way: the format segment gives the key a dot, so `MethodKey.owner("view:users/show.html")` answers `"view:users/show"` — a class name no run ever produced. Anything grouping keys by their **envelope-bearing owner** asks `MethodKey.envelope_owner` instead, which answers the unit key itself for a template unit, so a `view:` key is an `effects.envelopes:` subject in its own right. A `match:` entry selects it through the file `Runner#effect_sources` already recorded, and `EffectEnvelopePass` positions the finding at that template rather than at `.rigor.yml` (a view has no `def` for `EnvelopeCheck::Positions` to find). A `namespace:` entry never selects one, which is right — a view is not in a constant namespace. The seam is specified in [`macro-substrate.md`](macro-substrate.md) § Template units, including its cache identity and the bound that template units are re-analysed on every run rather than participating in `--incremental` dependents.
...
Besides the origin bundles, the ancestry and the taints, the scan records one further bit per unit, the **unclaimed** bit (#391). A call site is *claimed* when the catalogue, a plugin row or an imported envelope says what the callee does; a site none of them bounded is unclaimed. Because whether an unclaimed site matters depends on resolution the file cannot perform, the bit travels on the `Edge` (`FileCollection::Edge#unclaimed`) and the unit is marked directly only where there is no receiver class to key an edge on. It is **not a taint**: it does not enter `causes`, it does not move the exhaustiveness bit, and it is read by `rigor sig-gen`'s annotation emission and by nothing else — not the report, not the snapshot, not any diagnostic, not `Summary#trivial?`. See [`effect-labels.md`](../type-specification/effect-labels.md) § Emission for why emission alone needs it.
...
`mutate.local` requires the local to be freshly allocated (`[]`, `{}`, `""`, `+""`, `.new` on a class object as below, `.dup`, `.clone`, a lambda) and never to escape. The escape analysis is flow-insensitive and whole-body: a local that escapes anywhere disqualifies, even after the mutation. That is strictly more conservative than "escaped before the mutating call", which is the direction the false-positive budget runs ([ADR-5](../adr/5-robustness-principle.md)).
...
The receiver form rests on the same allocation witness as the local rule, and shares its remaining holes. A class's `new` can still return an object something else holds (a caching `def self.new`), an `initialize` can publish `self` (`@@all << self`), an object can override `class` to answer something that is not a class, and a project `dup` can return `self`. The singleton gate reads the bit the unit's body is scanned with (§ Which side a definition lands on), and there is one bit per unit. Two places are left where that bit says a class and Ruby's `self` is not one. The first is a block inside the body that rebinds `self`: it keeps the unit's bit, so in a singleton method `posts.instance_exec { new }` reads the proxy's `new` as the class's. The second is a `def Const.x` whose constant holds a plain object (`def EMPTY.next_node`), which is keyed and scanned as a singleton method of the enclosing class. (The price listed above is the opposite direction, a bit that says an instance where `self` is a class, and never proves a label it should not.) Where that method is the project's own, its summary still reaches the caller through its edge (`mutate.self`, `mutate.static`), so the caller is not trivial on the strength of the `mutate.local` alone; a gem's method gives no such edge.
...
The `includes` table is the **instance** ancestry that edge resolution, `super` and the `new` rule walk. `include` and `prepend` are calls on `self`, as `define_method` is (§ Which side a definition lands on). So one is recorded only where the syntax shows that `self` is the class the body opened, or `main` at the top level. Inside `class << self`, or a `class_eval` / `instance_eval` block on `singleton_class`, it mixes the module into the singleton class. That is what `extend` does, and the collection is as blind to it as to `extend` (§ Propagation). An `include` in an `instance_eval` block in a class body is recorded, because `instance_eval` moves the definee and leaves `self` the class.

Where that section leaves a definition unplaced (the block of `Class.new` and its kin, an eval on another receiver, `class << expr`), the syntax does not name the class the module goes into, and nothing is recorded either. Filing the module under the enclosing class would put it where `super` and the `new` rule look for that class. The class Ruby does put it in misses it, as it would miss an `extend`. That is `Other` for `Other.class_eval { include M }` and `K` for `K = Class.new(Base) { include M }`. It is also `W` itself for `W.class_eval { include M }` inside `class W`, which the context reads as an eval on any other receiver ([#1322](https://github.com/rigortype/rigor/issues/1322)). Unlike a dropped unit, a missed include can remove labels (§ Propagation, the `new` rule's bounds).

The same rules decide an aliased `initialize` (below). `alias` works on the default definee and `alias_method` on `self`, so either one inside `class << self` aliases a class method, as does `alias` in an `instance_eval` block. None of these records the opaque sentinel, and neither does an alias in an unplaced block. A block that rebinds `self` at run time without the syntax showing it is read as the enclosing body. An `include` in ActiveSupport's `class_methods do … end` therefore still files under the concern, although Ruby puts the module in `ClassMethods`, which the including class extends. One in `included do … end` files under the concern too, and the including class reaches it through the concern, in a different order from Ruby's.
...
**The `new` rule** ([#1039](https://github.com/rigortype/rigor/issues/1039)). `new` is the one selector in Ruby whose dispatch target is spelled under a different key: the collector records `Const.new` as `(Const, :singleton, "new")`, because that is what the typer saw, and the constructor the call runs is `Const#initialize`. A singleton `new` edge is therefore resolved as `initialize` on the instance side, through step 1's walk and nothing else:

- a project `def self.new` **wins**, resolved through the singleton ancestry so an inherited one wins too. It is the definition the call actually reaches;
- the receiver must be a class the project defines — it has a method, a declared superclass, or a recorded instance-side `include`. A gem class the project only calls `new` on is as undescribed as it was;
- `Class`, `Module`, `Struct` and `Data` are excluded by name. `Class.new` builds an anonymous class and runs `Class#initialize`, not the `#initialize` of a class the project happens to have reopened under that name;
- the closed-world subclass join is dropped **for a written constant receiver, and only for one**. `Base.new` names the class object it constructs, so `Sub#initialize` cannot run and joining it would put a proven label on the caller that no execution of that site can produce — the `super` argument rather than the ordinary-call one. An edge is keyed on the receiver's *type*, though, and `self.class.new`, a `Singleton[Base]` local's `.new` and a receiver-less `new` in a singleton body carry the identical tuple while genuinely constructing a subclass; `FileCollection::Edge#constant_receiver` (set by the scan from the Prism receiver node, and part of the resolution memo key) is what separates them. Every shape that is not a written constant keeps the join, over a subclass's `#initialize` and its `.new` alike;
- an ancestry the scan could not read declines outright. `FileCollection::OPAQUE_ANCESTOR` (`"<opaque>"`) is recorded by `Effects::AncestryRecorder` for three shapes: a superclass expression that is not a constant path (`class K < Struct.new(:a)`, `< Data.define(:a)`, `< DelegateClass(X)`, `< Sequel::Model(:t)`); a **call-valued constant assignment** (`Anon = Class.new(Base)`, `Point = Struct.new(:x)`, `Rec = Data.define(:x)`), which is the same shape spelled the other way round — without it a later `class Point; …; end` reopening is all the scan sees, and the class reads as project-known with no ancestry at all; and a class body that aliases the instance-side `initialize` (`alias initialize setup`, `alias_method :initialize, :setup`, either spelled with symbols or with strings — the scan models no aliases; one that aliases the singleton class's `initialize`, or sits in a block the syntax does not place, records nothing, § Per-file shape and merging). The sentinel matches no summary key, so every other walk steps over it; the constructor rule asks for it by name, because a load-time-built or aliased constructor is the opposite of an absent one. It is **sticky**: neither a later `class Anon < Base` reopening nor another file's spelling of the same class displaces it (`FileCollection.merge_all` keeps it), because `Anon = Class.new(Base) { def initialize; … end }` puts the constructor in a block the scan files under the enclosing namespace and cannot attribute to `Anon`, and because the answer must not depend on which file a run reads first. For `Class.new(Base)` the named superclass is recorded *beside* the sentinel, so the subclass index still files `Anon` under `Base` and a `self.class.new` in `Base` sees the unreadable constructor below it. It is asked for **upward** through the ancestry always, and **downward** through the subclass closure wherever the join applies — a subclass whose own constructor is unreadable is a key the join cannot find, and the join is the only reason a non-constant receiver may construct that subclass at all.

When the walk finds no `#initialize` and the ancestry **closes inside the project** — every superclass is a class the project defines, up to an implicit or explicitly written `Object` / `BasicObject`, and no class in the chain has a recorded instance-side `include` — the constructor is `BasicObject#initialize`, whose footprint is ∅. The edge then resolves to nothing *and* the caller is claimed: the callee was read and there is nothing in it. **No summary row is invented** for `BasicObject#initialize`; a row would list a method of the project in the snapshot, in `rigor effects` and in the pure report for a definition the project does not contain, and the answer belongs to the edge, which is already what `owner_resolved?` is asked about. An ancestry that leaves the project — a gem's base class, or any recorded `include`, whose as-written candidate spellings the collection flattens and so cannot attribute to a project module — resolves to nothing and keeps marking its caller unclaimed, because the constructor it inherits really was not read.

Three bounds on that reading, because this rule is the first place where a partial analysis can make emission *more* generous rather than less:

- **"the project" means the analysed set.** A narrowed `paths:`, or a run over a subdirectory, can leave a class's `#initialize` in a file the run never read while the class itself is known from a file it did — and the answer is then "claimed, empty" for a constructor that is neither. Every other resolution answer degrades the safe way under a partial run (an unresolved edge is dropped and its caller stays unclaimed); this one does not. A project that emits annotations should do it from a run over its whole source.
- **`extend` is invisible**, and so is an `include` or `prepend` inside `class << self` or a block on `singleton_class`, which mixes in the same way (§ Per-file shape and merging). The collection records neither, so a singleton `new` supplied by either is not found by the `def self.new` step, and the rule resolves to `#initialize` instead. When the ancestry has no `#initialize` and closes inside the project, that is the empty constructor, and the caller is claimed even where the mixin's `new` does work. `rigor sig-gen` can then write `%a{pure}` on that caller ([#1325](https://github.com/rigortype/rigor/issues/1325)). The same blindness bounds singleton `super` resolution (§ Propagation, `resolve_super`). The walk steps over the module, so a singleton `super` reaches the superclass chain's definition without the module's labels, and taints only when the chain has none.
- **An include or alias the scan leaves unplaced is missed by the class Ruby puts it in** (§ Per-file shape and merging), on the instance side too. For `W.class_eval { include M }` or `W.class_eval { alias_method :initialize, :setup }` inside `class W` that class is `W` ([#1322](https://github.com/rigortype/rigor/issues/1322)). Then `W.new` can be claimed empty although `M#initialize` or the aliased `setup` does work, and a `super` in `W` that Ruby sends into `M` resolves past it and stays exhaustive without `M`'s labels. A dropped unit only loses an edge, but a missed include or alias can remove labels.
...
1. the walk starts *above* the enclosing class — its includes, then its superclass chain, with the class itself excluded, because a method never `super`s into itself. The includes are consulted for an instance-side `super` only: `include M` puts `M#m` where `super` looks, while `M.m` is a singleton method `include` never contributes. An `include M` where `self` is the singleton class is not in the table at all, because it puts `M#m` above the singleton class, where an instance `super` never looks;
...
Edge resolution is memoised on `(receiver class, kind, selector, super?, constant receiver?)`, and the transitive subclass closure on the class name. Both are required, not incidental: one tuple is asked for once per call site, and a Rails root class's subclass forest is otherwise re-walked thousands of times.
...
`direct_unclaimed(m)` is the unit bit joined with the edge-resolution verdict above; the lane is a monotone boolean OR like `exhaustive`'s AND, so it converges with the rest and no consumer but emission observes it.

The lattice is finite (label sets over a closed vocabulary × two bits × a closed cause enum) and every step is monotone, so a recursive or mutually recursive cycle converges on its own. **No recursion cap is used or wanted here** — unlike the return-type walk's Kleene iteration, there is no widening to force.
...
**A declaration's own annotation is re-attached before the buffer is handed over.** `RBS::Inline::Writer` emits a *member's* annotations as real annotation lines and a *declaration's* as nothing at all: it echoes the author's comment block above `class Foo` and stops there. So a class- or module-level bound — the cheapest one to write, and the only one that needs no per-method annotation — reached the reader as a comment, bounding nothing and saying nothing about it, while the identical `%a{pure}` above the same `class` in a `.rbs` file worked. `rigor-rbs-inline` rewrites the rendered buffer before returning it, re-emitting an echoed `# @rbs %a{…}` as an annotation line at the declaration's own indentation ([#452](https://github.com/rigortype/rigor/issues/452)). It reads only what the writer itself emitted, and three properties keep it literal rather than clever: a comment block is echoed only when its member is emitted, so an annotation the author detached with a blank line — which upstream drops — is never re-attached either; any non-comment line ends the pending run, so a member's own annotation, which the writer *does* emit between the echo and the `def`, cannot be claimed by a declaration and nothing is attached twice; and that same property makes the rewrite contribute nothing the day upstream emits the declaration annotation itself.
...
Consulted in `UnitScan#attribute_plugin`, beside the catalogue and beside `effects.attribution:`, on the same call. Three receiver shapes are tried, nearest-syntax first, and **every** shape is tried rather than the first that applies: a class name matched through the project's own superclass chain (`ActiveRecord::Base#save` reaching `user.save` on a `User < ApplicationRecord < ActiveRecord::Base`), a receiver path matched as written (`Rails.cache` + `read`), a self path scoped by `within:` (`self.session` + `[]=` inside a controller), and a `on_result:` row matched on the class that produced the receiver (`UserMailer.welcome(u).deliver_now`). The ancestry walked is the cross-file pre-pass's `discovered_superclasses` **and `discovered_includes`**, capped and cycle-guarded; the RBS ancestor chain is deliberately not consulted.

Included and prepended modules are ancestors here because a framework's contract is as often a module as a base class: a Sidekiq worker is `class TriggerWebhookWorker; include Sidekiq::Job`, with no base class at all, so a superclass-only walk could match no row about it however the plugin spelled one ([#456](https://github.com/rigortype/rigor/issues/456)). `ExpressionTyper#enqueue_ancestors` already resolves a self-call through both tables; the two disagreeing about what an ancestor is was the defect.

A chain that leaves project source is continued only by an **`effect_ancestry:` claim from a bundled plugin** ([#465](https://github.com/rigortype/rigor/issues/465)): `class UserMailer < Devise::Mailer` stops at a gem class, and `Devise::Mailer < ActionMailer::Base` is a line `rigor-devise` knows and the project never wrote. `parent:` need only be a **true** ancestor — the sole use of the ancestry is to make a row reachable and no plugin row is ever keyed on a project class, while insisting on the immediate parent would force a claim that is sometimes false (`Devise::SessionsController`'s real parent is `DeviseController`, whose own parent is the configurable `Devise.parent_controller`). Claims are walked **after** the project's own superclass, so a project declaration always wins the first-match race. Bundled plugins only, on the same grant `discharge:` reads: a claim carries no labels and so looks harmless, but it makes *other* plugins' rows reachable, and a third-party plugin asserting `Foo < ActiveRecord::Base` would pull first-party discharging rows onto `Foo`. A refused claim is warned about, never silent. The `PluginFacts` digest covers the claims, so a plugin upgrade that moves one re-colours summaries. The walk is breadth-first with modules ahead of the superclass, which is Ruby's own order, and `ANCESTRY_CAP` therefore bounds **work rather than depth** — it was 24 while only superclasses were walked, and Rails concerns make breadth ordinary enough (Mastodon's `Account` includes 21 modules) that a cap reachable by ordinary code silently drops the base class the row is written about.
...
- the edge resolves to nothing at propagation — `Propagator.taint_unresolved_callee` seeds the carried cause into the caller's state, the same way `taint_unresolved_super` does and for the same reason: only the merged table can answer. A `render partial: @thing` whose name is computed is the measured case; a layout was one until [#1047](https://github.com/rigortype/rigor/issues/1047) gave it a unit, and a `.js.erb` rendering an HTML-only partial was one until [#1065](https://github.com/rigortype/rigor/issues/1065) gave the edge a fallback.

**Fallback selectors** ([#1065](https://github.com/rigortype/rigor/issues/1065)). An edge MAY carry `FileCollection::Edge#fallback_selectors`, copied by the rule from the row's `callee_fallbacks:` table (`docs/internal-spec/plugin.md` § `callee_fallbacks:`). `Propagator::Index#resolve` asks `targets_for` the edge as recorded and, only when that answers nothing, asks it again with each fallback selector in order, taking the **first** non-empty answer. The requested selector therefore always wins where its unit exists, and a fallback never joins a second target beside it — the framework runs one template, and a second edge would put a label on the caller that no execution of the render produces. A candidate is not tried past a **declined** requested key — a key the run knows a template file for and has no summary for, supplied by `Analysis::TemplateUnits#declined_unit_keys` — because the file the framework runs is the one that produced nothing. `taint_if_unresolved` is seeded only when every candidate failed, so the step stays monotone and independent of visit order. The member is in `freeze_edges`' sort key and bumped the cache identity to `schema:5`.
...
The **effects identity** is the diagnostics identity plus the five inputs that change what a summary *means* without moving one analyzed byte: the **summary schema version** (a literal component, currently `schema:5`, bumped whenever a field is added to what a `FileCollection` persists or whenever edge resolution itself moves — an entry written before `Summary#unclaimed?` existed restores with the bit false, which is the direction emission must never guess, and an entry written before the `new` rule in § Propagation serves a closure resolved under the old one, so such an entry must not be served at all rather than read carefully), the vocabulary version (`Registry#vocabulary_version`), the catalogue identity (`Catalog#identity` — its schema and a content digest of `data/effects/core.yml`), the digest of the `effects:` block, and the **plugin fact digest** (`PluginFacts#digest` — every compiled label, attribution, edge and preset with the plugin that contributed it, #387; a plugin upgrade that re-colours `perform_later` re-colours summaries the source never moved). `Rigor::Effects::Identity` is the one place it is computed, in two spellings of the same inputs: `.digest` (a hex string, for a store with no descriptor of its own) and `.descriptor` (the run's key descriptor composed with one `configs:` entry carrying that digest). `Snapshot.config_digest` — the snapshot header's `config_digest` — is the same method, so the committed record and the cache key can never disagree about which `effects:` block produced it.

The unit of persistence is the **per-file `FileCollection`** — that is what per-file reuse is sound over, and it is all the ADR-46 incremental snapshot may carry: a recheck re-collects a partial closure, which a stored table cannot serve. The whole-run cache additionally persists the **propagated table**, because its invalidation already is what a stored closure needs — every analyzed file through the post-run dependency descriptor, every meaning input through the effects identity — so a warm hit re-runs neither the merge nor the fixpoint. An empty table stored over non-empty summaries would freeze one run's fail-soft propagation failure into every warm hit, so the writer stores no table in that corner and the hit re-runs the fixpoint from the collections instead.

The whole-run cache spends **two entries under one key descriptor and one dependency descriptor** (#482), because what a warm run reads and what a warm run stores are different sets. The summary entry carries the table, the unit sources and the merged as-written superclass table — everything a warm consumer asks for; the collections entry carries the per-file form that only an incremental recheck and the fail-soft re-propagation need. Splitting them is what lets a warm run skip a blob that is 4.8 MB on a monorepo, and the collections stay reachable behind a **loader**, forced by `Runner#effect_collections_by_path`, so a consumer the split did not anticipate loads them rather than reading an empty table. The summary is written last: a run interrupted between the two leaves collections whose summary miss simply re-collects, never a summary whose collections are absent.

Three slots carry it:

| Store | Slot | Keyed by |
| --- | --- | --- |
| ADR-45 whole-run cache | producer `analysis.run-effects-table` — the table, sources and ancestry a warm run serves from | `Effects::Identity.descriptor` — the `analysis.run-diagnostics` key descriptor plus the effects entry |
| ADR-45 whole-run cache | producer `analysis.run-effects` — the per-file collections, read lazily | the same descriptor |
...
A run with collection **on** is no longer excluded from the whole-run result cache. It consults the effects slots first, because those are what can force work, and in cost order: the summary entry serves outright; its absence is not yet a miss, because a fail-softed run and a pre-split entry both leave collections alone, so the second lane adopts those and re-runs the fixpoint over them; only when neither answers does the run re-analyze (collection is observational, so the only way to collect is to analyze) and write both. Either way the diagnostics slot decides for itself — an effects miss whose diagnostics entry is warm serves the warm diagnostics, and both effects entries validate against the same post-run dependency descriptor, so exactly the file set that invalidates diagnostics invalidates summaries.
...
Consequently `rigor effects` and the snapshot verbs go through the same cache `rigor check` does: after a `rigor check` under a configured `effects:` block, `rigor effects check` in the same job is a warm hit, and never re-collects.

Those surfaces have a probe of their own (ADR-104, `Analysis::EffectsCacheProbe`), and it is the same shape one question further out: the report and the snapshot verbs read the summary entry directly and never load the engine at all. What makes an engine-free key possible is that the effects identity is a pure function of things the engine does not produce — the diagnostics key descriptor the ADR-87 probe already computes, the shipped vocabulary and catalogue, the `effects:` block, and `PluginFacts#digest`, which is computed from the plugins' class-level declarations before the project's superclass table is even assigned to the object, so a run with no discovery tables digests the same value as one with them. `#prepare` is therefore not run: a plugin's contributions are fixed by `init`, which the loader runs, and `#prepare` is the half that parses routes and walks worker trees.

What the surfaces may serve is bounded by the same rule the two out-of-band passes above illustrate: **a probe may serve a surface only when every answer that surface can give is reproducible from stored values and declarations alone.** The report qualifies because it reads the table and the sources and nothing else — the envelope diagnostics an analysing run also computes are discarded by it — and the snapshot verbs because `Snapshot.build` takes exactly the table, the sources and the vocabulary, with `explain`'s edge paths already inside the stored table. Adding an effects answer computed from live analysis state means adding a field to the summary entry or a decline to the probe, in the same commit; the specs pin it by asserting `$LOADED_FEATURES` carries no engine entry on a served run, against an analysing control that loads ~90.
