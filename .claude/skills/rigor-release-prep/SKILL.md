...
Entry shape (single sentence per top-level bullet, subsystem label, user-facing
only) is documented in `AGENTS.md` § "CHANGELOG Style" — read that section
before writing or editing entries. The rules there are the canonical reference;
this file only covers the release-mechanics steps below.

At release time, review every `[Unreleased]` entry and rewrite or consolidate
any that still read like commit messages before sealing the version.
...
- `Rigor::VERSION` (`lib/rigor/version.rb`) and `Gemfile.lock` agree on the
  new version.
- `CHANGELOG.md` entries follow the style in `AGENTS.md` § "CHANGELOG Style".
  `[Unreleased]` entries that look like commit messages have been consolidated
  or rewritten before sealing the version.
