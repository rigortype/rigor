...
- **Two-arg / block-shaped fold dispatch.** Include-aware lookup landed in v0.0.5 and unlocks the catalog tier for `Integer#clamp(0..10)` etc. — but the underlying `try_fold` / `try_fold_binary` only handle 0- and 1-arg method calls. `Comparable#between?(min, max)` and Enumerable's block-taking methods (`select { |x| … }`, `inject(seed) { |memo, x| … }` etc.) would unlock once `try_fold` grows two-arg / block-aware paths.
