...
**v0.1.0 slice 5 (partial) landed on `master` (unreleased).** Qualified-rule text rendering: `Diagnostic#to_s` now appends `[<source_family>.<rule>]` when the diagnostic carries a non-builtin source family (per [ADR-2 § "Plugin Diagnostic Provenance"](adr/2-extension-api.md)). Loader failures (`:plugin_loader` family, `load-error` rule) and any future `plugin.<id>.<rule>` / `rbs_extended.<rule>` / `generated.<provider>.<rule>` diagnostics surface their provenance in standard `rigor check` output without changing built-in-rule layout. **Plugin emission protocol intentionally deferred** — the matching hook on `Rigor::Plugin::Base` carries plugin-protocol design decisions tied to slices 4 / 6.

**v0.1.0 slice 4 (substrate / 4a) landed on `master` (unreleased).** Per [ADR-7 § "Slice 4-A"](adr/7-v0.1.0-slice-decisions.md): canonical `Rigor::FlowContribution::Fact` value object (`target_kind` / `target_name` / `type` / `negative` + `#target` merge-key accessor); `PredicateEffect#to_fact` and `AssertEffect#to_fact` translation methods; `RbsExtended.read_flow_contribution` now populates slot payloads with Facts instead of the parser-side typed Effect carriers, with the assert-effect condition routing the slot (closing a v0.0.9 imperfection). Public-API drift snapshot pins the Fact surface. The internal-narrowing consumer migration to `read_flow_contribution` + `Merger.merge` is **slice 4b** (separate commit). Spec at [`docs/internal-spec/flow-contribution-merger.md`](internal-spec/flow-contribution-merger.md). **Working state: 1860 RSpec examples / 0 failures, RuboCop 196 files / 0 offenses, `bundle exec exe/rigor check lib` reports 0 diagnostics.**
...
4. **FlowContribution wiring through internal narrowing.** ⚠️ partial — substrate (slice 4a) shipped: `Rigor::FlowContribution::Fact` value object + `PredicateEffect#to_fact` / `AssertEffect#to_fact` translation + `RbsExtended.read_flow_contribution` populating slot payloads with Facts. Per-call-site merger wiring of the eight `Rigor::RbsExtended::*` consumer call sites (slice 4b) is the remaining work. Working decisions pinned by [ADR-7 § "Slice 4"](adr/7-v0.1.0-slice-decisions.md).
5. **Plugin diagnostic emission protocol.** ⚠️ partially landed — qualified-rule text rendering shipped on `master` at `ef730b2`. Working decisions for the remaining emission-side surface pinned by [ADR-7 § "Slice 5"](adr/7-v0.1.0-slice-decisions.md): per-file flat hook (`Plugin::Base#diagnostics_for_file`) as the v0.1.0 MVP (5-A); runner auto-stamps `source_family: "plugin.<id>"` (5-B); `Conflict#to_diagnostic(path:, line:, column:)` carries the merger conflict → diagnostic conversion (5-C). The `Rule<TNode>` node-scoped surface stays deferred to v0.1.x.
6. **Plugin-side cache producers.** Working decisions pinned by [ADR-7 § "Slice 6"](adr/7-v0.1.0-slice-decisions.md): DSL declaration (`Plugin::Base.producer`) + Services helper (`Services#cache_for`) hybrid (6-A); loader auto-attaches the per-plugin `PluginEntry` template (6-B); producer ids are auto-prefixed `plugin.<manifest.id>.` to sandbox the cache root (6-C); the v0.0.9 per-method Reflection cache re-attempt is descoped to a separate v0.1.x ticket (6-D).
