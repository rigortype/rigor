...
An `include` or `prepend` is recorded only where `self` is the class, because both are calls on `self`, as `define_method` is (§ Which side a definition lands on). Where `self` is the singleton class, inside `class << self` or a `class_eval` block on `singleton_class`, they mix the module into the singleton class. That is what `extend` does, and the `includes` table is the instance ancestry that edge resolution, `super` and the `new` rule walk, so such an include is not recorded. The collection is as blind to it as to `extend` (§ Propagation). An `include` in an `instance_eval` block in a class body is recorded, because `instance_eval` moves the definee and leaves `self` the class.
...
- **`extend` is invisible**, and so is an `include` or `prepend` where `self` is the singleton class, which mixes in the same way (§ Per-file shape and merging). The collection records only the instance-side `include` / `prepend`, so a singleton `new` supplied by either is not found by the `def self.new` step and the rule resolves to `#initialize` instead. With no `#initialize` in an ancestry that closes inside the project, that is the empty constructor, and the caller is claimed. A module's `initialize` mixed in this way is a class method that `new` never calls, and it is not found either. The same blindness already bounds singleton `super` resolution (§ Propagation, `resolve_super`).
