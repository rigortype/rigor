...
- `recv.foo(arg)` with **RBS::Extended predicate annotations** (Slice 7 phase 15): when the receiving method's RBS signature carries `%a{rigor:v1:predicate-if-true <target> is <Class>}` or `%a{rigor:v1:predicate-if-false <target> is <Class>}` annotations on its `RBS::Definition::Method`, the analyser MUST narrow the matching argument on the corresponding edge through `narrow_class`. The first-preview parser recognises a strict shape: `<target>` is a Ruby parameter name (matched against the selected overload's `required_positionals` / `optional_positionals`) or `self`; `<Class>` is a single optionally-namespaced class identifier (`String`, `Foo::Bar`, `::Foo`). Narrowing applies only when the matching call argument is a `Prism::LocalVariableReadNode` (the engine's narrowing surface). `self`-targeted effects are accepted by the parser but produce no scope edits in the first preview because the engine does not yet narrow `self`. Other directives (`assert`, `assert-if-*`, `param`, `return`, `conforms-to`) and negative refinements (`~T`) are deferred to follow-up slices.
