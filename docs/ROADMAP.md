...
- **External-author plugin SKILL.** Today's [`.claude/skills/rigor-plugin-author/`](../.claude/skills/rigor-plugin-author/) is **contributor-only** — it assumes the rigor monorepo's `Makefile`, `spec/integration/{plugins,examples}/`, `.rubocop.yml`, and `plugins/<id>/` / `examples/<id>/` placement, and references rigor-internal commands like `make verify` / `bundle exec exe/rigor check lib`. A separate SKILL aimed at **external library developers and project-specific plugin authors** — writing a standalone `rigor-foo` gem in their own repo that depends on `gem "rigortype"`, or maintaining a project-private plugin under their own `lib/` — is committed for the v0.2.0 cycle. Out of scope today (v0.1.x): no external `rigor-*` gems exist outside this monorepo; ADR-2 plugin contract surface still pre-1.0; subtree-split / RubyGems publishing flow stays internal. The external SKILL should cover (a) gemspec authoring against the published `rigortype` API surface, (b) test framework choice (RSpec / Minitest) without rigor's `plugin_helpers.rb`, (c) project-private plugin pattern (no gemspec, loaded via `Dir["lib/**/*.rb"]` or similar), (d) version pinning + breaking-change strategy, (e) `rbs_collection` integration for project-side types. Will likely live under a new top-level `skills/` tree (the slot previously vacated when this SKILL was moved back to `.claude/skills/`).
- **Baseline mechanism + `rigor-project-init` + `rigor-baseline-reduce` SKILLs** ([ADR-22](adr/22-baseline-and-project-onboarding.md)). Proposed (2026-05-19): PHPStan-shaped `.rigor-baseline.yml` recording `(file, rule, count)` snapshots of diagnostics known at baseline-generation time; `rigor check` filters baselined diagnostics out, surfaces only excess. Two new SKILLs scaffold the workflow: `rigor-project-init` (Gemfile / Gemfile.lock walk → plugin selection → severity profile → `.rigor.dist.yml` template → initial baseline → surface concentrated rules as likely real bugs) and `rigor-baseline-reduce` (per-rule walkthrough → sample-and-classify each site real-bug / stylistic / FP → apply fix / `# rigor:disable` / Rigor-side regression spec). New CLI subcommand family: `rigor baseline {generate, dump, drift, prune, regenerate}` + `--baseline=PATH` / `--no-baseline` on `rigor check`. Six implementation slices sketched; no slice scheduled by the ADR. Driven by the five-project survey ([`docs/notes/20260519-oss-library-survey.md`](notes/20260519-oss-library-survey.md)) where mature codebases routinely carry hundreds-thousands of diagnostics on first contact and a zero-diagnostic-required policy blocks adoption.
