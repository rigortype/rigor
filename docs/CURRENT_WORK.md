...
2. **O2 — macro template / heredoc-Ruby expansion.** Substrate floor LANDED through [ADR-16](adr/16-macro-expansion.md) slices 1–5a + slice 7 docs (commits 584ae85…56706a5). Tier A (block-as-method) + Tier C (heredoc template) + Tier B (trait-inlining registry) engine integration shipped via a new `SyntheticMethodIndex` + pre-pass scanner; Tier D ships contract only (engine integration queued per slice-5a deferment); Concern (`included do`) re-targeting handled in the scanner. Three worked consumers landed: `rigor-sinatra` (Tier A), `rigor-dry-struct` (Tier C), `rigor-devise` (Tier B). Remaining work — **slice 5b** (Tier D engine integration: top-level `self_type` narrow + `bound_ivars` pre-binding for matched external files) and **slice 6** (precision promotion: route Tier C `returns:` strings through ADR-13's `Plugin::TypeNodeResolver`) — stays demand-driven, gated on concrete plugin-author cases. Grounding survey at [`docs/notes/20260515-macro-expansion-library-survey.md`](notes/20260515-macro-expansion-library-survey.md).
...
| O2 | substrate floor LANDED (Tier A/B/C engine + Tier D contract + Concern re-targeting + 3 worked consumer plugins + docs); slice 5b + slice 6 queued | Macro-template / heredoc-Ruby expansion. Shipped through [ADR-16](adr/16-macro-expansion.md) slices 1–5a + 7 (commits 584ae85…56706a5). Substrate is functionally complete at the WD13 floor. tDiary's `instance_eval` plugin pattern + Rails-generator `.rb`-as-ERB templates stay relevant for the deferred slice 5b (Tier D engine). |
