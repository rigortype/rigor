...
| [config.md](config.md) | `.rigor.yml` semantics — the two co-equal sources of truth (`Configuration::DEFAULTS` + the JSON schema), the three validation tiers and their failure modes, reserved namespaces (schema-checked, never evaluated), and the reserve pipeline for a sibling implementation's keys (ADR-99). |
...
| [plugin-cache-producers.md](plugin-cache-producers.md) | Plugin-side cache producers — the `Plugin::Base.producer` DSL (incl. `watch:`), the record-and-validate `#cache_for` callable, descriptor auto-prefixing, and the `producer_value` / `producer_error` helpers. |
