...
- **DX tooling track.** ~~`rigor explain <rule-id>`~~ (closed in v0.1.2 Track 2; canonical / legacy / family lookup with text + JSON output), `rigor diff <baseline>`, `# rigor:disable-file <rule>`, `.rigor.yml` JSON schema for editor autocomplete. Separate user-facing surface; rest queue for v0.1.3+.
