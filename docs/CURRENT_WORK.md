...
> 0. **v0.1.18 — CI-environment support** (themed cut before v0.2.0; full surface in [`ROADMAP.md`](ROADMAP.md) § "v0.1.18 — CI-environment support"). **The CI diagnostic-output + templates half LANDED** ([ADR-51](adr/51-ci-diagnostic-output-formats.md)): `rigor check --format sarif|github|gitlab` (three CI-native renderings over the `--format json` fields, `lib/rigor/cli/diagnostic_formats.rb`) + the copy-paste CI setup templates under `docs/manual/ci-templates/` + the `docs/manual/11-ci.md` write-up. The two open scoping calls were resolved: new ADR (not an ADR-27 amendment), all three formats now with SARIF the recommended default. **Remaining for the cut:** the `--treat-all-as-inline-rbs`-style ergonomics ADR-51 defers (`--output FILE`, JUnit XML, richer SARIF rule metadata — all demand-gated), the ADR-27 § WD3 `setup-ruby` prebuilt-4.0 timing caveat, and whatever else the standing backlog contributes before tagging v0.1.18. **Do the rigor-release-prep skill for the actual cut.**
