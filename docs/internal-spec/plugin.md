...
| `rbs_complete_ancestors` | `Array<String>` | Class names whose RBS surface this plugin's `signature_paths:` declares COMPLETELY (ADR-43 WD4). A Ruby-source subclass of a listed class bridges inherited calls to the ancestor's RBS — the manifest-declared half of the `RbsDispatch` allow-list — so `call.undefined-method` / `call.wrong-arity` / `call.argument-type-mismatch` apply to inherited calls on every subclass. Only a class whose signature is genuinely exhaustive belongs here; pairing with `open_receivers:` covers a surface that is declared-complete for its own members but still open to plugin/user extension. |
