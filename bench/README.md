...
Files that drive the `make bench` perf-regression gate
...
| `baseline.updated.json` | **Not committed** (gitignored). The suggested baseline `make bench` writes when the committed baseline is uncalibrated, or for inspection. |
...
reported `nil` and the gate skips it, so local `make bench` still measures the
