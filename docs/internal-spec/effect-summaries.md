...
- **no closed-world override join.** This is the `super` argument rather than the ordinary-call one: `Const.new` names the class object it constructs, and `Sub#initialize` runs only for `Sub.new`, which records its own edge. Joining it would put a proven label on the caller that no execution of that call site can produce.
