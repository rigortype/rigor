...
- [ADR-48](docs/adr/48-data-struct-value-folding.md) — Struct / Data value folding (proposed — `Data.define` first; two new member-shape carriers `Type::DataClass` + `Type::DataInstance` (HashShape-shaped but nominal/class-tagged) so `Point = Data.define(:x, :y); Point.new(1, 2).x → Constant[1]`; precision-additive only — no diagnostic, no FP surface, degrades to `Dynamic[top]` on any uncertainty (block body / non-literal members / arity mismatch); reuses the existing `ScopeIndexer` `data_define_call?`/`meta_member_names` recognition that today only registers member *existence*; `Struct` deferred behind its mutation-soundness story (setters/`[]=` invalidate the instance member map; `Data`'s frozen instances are why it is first); four slices: DataClass → DataInstance + member-read folding → `class Point < Data.define(...)` subclass idiom → block-body degradation hardening)
