...
- They **contradict** only when the contradiction is **proven** — no value and no call satisfies both sides: some position's two types are **proven disjoint**, the two accept positional-argument counts that cannot meet while neither takes keywords, or one requires a keyword the other cannot take in any form (it has no `**rest`, no `*rest`, and no positional parameter a keyword Hash could fill). Two types are proven disjoint only by exact values and loaded classes: two distinct literal values, a literal outside a class, or two classes neither of which is an ancestor of the other. A module or an interface on either side never proves disjointness, since any class may include it; nor do two element types of one generic class (an empty `Array` is both an `Array[Integer]` and an `Array[String]`); nor does a subset, which is a refinement (`TrueClass` of `bool`). A block's parameter count never contradicts, since a block takes what its caller passes. The `.rbs` declaration binds, so the class still builds, and Rigor MUST report `rbs.contradicting-signature` as an error ([diagnostic-policy.md](diagnostic-policy.md)). A stale generated signature is the usual cause.
...
> **Partly implemented as of this writing ([ADR-112](../adr/112-extrbs-comment-channel.md) WD5).** The comparison above covers a `sig/` declaration against an inline `@rbs` / `#:` one ([#1075](https://github.com/rigortype/rigor/issues/1075)). The same rule between `@rbs` / `#:` and `@extrbs` waits on the `@extrbs` reader ([#1073](https://github.com/rigortype/rigor/issues/1073)). Class relationships are proven only for classes the analyzer process itself has loaded (core, stdlib and Rigor's own dependencies), so two project classes are undecided rather than contradictory.
