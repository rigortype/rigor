...
1. **Finish slice 1.** **Slice 1b landed** (the ancestry edge): a `class_sources` map (`class_name → Set<declaring file>`) is built in `ScopeIndexer.accumulate_project_index`, seeded onto the recording scope, and `Scope#superclass_of` / `#includes_of` record the full declaring-file set on resolution (a class reopened across files → consumer depends on every reopening; over-records by design). The same change fixed a latent slice-1a gap — the ADR-44 single-allocation body scopes (`build_user_method_body_scope` / `build_fresh_body_scope`) dropped `discovered_def_sources`, so the method-body edge was silently unrecorded for implicit-self ancestor-resolved calls; both now carry `def_sources` + `class_sources`. Off by default, `make verify` green, diagnostics byte-identical. **Still TODO in slice 1:** (a) the discovered-*class* lookup edge — class-name resolution reads the `discovered_classes` hash directly (no choke-point accessor), so a file's dependency on `class Foo`'s *existence* (vs its ancestry) is not yet recorded; needs either a choke-point accessor or a negative-edge-on-class-resolution pass. (b) persist `deps` / `dependents` + per-file diagnostic entries (reuse ADR-45's `Cache::Store#fetch_or_validate`). (c) the **mandatory** `--verify-incremental` cross-check mode (incremental vs full `--no-cache` must be byte-identical — the acceptance gate).
