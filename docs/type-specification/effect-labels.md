...
"Where that bound was written" is a position in a file the author has: the `.rbs` line for a signature-tree declaration, the `.rb` line the comment sits on for an rbs-inline one, and `.rigor.yml effects.envelopes[N]` for a configured one. An rbs-inline annotation reaches the reader inside a *synthesized* signature derived from the `.rb`, and the line it occupies there is never reported — that document does not exist on disk, and on a file with a licence header the number lands inside the copyright notice, where being wrong is not even visible ([#432](https://github.com/rigortype/rigor/issues/432)). The declaration reference is the actionable half of the message, so it must name a line the reader can open and edit. The same rule binds every surface that renders a declaration: `effect.liskov-widened`, `effect.unknown-label`, `effect.annotations-unchecked`, and the JSON and LSP renderings of all four.
...
An attribution row MAY additionally name a **callee rule** ([#1048](https://github.com/rigortype/rigor/issues/1048)), which is the same contribution keyed on a call *site* rather than on a class: `render :show` in a controller runs `app/views/users/show.html.erb`, which is an effect unit of its own, and the template it names is spelled by the literals the author wrote. Like argument-dependent narrowing, a rule reads the call's own argument literals — plus the unit's owner and its own key — and **nothing else**; the plugin supplies the rule's name and the engine owns the strategy, so the contribution stays declarative and the walk stays on the engine's side of [ADR-103](../adr/103-effect-labels.md) WD13.

Two consequences are normative. A rule that cannot settle the target from literals alone contributes **no edge**, and the row's `taint:` stands unchanged — "the target is computed" and "the target is not a template" are both "and possibly more". A rule that names a key the run's table has no unit for produces an edge that resolves to nothing, and the row's `taint:` is then **seeded at edge resolution** rather than at the site: the taint is added where the edge failed, never subtracted where it succeeded, so the fixpoint stays monotone and a render whose template was never analysed can never silently read as exhaustive. A rule MAY also answer for a whole **unit** rather than a site, which is what Rails' implicit render needs — the producing fact is a body that made no call at all. A unit rule contributes an edge and no labels, because a label read off an absence would colour every method the class happens to define.

A row MAY also carry the framework's **lookup order** for a rule's answer as data ([#1065](https://github.com/rigortype/rigor/issues/1065)): an ordered list of alternative selectors per selector, which Action View needs because a `.js` template's partial lookup is `[:js, :html]`. The rule decides whether the list applies — only to a target whose selector it inherited from context rather than read off the author's literals — and edge resolution decides where it lands: the requested selector first, then each alternative in order, and the edge's target is the **first** that resolves and nothing beside it, because the framework runs one template. An implementation MUST NOT try an alternative past a requested key it knows a source file for: "no unit answers" is not "no such target", and the framework runs the file that produced nothing. The row's `taint:` is seeded only when every candidate failed.
...
- Every callee in its closure is **claimed**: described by the catalogue, by a plugin row, by an envelope, or by a project definition the closure reads. Exhaustiveness answers "was every call resolved"; this answers "is every callee's footprint known", and the two come apart at exactly the site the model drops on purpose — an ordinary edge that resolves to no project definition is dropped rather than tainted, because most such calls are inherited or gem calls the catalogue has no row for. That posture is right for a report and for a judgment, both of which read the proven lane; it is wrong for a written bound, so emission MUST consult the second question. Implementations carry it as a bit beside the summary that NO other consumer reads (`docs/internal-spec/effect-summaries.md` § What a unit's scan records and § Propagation): it is not a taint, not a label, and MUST NOT change the exhaustiveness bit, the report, the snapshot or any diagnostic.
- The method carries **no authored bound of its own**. An envelope on the method, on its class, in `effects.envelopes:` (by `namespace:` or by `match:`) or in an rbs-inline annotation is the author's statement about this very method; it is checked rather than propagated, so it never enters any summary's `≤` lane and an emitter reading the table alone cannot see it. Writing an inferred annotation over it would replace a contract with an inference about the body the contract was written about. An annotation whose label the registry does not know counts as authored: it reads as ⊤ and bounds nothing, but overwriting it would remove the annotation the unknown-label report is attached to — the opposite of the repair the report asks for.
- Its **declared lane is empty** of labels the proven lane does not already admit. A callee's imported envelope lands in the caller's `≤` lane with no taint and no proven label, so a method whose whole body is one enveloped call reads `[] ≤ [io.net.http]`; annotating it `%a{pure}` would overwrite a claim the project carries with a bound nothing derived. The labelled spelling is withheld there too: the grammar carries one bound per declaration and cannot say "proves this, claims that".
