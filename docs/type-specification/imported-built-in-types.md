...
The four empty-witness refinements — `non-empty-string`, `non-zero-int`, `non-empty-array[T]` and `non-empty-hash[K, V]` — each remove one value (`""`, `0`, `[]`, `{}`) from their base, so they accept a type only when the base accepts it AND the type proves that value absent. The proof is read off the carrier: a literal other than the removed value; an `Integer` range that does not cover `0` (`positive-int`, `Integer[-5..-1]`); a tuple of non-zero arity (`[1, 2, 3]`), since a tuple's arity is fixed; a hash shape with at least one required key, open or closed (`{ name: 1 }`), since every inhabitant holds that key; the same refinement; an intersection with a member that is a proof; and a union whose every member is one. Anything else overlaps the removed value and MUST be rejected: the base itself (`Array[Integer]`), a range covering `0` (`non-negative-int`), the empty tuple or closed `{}`, a hash shape whose every key is optional (`{ ?name: Integer }` holds `{}`), or a union with any such member. A method whose `RBS::Extended` return is `non-empty-array[Integer]` therefore accepts a body of `[1, 2, 3]` or `flag ? [1] : [2, 3]`, and still reports `def.return-type-mismatch` on `flag ? [1] : []`.
