...
module Rigor
  module Inference
    ...
    module ScopeIndexer
      ...
      # An index target stores the same way — a multi-assign `@h[k], x = …`, a `for @h[k] in xs` index and a
      # `rescue => @h[k]` reference alike; it is outside `NODE_CLASSES` only because the straight-line seam types
      # its stored value where the target sits, and this widening takes no value.
