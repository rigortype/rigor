...
- **[rigor-activerecord]** A PostgreSQL `db/structure.sql` is now accepted as a schema source, so `schema_format = :sql` projects (GitLab-class apps) are no longer inert.
  - When `db/schema.rb` is absent the plugin falls back to parsing `db/structure.sql` (configurable via `structure_sql_file`) into the same schema table the Ruby-DSL parser produces — column names, types, and Postgres array columns. Previously such a project loaded no schema at all: every AR call check was skipped, and ordinary relation chains degraded to `Array`, cascading false diagnostics. The DDL is read line-by-line (no SQL-parser dependency); an unmappable column type (custom enum, `tsvector`) degrades to `Object` rather than being dropped, and partition tables in non-`public` schemas are skipped.
  - A `serialize` / `mount_uploader` / custom-`attribute` column (whose runtime value is a rich object, not the SQL scalar) is no longer narrowed to its schema type — its instance reads stay lenient, so `note.position.diff_refs` and `diff.external_diff.store!` no longer read as `undefined-method`. Column *existence* checks (`where(col:)`) are unaffected. Declarations inside a concern's `included do … end` are covered.
...
[Unreleased]: https://github.com/rigortype/rigor/compare/v0.2.8...HEAD
