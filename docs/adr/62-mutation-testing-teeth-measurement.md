...
| Fire on **nilable** unions (`String?`) | **Deferred — conflicts with deliberate silence.** The N3 decision (`safe_navigation_undefined_method_spec.rb`) intentionally keeps `T \| nil` silent for undefined-method to avoid a cross-file project-def false positive — a class `method_present_anywhere?` does not fully eliminate. Open question for a future slice: narrow the silence to fire only when every non-nil arm is a fully-known core/stdlib class (needs a corpus FP study first). |
...
| Broad-fuzz mode (crash / hang / soundness-contradiction detection) | **Proposed / demand-gated** — same warm loop, aggressive random mutation, watching for `internal analyzer error:`, per-mutant timeout, and mutants that *remove or flip* a baseline diagnostic. Crash detection is nearly free (the Runner already rescues into one diagnostic). |
