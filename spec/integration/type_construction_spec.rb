...
RSpec.describe "Rigor type construction (integration)" do
  ...
  describe "fixtures/constant_path_write_nesting/ — a path write keyed and typed by its nesting" do
    ...
    # INSIDE the writing module, and the `class_eval` trio: the name the block writes, the
    # lexical name it must not be filed under, and the declined unnameable receiver — the
    # innermost-owned pair whose two spellings must answer alike, and the #540
    # mutation-widening pair that rides on the same key.
