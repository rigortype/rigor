...
3. **ADR-14 follow-up: observation-aware `initialize` stubs + kwarg / `.new` routing.** The Steep cross-check landed initialize-stub emission for non-trivial constructors (see CHANGELOG `[Unreleased]` "Cross-check sig-gen output with Steep + emit initialize stubs"). The stub currently hard-codes `untyped` at every parameter position because three orthogonal pieces are missing in the observation pipeline: **(a)** `Generator#initialize_stub_candidate` does not consult the `@observations` map (built only for `def`-method body-typing paths); **(b)** `ObservationCollector#record_call` routes `Class.new(args)` under `[Class, :new]` rather than `[Class, :initialize]`, so `.new` → `#initialize` implicit binding is broken at the observation layer; **(c)** keyword arguments at the call site fail `collect_arg_types` via `non_positional?` and discard the entire observation, so `.new(path: ..., mutating_selectors: ...)`-style kwarg-only calls land zero observations. Fixing all three together turns `MethodCatalog#initialize: (path: untyped, ?mutating_selectors: untyped) -> void` into a precise stub when `--params=observed` includes the source tree. Sized approximately ~30 LoC in `ObservationCollector` + ~35 LoC in `Generator` (split between `initialize_stub_candidate` and a new keyword-aware `render_initialize_param_list_with_observations`) plus 4–6 new specs.
