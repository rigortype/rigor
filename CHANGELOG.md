...
- **`Rigor::Reflection` consumer migration (pre-v0.1.0 slice).** Five engine-internal callers move from raw `scope.environment.rbs_loader` access to the `Rigor::Reflection` facade: `Analysis::CheckRules` (four call sites covering the undefined-method, wrong-arity, possible-nil-receiver, and argument-type-mismatch rules), `Inference::Narrowing#resolve_rbs_extended_method`, `Inference::StatementEvaluator#resolve_call_method`, `Inference::MethodDispatcher#user_class_fallback_receiver`, `Inference::MethodParameterBinder#lookup_rbs_method`, and `Inference::MethodDispatcher::RbsDispatch`'s `lookup_method` / `build_type_vars` helpers. The facade gains a few extra methods to support the migration: `rbs_class_known?` (RBS-only variant of `class_known?`, since the diagnostic-rule paths intentionally exclude source-discovered classes), `instance_definition` / `singleton_definition` (full RBS class-definition lookups for callers that walk method tables), `class_type_param_names` (for generic-method substitution), and an `environment:` kwarg on every reads method as an alternative to `scope:` (for dispatcher call paths that have only an environment in scope). Two `loader.rbs_loader` direct accesses remain in `Inference::MethodDispatcher::RbsDispatch#try_dispatch` / `block_param_types` because they are only presence checks, not data reads, and migrating them would add wrapper noise without simplifying the call sites.

- **`Rigor::Reflection` read-side facade (pre-v0.1.0 slice).** A new module joins Rigor's three reflection sources — `Environment::ClassRegistry` (registered Ruby `Class` / `Module` objects), `Environment::RbsLoader` (RBS-declared methods, hierarchy, constants), and `Scope` discovered facts (user-defined classes / methods / in-source constants from `ScopeIndexer`) — into one read-only API. The first-pass surface covers nine queries: `class_known?`, `class_ordering`, `nominal_for_name`, `singleton_for_name`, `constant_type_for` (in-source wins on collision with RBS), `instance_method_definition`, `singleton_method_definition`, `discovered_class?`, `discovered_method?`. Specs at [`docs/internal-spec/reflection.md`](docs/internal-spec/reflection.md). Internal call sites that read directly from `Scope` / `RbsLoader` continue to work unchanged; they migrate to the facade at their own pace. Provenance, the unified `MethodDefinition` carrier, and cache slice descriptors are explicitly deferred to the v0.1.0 plugin API per [`docs/design/20260505-v0.1.0-readiness.md`](docs/design/20260505-v0.1.0-readiness.md).

- **Tuple#zip per-position fold.** `[1, 2, 3].zip([4, 5, 6])` folds to `Tuple[Tuple[1, 4], Tuple[2, 5], Tuple[3, 6]]`. Short other-Tuples pad with `Constant[nil]` (matching Ruby's runtime semantics) and multi-arg `zip` produces wider per-position Tuples (capped at `MAX_ZIP_ARITY = 8`). Non-Tuple arguments decline so the RBS tier widens to `Array[Array[…]]`.

- **HashShape projections — `first` / `flatten` / `compact`.** Three additional ShapeDispatch handlers for closed shapes with no optional keys: `first` returns the first `[k, v]` pair as a 2-Tuple (or `Constant[nil]` for the empty shape); `flatten` produces the per-position `[k_1, v_1, k_2, v_2, …]` Tuple at depth 1; `compact` drops every entry whose value is `Constant[nil]`. `compact` declines on non-Constant values so the drop set stays decidable.

- **Empty hash literal carrier — `{}` → `HashShape{}`.** Mirrors the v0.0.6 empty-array literal change. `ExpressionTyper#type_of_hash` now resolves `{}` to the empty `HashShape{}` so the v0.0.7 HashShape projections (`empty?`, `count`, `first`, `keys`, `values`) fold against it. Both carriers erase to plain `Hash` for RBS interop.

- **`Array.new(n, value)` / `Array.new(n)` constant lift.** `MethodDispatcher#meta_new` gains an `array_new_lift` arm: when `n` is a small `Constant<Integer>` (capped at `ARRAY_NEW_TUPLE_LIMIT = 16`), the call lifts to a per-position `Tuple[…]`. `Array.new(3, 0)` folds to `Tuple[Constant[0], Constant[0], Constant[0]]`; `Array.new(3)` folds to `Tuple[Constant[nil], Constant[nil], Constant[nil]]`. Oversize `n` falls back to `Nominal[Array]` so the carrier stays bounded.

- **Pathname delegation folds.** `Pathname` joins `Type::Constant::SCALAR_CLASSES` (Pathname is immutable per its docstring), and `MethodDispatcher#meta_new` gains a `CONSTANT_CONSTRUCTORS` table that lifts `Pathname.new(Constant<String>)` to a `Constant<Pathname>` carrier. A curated set of pure path-manipulation methods on `Constant<Pathname>` receivers fold through dedicated `try_fold_pathname_unary` / `try_fold_pathname_binary` arms in `ConstantFolding`, bypassing the catalog's `:dispatch` classification (the Pathname Ruby prelude routes most methods through File / Dir, so static catalog purity says "dispatch" even for filesystem-independent operations). The folded set covers `to_s` / `to_path` / `to_str`, `basename` / `dirname` / `extname` / `cleanpath` / `parent` / `sub_ext`, `root?` / `absolute?` / `relative?`, `hash` / `inspect`, `+` / `join` / `relative_path_from`, and `<=>` / `==` / `eql?` / `===`. Filesystem-touching methods (`exist?`, `file?`, `read`, `stat`, …) are intentionally NOT folded because their answer depends on the analysis machine's filesystem, which is neither stable nor relevant to the analyzed program; `foldable_constant_value?` is widened to accept Pathname.

- **Tuple ↔ HashShape conversion folds.** Five new ShapeDispatch handlers turn the common shape-conversion idioms into precise carrier transitions instead of the wider RBS projections. `Tuple#to_h` folds a Tuple of 2-Tuples whose first element is a `Constant` to a closed `HashShape` (declining on duplicate keys, malformed inner pairs, or non-Constant keys); `HashShape#to_a` folds to a per-entry `Tuple[Tuple[Constant<k>, V], …]`; `HashShape#to_h` folds to the receiver shape itself; `HashShape#invert` folds to a key-value-swapped HashShape when every value is a `Constant<Symbol>` / `Constant<String>` (declining on Integer-valued shapes because `HashShape` keys must be `Symbol` / `String`, and on duplicate values that would alias under inversion); `HashShape#merge(other)` folds when both sides are closed shapes, with right-hand entries overriding left-hand entries on key collision.

- **Regexp literal lift to `Constant<Regexp>`.** `Type::Constant::SCALAR_CLASSES` now accepts `Regexp` and `ExpressionTyper#type_of_regexp` lifts non-interpolated `Prism::RegularExpressionNode`s into `Constant<Regexp>` (preserving the source via `node.unescaped` and the option flags via `node.options`). Interpolated regexes (`/foo#{x}/`) keep the conservative `Nominal[Regexp]` answer because their value depends on runtime interpolation. The lift activates the v0.0.7 string-array binary fold's Regexp arm end-to-end: `"hello,world".scan(/o/)` folds to `Tuple[Constant["o"], Constant["o"]]`. `foldable_constant_value?` is widened to accept Regexp, matching the carrier's expanded `SCALAR_CLASSES`.
