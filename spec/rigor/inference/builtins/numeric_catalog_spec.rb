...
RSpec.describe "Rigor::Inference::Builtins::NUMERIC_CATALOG" do
  subject(:catalog) { Rigor::Inference::Builtins::NUMERIC_CATALOG }
  ...
  describe "#safe_for_folding?" do
    ...
    it "approves C-body leaf methods promoted by static analysis" do
      ...
      expect(catalog.safe_for_folding?("Integer", :**)).to be(true)
    ...
    it "approves leaf_when_numeric methods (Integer#+ etc.)" do
      expect(catalog.safe_for_folding?("Integer", :+)).to be(true)
      expect(catalog.safe_for_folding?("Float", :+)).to be(true)
      expect(catalog.safe_for_folding?("Float", :<)).to be(true)
    end

    it "approves prelude trivial methods (literal returns)" do
      expect(catalog.safe_for_folding?("Integer", :integer?)).to be(true)
      expect(catalog.safe_for_folding?("Integer", :to_int)).to be(true)
      expect(catalog.safe_for_folding?("Float", :to_f)).to be(true)
    end

    it "rejects dispatch-classified methods (callout into user Ruby)" do
      # Numeric#abs delegates via `num_funcall0` -> dispatch -> rejected.
      expect(catalog.safe_for_folding?("Numeric", :abs)).to be(false)
      # Numeric#coerce dispatches into user-defined #coerce.
      expect(catalog.safe_for_folding?("Numeric", :coerce)).to be(false)
    end

    it "rejects block-dependent methods" do
      expect(catalog.safe_for_folding?("Integer", :upto)).to be(false)
      expect(catalog.safe_for_folding?("Integer", :downto)).to be(false)
      expect(catalog.safe_for_folding?("Integer", :times)).to be(false)
    end

    it "returns false for an unknown class or selector" do
      expect(catalog.safe_for_folding?("NoSuchClass", :foo)).to be(false)
      expect(catalog.safe_for_folding?("Integer", :no_such_method)).to be(false)
    end

    it "distinguishes singleton from instance methods" do
      # Integer.try_convert is a leaf singleton method.
      expect(catalog.safe_for_folding?("Integer", :try_convert, kind: :singleton)).to be(true)
      # The same selector is not registered as an instance method.
      expect(catalog.safe_for_folding?("Integer", :try_convert, kind: :instance)).to be(false)
    end
  end

  describe "#method_entry" do
    it "returns the full record for a known method" do
      entry = catalog.method_entry("Integer", :abs)
    ...
    it "returns nil for an unknown method" do
      expect(catalog.method_entry("Integer", :no_such)).to be_nil
    end
  end
end
