...
Theme: **three accepted-and-implemented ADRs unlock per-call-site precision uplift through cross-plugin facts, project-side monkey-patch recognition, and a foundation dry-rb plugin — plus the first cut of editor / IDE integration through a PHPStan-style buffer-binding contract.** Commits `3c99eed` → `9dd28a3`.
...
- **Multi-buffer editor mode** (`--buffer A=B --buffer C=D`). Defer until an LSP daemon multiplexes save events.
- **LSP daemon / persistent process / file-watch.** ADR-0 deferred LSP to a later phase. With editor mode v1 in place, the CLI shape is now a viable LSP backend; the daemon is its own design.
