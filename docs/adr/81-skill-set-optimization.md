...
- **WD4 — description length is a per-skill judgment.** `waza`'s cross-model-density advisory (≤60-word descriptions, lead with an action verb) is applied only to an over-broad catalogue entry, never as a blanket rule that would trade trigger recall — the skill-creator "pushy description" guidance still governs. At the 2026-07-05 decision, only `rigor-ask` (169 words) was a candidate for tightening; the 2026-09-16 audit applied the same judgment to every current description because their example-heavy trigger lists had become routing noise.
