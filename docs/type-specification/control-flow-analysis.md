...
`Kernel#loop` is declared `() { () -> void } -> bot` in core RBS, but it rescues a `StopIteration` its block raises and returns that exception's `result`, so the enumerator-draining idiom `loop { out << e.next }` returns normally (with `e`'s own `each` value) once `e` is drained ([#1107](https://github.com/rigortype/rigor/issues/1107)). A receiver-less, `self.`, or `Kernel.` call of `loop` whose dispatch answers `bot` MUST therefore be typed `untyped` (`StopIteration#result`'s declared type) for its normal completion, and `break` arms MUST be unioned with it as usual. The declared `bot` MUST be kept only when the block body provably cannot raise: every node in it is a literal (without interpolation), a local or instance-variable read or write, `self`, an array literal without a splat, `if` / `unless` / `&&` / `||`, parentheses, or a `break` / `next` / `redo` / `return`. Any call (operators and `[]` included), `yield`, `super`, constant read, splat, interpolation, or `rescue` may raise `StopIteration`, and so does `raise` itself, which the walk does not tell apart from `raise StopIteration`. A block-pass (`loop(&blk)`) has no body to inspect and completes too.
