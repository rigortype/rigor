...
| **Plugin contract** — `Plugin::Base` hooks + manifest fields (the [ADR-37](adr/37-plugin-interface-segregation.md) narrow protocols: `node_rule` / `dynamic_return` / `narrowing_facts` (renamed from `type_specifier` in ADR-80; the old verb is a deprecating alias removed in 0.3.0) + the declarative fields) and the read-side namespaces (`Scope`, `Type`, `Reflection`, `Environment`, …) | [`docs/internal-spec/public-api.md`](internal-spec/public-api.md), pinned by [`public_api_drift_spec.rb`](../spec/rigor/public_api_drift_spec.rb) | [ADR-2](adr/2-extension-api.md) + the plugin examples under [`examples/`](../examples/README.md) | **yes** — the narrow ADR-37 protocols (the deprecated fat hooks are removed pre-1.0, e.g. ADR-52 slice 5b / ADR-60) |
