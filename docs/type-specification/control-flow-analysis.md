...
- **Narrowing.** A condition whose value is a reader's binds `$_` on both edges to the reader's RBS return narrowed as that value is: `String` on the truthy edge, and `nil` on the falsey edge of `gets`. The condition is the call itself (`while $stdin.gets`, `if STDIN.gets`, `$stdin.gets or break`, `next unless $stdin.gets`, `!$stdin.gets`) or a variable written with it (`while (line = STDIN.gets)`); `until (line = STDIN.gets).nil?` and other readings of the value do not narrow `$_`. The body of `while $stdin.gets` reads `$_` as `String` on every pass unless a `redo` targets the loop (a `redo` runs the body again without testing the predicate), and after the loop `$_` is `nil` unless the body can `break`. The arms of a reader condition bind `$_` differently, so where they join — after `if $stdin.gets … end`, `$stdin.gets ? a : b` or `(1 if $stdin.gets)` — `$_` MUST be unbound, not `String?`; a `String?` there would report correct code that proves the line through a flag it set in the arm (`ok = $stdin.gets ? true : false; return unless ok; line = $_; line.chomp`). The reader MUST be certain:
  ...
  **Standard-stream reader assumption** ([ADR-117](../adr/117-standard-stream-reader-assumption.md)). Rigor assumes that the readers a program relies on are the core ones. A replacement the analysed code does not show is not modelled: `$stdin` reassigned in another file, a patch installed by a test helper, a String `class_eval` or a test double (`allow($stdin).to receive(:gets)`), a gem's injection, a receiver typed `IO` or `File` that is a subclass instance with a Ruby reader at run time, and a run mode the file cannot show (`load(file, M)`, a DSL's `instance_eval(File.read(f))`). Code that reads `$_` after a reader only works if the reader set it, so a program that swaps a Ruby reader in and still reads `$_` loses a report rather than gaining one. Counter-evidence in the analysed code still declines, as follows.

  An implicit-self or `self.` reader (`while gets`, `self.readline`) does not narrow `$_` yet, in a method or in the script body. ADR-117 decides that it will, under the same assumption, declining where the file shows that `self`'s reader may be Ruby's: a mixin into `main` or `Object` (`include Readline`, `class << self; include M; end`), a reader defined on `main` (`def self.gets`), a block whose `self` is rebound (`instance_exec`), or a class whose ancestry names a Ruby reader ([#1415](https://github.com/rigortype/rigor/issues/1415)). A program that defines or patches in a method named `gets` or `readline` gets no narrowing from that name on any receiver either: a reopened `IO` or `Kernel`, or a project object another file binds to `$stdin`, runs the program's Ruby method. That covers a `def` of the name anywhere in the program, and, in the file being read, a `define_method`, `define_singleton_method` or `alias_method` call on any receiver (or a `send`, `__send__` or `public_send` naming one) and an `alias`, whose method name, the new name for an alias, is `gets` or `readline` or is not a literal, since a computed name may be either (`$stdin.define_singleton_method(:gets) { … }`, `STDIN.singleton_class.define_method(:gets)`, `IO.define_method(name)`). A safe-navigation reader (`io&.gets`) narrows its truthy edge only: when `io` is `nil` it reads nothing.
