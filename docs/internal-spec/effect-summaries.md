...
The `includes` table is the **instance** ancestry that edge resolution, `super` and the `new` rule walk. `include` and `prepend` are calls on `self`, as `define_method` is (§ Which side a definition lands on), so one is recorded only where `self` is the class the body opened, or `main` at the top level. Inside `class << self`, or a `class_eval` / `instance_eval` block on `singleton_class`, it mixes the module into the singleton class. That is what `extend` does, and the collection is as blind to it as to `extend` (§ Propagation). Where that section leaves a definition unplaced (the block of `Class.new` and its kin, an eval on another receiver, `class << expr`), the module goes into a class no key names, and nothing is recorded either. The same over-drop applies: `W.class_eval { include M }` inside `class W` itself is missed, as an `extend` is. An `include` in an `instance_eval` block in a class body is recorded, because `instance_eval` moves the definee and leaves `self` the class. The same rules decide an aliased `initialize` (below). `alias` works on the default definee and `alias_method` on `self`, so either one inside `class << self` aliases a class method, as does `alias` in an `instance_eval` block. None of these, and no alias in an unplaced block, records the opaque sentinel. ActiveSupport's `class_methods do … end` is not a rebinding the syntax shows, so an `include` in it still files under the concern, although Ruby puts the module in `ClassMethods`, which the including class extends.
...
Two bounds on that reading, because this rule is the first place where a partial analysis can make emission *more* generous rather than less:
