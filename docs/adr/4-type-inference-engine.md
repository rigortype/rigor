...
- **Phase 2b — Class-method (singleton-scope) dispatch.** Adds a singleton-class type carrier (`Rigor::Type::Singleton[name]` representing the value of the constant `Foo` itself, not an instance of `Foo`). `ExpressionTyper#type_of_constant_read` and `type_of_constant_path` will return that carrier instead of `Nominal[name]`; `RbsDispatch` will branch to RBS singleton-method lookup on a singleton receiver. This is the structural fix for `Foo.bar` calls (currently they look up `Foo` *instance* methods, which is wrong and silently misses).
- **Phase 2c — Argument-typed overload selection.** Adds `Rigor::Type#accepts(other, mode:)` returning a Trinary, plus the matching `Rigor::Type::SubtypeResult`/`AcceptsResult` carriers. The dispatcher then picks the first overload whose parameter list accepts the actual argument types, instead of always picking the first overload.
- **Phase 2d — Generics instantiation.** Carries type arguments on `Rigor::Type::Nominal` (or in a sibling `Application` type) and threads them through `RbsTypeTranslator` so `Array[Integer]#first` can substitute `Element` and return `Integer?`.
