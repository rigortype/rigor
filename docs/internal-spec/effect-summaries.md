...
The `includes` table is the **instance** ancestry that edge resolution, `super` and the `new` rule walk. `include` and `prepend` are calls on `self`, as `define_method` is (§ Which side a definition lands on). So one is not recorded where the scanner reads `self` as the singleton class: inside `class << self`, or in a `class_eval` block on `singleton_class`. There it mixes the module into the singleton class, which is what `extend` does, and the collection is as blind to it as to `extend` (§ Propagation). An `include` in an `instance_eval` block in a class body is recorded, because `instance_eval` moves the definee and leaves `self` the class. The same split applies to an aliased `initialize` (below). `alias` works on the default definee and `alias_method` on `self`, so either one inside `class << self` aliases a class method, as does `alias` in an `instance_eval` block, and none records the opaque sentinel. A block that makes some other class `self` is not modelled: `Class.new { … }`, `Struct.new(:a) { … }`, `Data.define(:x) { … }`, `Other.class_eval { … }`, and ActiveSupport's `class_methods do … end`. An `include` inside one of those still files under the enclosing class, although Ruby puts the module elsewhere.
